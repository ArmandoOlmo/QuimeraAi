        ...
        try {
            ...
            if (usageDoc.exists()) {
                ...
                if (tenantDoc.exists()) {
                    ...
                    const used = tenantData.usage?.aiCreditsUsed || 0;
                    const limit = tenantData.limits?.maxAiCredits || plan.limits.maxAiCredits;
                    const remaining = Math.max(0, limit - used);
                    const percentage = limit > 0 ? Math.min(Math.round((used / limit) * 100), 100) : 0;
                    ...
                    // Usar valores por defecto del plan free
                    const freePlan = SUBSCRIPTION_PLANS.free;
                    setUsage({
                        used: 0,
                        limit: freePlan.limits.maxAiCredits,
                        remaining: freePlan.limits.maxAiCredits,
                        percentage: 0,
                        plan: freePlan.name,
                        planId: 'free',
                        color: getUsageColor(0),

  ...
  return (
    ...
    >
      ...
      <div className="relative flex items-center gap-3 p-3">
        {/* Badge sugerido */}
        {isSuggested && (
          <div className="absolute -top-0.5 right-3 bg-q-success text-white text-[10px]
        ...
        >
          ...
          <button
            ...
            className="absolute inset-0 bg-q-text/70 opacity-0 group-hover:opacity-100
...
const TemplatePreviewModal: React.FC<TemplatePreviewModalProps> = ({
  ...
  return (
    <div
      className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-q-text/80 backdrop-blur-sm"
      onClick={onClose}
    >
      <div
        className="bg-[#0d0514] rounded-2xl max-w-4xl w-full max-h-[90vh] overflow-hidden
                   border border-q-border/10 shadow-2xl"
        onClick={e => e.stopPropagation()}
      >
        {/* Header */}
        <div className="flex items-center justify-between p-4 border-b border-q-border/10">
          <div>
            <h3 className="text-xl font-bold text-white">{template.name}</h3>
            <p className="text-sm text-q-text-muted">{template.category || 'General'}</p>
          </div>
          <button
            onClick={onClose}
            className="p-2 hover:bg-q-surface/10 rounded-lg transition-colors"
          >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
              {template.thumbnailUrl ? (
                ...
                <div className="w-full aspect-video rounded-lg bg-gradient-to-br from-q-accent to-q-accent flex items-center justify-center">
            ...
            <div className="space-y-5">
              ...
              {template.description && (
                <div>
                  <h4 className="text-sm font-bold text-q-text-muted uppercase tracking-wider mb-1">
                    Descripción
                  </h4>
              ...
              {/* Paleta de colores */}
              <div>
                <h4 className="text-sm font-bold text-q-text-muted uppercase tracking-wider mb-2">
                  Paleta de Colores
                </h4>
                <div className="grid grid-cols-2 gap-2">
                  {[
                    ...
                    <div key={name} className="flex items-center gap-2">
                      <div
                        className="w-8 h-8 rounded border border-q-border/20"
                        style={{ backgroundColor: color }}
                      />
                      <div>
                        <p className="text-xs text-q-text-muted">{name}</p>
                ...
                <div>
                  ...
                  <div className="flex gap-6">
                    {template.theme?.fontFamilyHeader && (
                      <div>
                        <p className="text-xs text-q-text-muted">Headers</p>
                    ...
                    {template.theme?.fontFamilyBody && (
                      <div>
                        <p className="text-xs text-q-text-muted">Body</p>
                        <p className="text-white capitalize">
                  ...
                  <div className="flex flex-wrap gap-1">
                    {template.componentOrder
                      ...
                      .map(section => (
                        <span
                          key={section}
                          className="text-xs bg-q-surface/10 text-q-text-muted px-2 py-1 rounded capitalize"
                        >
                          {section}
                        </span>
                      ))
        ...
        {/* Footer */}
        <div className="flex items-center justify-between p-4 border-t border-q-border/10 bg-q-text/20">
          <button
            onClick={onClose}
            className="px-4 py-2 text-q-text-muted hover:text-white transition-colors"
          >
            Cancelar
          </button>
          <button
            onClick={onSelect}
            className="px-6 py-2.5 bg-q-accent text-black font-bold rounded-lg
                       hover:bg-q-accent transition-colors flex items-center gap-2"
          >
            <Check size={18} />
            Usar este template
          </button>
  ...
  return (
    <div className="space-y-4">
      ...
      {suggestedTemplateId && suggestedReasoning && (
        <div className="bg-q-success/10 border border-q-success/30 rounded-lg p-3 flex items-start gap-2">
          <Sparkles className="w-4 h-4 text-q-success mt-0.5 flex-shrink-0" />
          <div>
            <p className="text-q-success font-medium text-xs">Template recomendado para tu negocio</p>
            <p className="text-q-success/70 text-xs mt-0.5">{suggestedReasoning}</p>
          </div>
        </div>
      )}
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        ...
        <div className="relative">
          <Filter className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-q-text-muted" />
          <AppSelect
            value={selectedCategory || ''}
            onChange={(e) => setSelectedCategory(e.target.value || null)}
            className="bg-[#130a1d] border border-q-border/10 rounded-lg pl-9 pr-6 py-2
                       text-white text-sm appearance-none cursor-pointer focus:ring-2
                       focus:ring-q-accent/50 focus:border-q-accent/50 outline-none
                       min-w-[160px]"
          >
      ...
      <div className="flex items-center justify-between">
        ...
        {onSkip && (
          <button
            onClick={onSkip}
            className="text-xs text-q-text-muted hover:text-q-accent transition-colors flex items-center gap-1"
          >
      ...
      {sortedTemplates.length === 0 && (
        <div className="text-center py-8">
          <Layout className="w-10 h-10 text-q-text-muted mx-auto mb-3" />
          <p className="text-q-text-muted text-sm mb-2">No se encontraron templates</p>
          <button
            onClick={() => {
              setSearchTerm('');
              setSelectedCategory(null);
            }}
            className="text-q-accent hover:text-q-accent text-xs transition-colors"
          >
            Limpiar filtros
          </button>
        </div>

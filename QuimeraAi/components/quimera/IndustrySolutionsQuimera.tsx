...
const IndustrySolutionsQuimera: React.FC<IndustrySolutionsQuimeraProps> = ({
    ...
    const getDefaultIndustries = (t: any) => [
        {
            icon: 'Building2',
            name: t('quimera.industries.item1.name', 'Inmobiliarias'),
            title: t('quimera.industries.item1.title', 'Motor de Búsqueda de Propiedades'),
            description: t('quimera.industries.item1.desc', 'Directorio completo de propiedades con filtros avanzados, mapas interactivos y galerías de imágenes. Gestiona agentes, listados y leads desde un panel especializado.'),
            featuresList: t('quimera.industries.item1.features', 'Filtros avanzados (precio, ubicación, tipo)\\nMapas integrados y geolocalización\\nPáginas de detalle dinámicas por propiedad\\nFormularios de contacto asignados por agente'),
            imageColor: 'from-blue-900 to-black'
        },
        {
            icon: 'ShoppingBag',
            name: t('quimera.industries.item2.name', 'E-commerce'),
            title: t('quimera.industries.item2.title', 'Tiendas Online Completas'),
            description: t('quimera.industries.item2.desc', 'Vende productos físicos o digitales con un carrito de compras nativo, pasarelas de pago integradas y gestión de inventario en tiempo real.'),
            featuresList: t('quimera.industries.item2.features', 'Catálogo de productos dinámico\\nCarrito de compras y Checkout fluido\\nGestión de categorías y variantes\\nIntegración con Stripe y PayPal'),
            imageColor: 'from-purple-900 to-black'
        },
        {
            icon: 'UtensilsCrossed',
            name: t('quimera.industries.item3.name', 'Restaurantes'),
            title: t('quimera.industries.item3.title', 'Menús y Reservas Digitales'),
            description: t('quimera.industries.item3.desc', 'Digitaliza tu restaurante con menús interactivos, sistema de reservas online y gestión de pedidos para llevar o entrega a domicilio.'),
            featuresList: t('quimera.industries.item3.features', 'Menús digitales con categorías e imágenes\\nSistema de reservas de mesas integrado\\nPedidos online con horarios configurables\\nGalerías de platos de alta conversión'),
            imageColor: 'from-orange-900 to-black'
        },
        {
            icon: 'Briefcase',
            name: t('quimera.industries.item4.name', 'Servicios Profesionales'),
            title: t('quimera.industries.item4.title', 'Agendamiento y Portafolios'),
            description: t('quimera.industries.item4.desc', 'Perfecto para consultores, clínicas, salones y agencias. Muestra tus servicios, testimonios y permite a los clientes agendar citas automáticamente.'),
            featuresList: t('quimera.industries.item4.features', 'Calendario de reservas online\\nPortafolios dinámicos de proyectos\\nSecciones de equipo y servicios\\nEmbudos de captación de leads'),
            imageColor: 'from-emerald-900 to-black'
        }
    ];

    const displayIndustries = industries || getDefaultIndustries(t);
    ...
    const displayTitle = title || t('quimera.industries.title', 'Soluciones por Industria');
    const displaySubtitle = subtitle || t('quimera.industries.subtitle', 'Escribe el subtítulo aquí...');

    const [activeTab, setActiveTab] = useState(0);

    // Ensure activeTab is valid
    const safeTab = (activeTab >= 0 && activeTab < displayIndustries.length) ? activeTab : 0;
    const currentIndustry = displayIndustries[safeTab];
                ...
                <div className="flex flex-col lg:flex-row gap-8 md:gap-12 items-start">
                    ...
                    <div className="w-full lg:w-1/3 flex flex-col gap-2">
                        {displayIndustries.map((industry, index) => {
                            const displayIndustryName = industry.name || t('quimera.industries.item.name', 'Industria');
                            return (
                            <button
                                key={index}
                            ...
                            <div className="rounded-3xl p-8 md:p-10 relative overflow-hidden min-h-[500px] flex flex-col justify-center border" style={{ backgroundColor: cardBg, borderColor: cardBorder }}>
                                ...
                                <div className="relative z-10 max-w-xl">
                                    ...
                                    <h3 className="text-3xl font-bold mb-4 animate-fade-in-up font-header heading-caps" key={`title-${safeTab}`} style={{ color: cardText }}>
                                        {currentIndustry.title || t('quimera.industries.item.title', 'Título de la Industria')}
                                    </h3>
                                    ...
                                    <button className="group flex items-center gap-2 font-bold transition-colors animate-fade-in-up font-button button-caps" style={{ animationDelay: '0.3s', color: accentColor }} key={`btn-${safeTab}`}>
                                        {t('quimera.industries.explore', 'Explorar solución para')} {currentIndustry.name || t('quimera.industries.item.name', 'Industria')}

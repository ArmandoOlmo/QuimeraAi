    ...
    const handleAuthAction = async (e: React.FormEvent) => {
        ...
        if (authMode === 'login') {
            ...
            try {
                ...
                if (!isTestUser && data.user?.identities?.length === 0) {
                     // Supabase workaround: if user already exists but isn't confirmed, signUp might return a fake user or specific state
                     setError(t('auth.errors.emailInUse'));
                } else if (!isTestUser && data.session === null) {

/**
 * ProjectSelectorPage
 * Página completa para seleccionar un proyecto para ecommerce
 * Rediseñado con header dedicado, filtros y vistas grid/list
 */
                            ...
                            <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4 mb-6">
                                <div className="min-w-0">
                                    ...
                                    <p className="text-q-text-muted">
                                        {t('ecommerce.selectProjectDescription', 'Cada proyecto puede tener su propia tienda de ecommerce. Selecciona uno para comenzar.')}
                                    </p>
                                </div>
                                <button
                            ...
                            <div className="text-center py-16">
                                <div className="p-4 bg-muted/30 rounded-full w-fit mx-auto mb-6">
                                    {searchQuery ? (
                                        ...
                                        <Store className="text-q-text-muted" size={48} />
                                    )}
                                </div>
                                <h3 className="text-xl font-bold text-foreground mb-2">
                                    {searchQuery
                                        ? t('ecommerce.noProjectsFound', 'No se encontraron proyectos')
                                        : t('ecommerce.noProjectsYet', 'No tienes proyectos aún')}
                                </h3>
                                <p className="text-q-text-muted mb-6 max-w-md mx-auto">
                                    {searchQuery
                                        ? t('ecommerce.tryDifferentSearch', 'Intenta con otros términos de búsqueda')
                                        : t('ecommerce.createFirstProject', 'Crea tu primer proyecto para comenzar a vender')}
                                </p>
                                {!searchQuery && (
                                    <button
                                        onClick={() => setIsOnboardingOpen(true)}
...
const ProjectCard: React.FC<ProjectCardProps> = ({ project, onSelect, formatDate }) => {
    ...
    return (
        <PreviewOverlayCard
            ...
            cornerAction={<ShoppingBag size={18} aria-hidden="true" />}
        />
    );
};
...
const ProjectListItem: React.FC<ProjectListItemProps> = ({ project, onSelect, formatDate }) => {
    ...
    return (
        <button
            onClick={onSelect}
            className="w-full flex items-center gap-4 p-4 bg-q-surface/80 backdrop-blur-xl hover:bg-q-surface border border-q-border/60 hover:border-q-border rounded-2xl transition-all text-left group relative overflow-hidden"
        >
            <div
            ...
            <div className="flex items-center gap-2 quimera-status-card-link transition-colors">
                <span className="text-sm font-medium hidden md:block">
                    {t('ecommerce.openStore', 'Abrir Tienda')}
                </span>

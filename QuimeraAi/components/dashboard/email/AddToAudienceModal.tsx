    ...
    return (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-[100] flex items-center justify-center p-4 animate-fade-in">
            <div className="bg-q-surface rounded-2xl border border-q-border w-full max-w-md shadow-2xl animate-scale-in overflow-hidden">
                    ...
                    {success ? (
                        ...
                        <>
                            ...
                            <div className="relative">
                                <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-q-text-muted" size={18} />
                                <input
                                    type="text"
                                    value={searchTerm}
                                    onChange={(e) => setSearchTerm(e.target.value)}
                                    placeholder={t('email.searchAudiences', 'Buscar audiencias...')}
                                    className="w-full pl-10 pr-4 py-2.5 bg-muted/30 border border-q-border rounded-xl text-foreground focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary/50 transition-all text-sm"
                                />
                            </div>
                                    ...
                                    {filteredAudiences.map((audience) => (
                                        <button
                                            ...
                                            className={`w-full flex items-center gap-3 p-3 rounded-xl border transition-all text-left ${selectedAudienceId === audience.id
                                                    ...
                                                    : 'border-q-border hover:border-primary/30 hover:bg-muted/30'
                                                }`}
                                        >
                ...
                {/* Footer */}
                {!success && (
                    <div className="p-5 border-t border-q-border flex gap-3">
                        <button
                            onClick={onClose}
                            className="flex-1 px-4 py-2.5 border border-q-border rounded-xl text-foreground hover:bg-muted transition-colors font-medium"
                        >

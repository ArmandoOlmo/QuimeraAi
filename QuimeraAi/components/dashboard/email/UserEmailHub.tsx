    ...
    const renderCampaigns = () => (
        <div className="space-y-6">
            <div className="flex items-center justify-between">
                <div>
                    <h2 className="text-lg font-bold text-q-text">{t('email.hub.campaigns.title')}</h2>
                    <p className="text-sm text-q-text-secondary">{t('email.hub.campaigns.totalCampaigns', { count: campaigns.length })}</p>
                </div>
                <button
            ...
            <div className="bg-q-surface border border-q-border rounded-xl p-4">
                <div className="flex items-center gap-2 bg-q-bg/50 rounded-lg px-3 py-2">
                    ...
                    <input type="text" placeholder={t('email.hub.campaigns.searchPlaceholder')} value={campaignSearch} onChange={e => setCampaignSearch(e.target.value)} className="flex-1 bg-transparent outline-none text-sm text-q-text placeholder:text-q-text-secondary" />
                    {campaignSearch && <button onClick={() => setCampaignSearch('')} className="text-q-text-secondary hover:text-q-text"><X size={14} /></button>}
                </div>
            </div>
            {/* Campaign List */}
            {filteredCampaigns.length > 0 ? (
                <div className="bg-q-surface border border-q-border rounded-xl overflow-hidden">
                    <div className="divide-y divide-editor-border">
                        {filteredCampaigns.map(campaign => (
                            <div key={campaign.id} className="flex items-center gap-4 px-5 py-4 hover:bg-q-bg/50 transition-colors cursor-pointer group" onClick={() => { setDetailCampaign(campaign); setShowDetailPanel(true); }}>
                                <div className="flex-1 min-w-0">
                                    <p className="text-sm font-medium text-q-text truncate">{campaign.name}</p>
                                    <p className="text-xs text-q-text-secondary mt-0.5">{campaign.subject}</p>
                                    <p className="text-xs text-q-text-secondary mt-0.5">{formatDate(campaign.createdAt)}</p>
                                </div>
                                <div className="hidden md:flex items-center gap-6 text-xs text-q-text-secondary">
                                    {campaign.stats && campaign.stats.sent > 0 && (
                                        <>
                                            <div className="text-center">
                                                <p className="font-medium text-q-text text-sm">{campaign.stats.sent.toLocaleString()}</p>
                ...
                <div className="text-center py-16 bg-q-surface border border-q-border rounded-xl">
                    <Send size={48} className="mx-auto text-q-text-secondary mb-4 opacity-30" />
                    ...
                    <p className="text-q-text-secondary text-sm mb-6">
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4" onClick={() => setShowNewCampaignModal(false)}>
                    <div className="bg-q-bg border border-q-border w-full max-w-md rounded-2xl shadow-2xl" onClick={e => e.stopPropagation()}>
                        <div className="p-5 border-b border-q-border flex items-center justify-between">
                            ...
                            <button onClick={() => setShowNewCampaignModal(false)} className="p-1.5 hover:bg-q-surface-overlay/40 rounded-lg"><X size={18} className="text-q-text-secondary" /></button>
                        </div>
                    ...
                    <div className="w-full max-w-lg bg-q-bg border-l border-q-border shadow-2xl overflow-y-auto animate-in slide-in-from-right duration-300" onClick={e => e.stopPropagation()}>
                        <div className="sticky top-0 z-10 bg-q-bg border-b border-q-border p-5">
                            <div className="flex items-center justify-between mb-3">
                                <div className="flex-1 min-w-0">
                                    <h3 className="text-lg font-bold text-q-text truncate">{detailCampaign.name}</h3>
                                    <p className="text-xs text-q-text-secondary truncate">{detailCampaign.subject}</p>
                                </div>
                                <button onClick={() => setShowDetailPanel(false)} className="p-2 hover:bg-q-surface-overlay/40 rounded-lg transition-colors"><X size={18} className="text-q-text-secondary" /></button>
                            </div>
                        </div>
                        <div className="p-5 space-y-6">
                            {/* Status */}
                            <div>
                                <label className="text-xs font-bold text-q-text-secondary uppercase tracking-wider mb-2 block">{t('email.hub.campaigns.detailPanel.status')}</label>
                            ...
                            {detailCampaign.stats && detailCampaign.stats.sent > 0 && (
                                <div className="bg-q-surface border border-q-border rounded-xl divide-y divide-editor-border">
                                    <div className="flex justify-between items-center px-4 py-2.5 text-sm"><span className="text-q-text-secondary">{t('email.hub.campaigns.detailPanel.sentLabel')}</span><span className="text-q-text font-medium">{detailCampaign.stats.sent.toLocaleString()}</span></div>
                                    <div className="flex justify-between items-center px-4 py-2.5 text-sm"><span className="text-q-text-secondary">{t('email.hub.campaigns.detailPanel.openRateLabel')}</span><span className="text-q-text font-medium">{detailCampaign.stats.sent > 0 ? `${(((detailCampaign.stats.uniqueOpens || 0) / detailCampaign.stats.sent) * 100).toFixed(1)}%` : '—'}</span></div>
                                </div>
                            )}
                            {/* Email Preview */}
    ...
    const renderAudiences = () => (
        <div className="space-y-6">
            ...
            <div className="bg-q-surface border border-q-border rounded-xl p-4">
                <div className="flex items-center gap-2 bg-q-bg/50 rounded-lg px-3 py-2">
                    <Search size={16} className="text-q-text-secondary flex-shrink-0" />
                    <input type="text" placeholder={t('email.hub.audiences.searchPlaceholder')} value={audienceSearch} onChange={e => setAudienceSearch(e.target.value)} className="flex-1 bg-transparent outline-none text-sm text-q-text placeholder:text-q-text-secondary" />
                </div>
            </div>
            {/* Audience Cards */}
                    ...
                    {filteredAudiences.map(aud => (
                        <div key={aud.id} className="bg-q-surface border border-q-border rounded-xl p-5 hover:border-q-accent/30 transition-all">
                            ...
                            <div className="flex gap-2">
                                <button onClick={() => { audienceActions.setSelectedAudienceId(aud.id); audienceActions.setManualEmail(''); }} className="flex-1 text-xs px-3 py-2 bg-q-bg border border-q-border rounded-lg hover:border-q-accent/30 transition-colors text-q-text">
                                    {t('email.hub.audiences.manage')}
                                </button>
                                <button onClick={() => { audienceActions.setSelectedAudienceId(aud.id); audienceActions.setShowImportCSV(true); }} className="text-xs px-3 py-2 bg-q-bg border border-q-border rounded-lg hover:border-q-accent/30 transition-colors text-q-text flex items-center gap-1">
                                    <Upload size={12} /> CSV
                                </button>
                            </div>
                            ...
                            {audienceActions.selectedAudienceId === aud.id && !audienceActions.showImportCSV && (
                                <div className="mt-4 border-t border-q-border pt-4">
                                    <div className="flex items-center gap-2 mb-3">
                                        <input type="email" value={audienceActions.manualEmail} onChange={e => audienceActions.setManualEmail(e.target.value)} onKeyDown={e => { if (e.key === 'Enter') audienceActions.handleAddManualEmail(aud.id); }} placeholder="email@ejemplo.com" className="flex-1 bg-q-bg border border-q-border rounded-lg px-3 py-1.5 text-xs text-q-text focus:outline-none focus:ring-1 focus:ring-q-accent" />
                                        <button onClick={() => audienceActions.handleAddManualEmail(aud.id)} className="px-3 py-1.5 text-xs bg-q-accent/20 text-q-accent rounded-lg hover:bg-q-accent/30">{t('email.hub.audiences.add')}</button>
                                    </div>
                            ...
                            {audienceActions.selectedAudienceId === aud.id && audienceActions.showImportCSV && (
                                <div className="mt-4 border-t border-q-border pt-4">
                                    <label className="block text-xs font-medium text-q-text-secondary mb-2">{t('email.hub.audiences.importCSV')}</label>
                                    <input type="file" accept=".csv" onChange={e => { if (e.target.files?.[0]) audienceActions.handleCSVUpload(aud.id, e.target.files[0]); }} disabled={audienceActions.csvUploading} className="text-xs text-q-text" />
                                    {audienceActions.csvUploading && <p className="text-xs text-q-accent mt-1">{t('email.hub.audiences.importing')}</p>}
                                    <button onClick={() => audienceActions.setShowImportCSV(false)} className="mt-2 text-xs text-q-text-secondary hover:text-q-text">{t('email.hub.audiences.close')}</button>
                                </div>
                            )}
                        </div>
                    ))}
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4" onClick={() => audienceActions.setShowCreateAudience(false)}>
                    <div className="bg-q-bg border border-q-border w-full max-w-md rounded-2xl shadow-2xl" onClick={e => e.stopPropagation()}>
                        <div className="p-5 border-b border-q-border flex items-center justify-between">
                            ...
                            <button onClick={() => audienceActions.setShowCreateAudience(false)} className="p-1.5 hover:bg-q-surface-overlay/40 rounded-lg"><X size={18} className="text-q-text-secondary" /></button>
                        </div>
                        <div className="p-5 space-y-4">
                            <div>
                            ...
                            {tabs.map((tab) => {
                                ...
                                return (
                                    <button
                                        ...
                                        className={`w-full flex items-center justify-between px-3 py-2.5 rounded-lg text-sm font-medium transition-all duration-200 ${isActive ? 'bg-q-accent/10 text-q-accent shadow-sm' : 'text-q-text-secondary hover:text-q-text hover:bg-q-surface-overlay/30'}`}
                                    >
                                        <div className="flex items-center gap-3 truncate">
                                            <div className={`flex-shrink-0 ${isActive ? 'text-q-accent' : ''}`}>{tab.icon}</div>
                                            <span className="truncate">{tab.label}</span>
                                        </div>
                                        {tab.count !== undefined && tab.count > 0 && (
                                            <span className={`px-1.5 py-0.5 text-[10px] rounded-full font-bold ${isActive ? 'bg-q-accent/20 text-q-accent' : 'bg-q-surface-overlay/50 text-q-text-secondary'}`}>
                                                {tab.count}
                                            </span>
                                        )}
                                    </button>
                                ...
                                {tabs.map((tab) => {
                                    ...
                                    return (
                                        <button
                                            key={tab.id}
                                            onClick={() => setActiveTab(tab.id)}
                                            className={`flex flex-col items-center justify-center gap-1 py-2 px-3 rounded-lg text-[10px] font-medium transition-colors min-w-[70px] snap-center ${isActive ? 'bg-q-accent/10 text-q-accent' : 'text-q-text-secondary hover:text-q-text hover:bg-q-surface-overlay/30'}`}
                                        >
                            ...
                            <div className="w-full h-full p-4 sm:p-6 lg:p-8">
                                ...
                                <div className="flex items-center justify-center py-24">
                                    <div className="flex flex-col items-center gap-3">
                                        ...
                                        <span className="text-sm text-q-text-secondary">{t('email.hub.loadingData')}</span>
                                    </div>
                                </div>
                            ) : (
                                <>

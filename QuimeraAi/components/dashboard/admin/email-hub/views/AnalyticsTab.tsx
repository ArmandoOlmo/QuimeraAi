...
const AnalyticsTab: React.FC<AnalyticsTabProps> = ({
    ...
    <div className="space-y-6">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            {[
                { label: t('adminEmail.overview.emailsSent'), value: stats.totalSent.toLocaleString(), icon: <Send size={20} className="text-q-accent" />, bg: 'bg-q-accent/10' },
                { label: t('adminEmail.overview.openRate'), value: `${stats.openRate}%`, icon: <Eye size={20} className="text-q-accent" />, bg: 'bg-q-accent/10', sub: `${stats.opened.toLocaleString()} abiertos` },
                { label: t('adminEmail.overview.clickRate'), value: `${stats.clickRate}%`, icon: <MousePointer size={20} className="text-q-accent" />, bg: 'bg-q-accent/10', sub: `${stats.clicked.toLocaleString()} clicks` },
                { label: 'Tasa de Rebote', value: `${stats.bounceRate}%`, icon: <AlertCircle size={20} className="text-q-error" />, bg: 'bg-q-error/10', sub: `${stats.bounced.toLocaleString()} rebotados` },
            ].map((metric, i) => (
                                ...
                                .map(campaign => {
                                    ...
                                    return (
                                        <tr key={`analytics-${campaign.id}`} className="hover:bg-q-bg/30 transition-colors">
                                            ...
                                            <td className="px-3 py-3 text-center">
                                                <span className={`text-sm ${complaints > 0 ? 'text-q-error font-bold' : 'text-q-text-secondary'}`}>
                                                    {complaints}
                                                </span>
                                            </td>
                                        </tr>
        ...
        <div className="grid md:grid-cols-2 gap-6">
            ...
            <div className="bg-q-surface border border-q-border rounded-xl p-6">
                <h3 className="text-lg font-semibold text-q-text mb-4 flex items-center gap-2">
                    <AlertTriangle size={18} className="text-q-accent" />
                ...
                <div className="space-y-3">
                    ...
                    <div className="text-sm text-q-text-secondary space-y-2">
                        ...
                        <p className="flex items-center gap-2">
                            <CheckCircle size={14} className="text-q-accent" />
                        ...
                        <p className="flex items-center gap-2">
                            <CheckCircle size={14} className="text-q-error" />

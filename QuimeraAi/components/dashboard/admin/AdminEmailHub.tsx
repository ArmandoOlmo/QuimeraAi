...
const AdminEmailHub: React.FC<AdminEmailHubProps> = ({ onBack }) => {
    ...
    const renderCampaigns = () => (
        <div className="space-y-6">
            ...
            <div className="flex items-center justify-between">
                <div>
                    <h2 className="text-lg font-bold text-q-text">{t('adminEmail.campaigns.title')}</h2>
                    <p className="text-sm text-q-text-secondary">{t('adminEmail.campaigns.totalCampaigns', { count: campaigns.length })}</p>
                </div>
                <button
            ...
            <div className="bg-q-surface border border-q-border rounded-xl p-4">
                <div className="flex flex-col md:flex-row md:items-center gap-4">
                    <div className="flex-1 flex items-center gap-2 bg-q-bg/50 rounded-lg px-3 py-2">
                        ...
                        <input
                            ...
                            className="flex-1 bg-transparent outline-none text-sm text-q-text placeholder:text-q-text-secondary"
                        />
                        {filters.campaignSearch && (
                            <button onClick={() => setCampaignSearch('')} className="text-q-text-secondary hover:text-q-text">
                                <X size={14} />
                            </button>
                        )}
                    </div>
            ...
            {filteredCampaigns.length > 0 ? (
                        ...
                        <table className="w-full">
                            <thead>
                                <tr className="border-b border-q-border">
                                    ...
                                    <th className="text-left px-4 py-3 text-xs font-medium text-q-text-secondary uppercase tracking-wider">{t('adminEmail.campaigns.audience')}</th>
                                    <th className="text-left px-4 py-3 text-xs font-medium text-q-text-secondary uppercase tracking-wider">{t('adminEmail.campaigns.tenant')}</th>
                                    <th className="text-left px-4 py-3 text-xs font-medium text-q-text-secondary uppercase tracking-wider">{t('adminEmail.campaigns.status')}</th>
                                    <th className="text-right px-4 py-3 text-xs font-medium text-q-text-secondary uppercase tracking-wider">{t('adminEmail.campaigns.sentCount')}</th>
                                    <th className="text-right px-4 py-3 text-xs font-medium text-q-text-secondary uppercase tracking-wider">{t('adminEmail.campaigns.opens')}</th>
                                    <th className="text-right px-4 py-3 text-xs font-medium text-q-text-secondary uppercase tracking-wider">{t('adminEmail.campaigns.clicks')}</th>
                                    <th className="text-right px-4 py-3 text-xs font-medium text-q-text-secondary uppercase tracking-wider">{t('adminEmail.campaigns.actions')}</th>
                                </tr>
                            </thead>
                            <tbody className="divide-y divide-editor-border">
                                {filteredCampaigns.map(campaign => (
                                    <tr
                                        ...
                                        className="hover:bg-q-bg/50 transition-colors group cursor-pointer"
                                        onClick={() => openDetailPanel(campaign)}
                                    >
                                        <td className="px-4 py-3">
                                            <p className="text-sm font-medium text-q-text truncate max-w-[200px]">{campaign.name}</p>
                                            <p className="text-xs text-q-text-secondary truncate max-w-[200px]">{campaign.subject}</p>
                                        </td>
                                        <td className="px-4 py-3">
                                        ...
                                        <td className="px-4 py-3 text-right text-sm text-q-text">
                                        ...
                                        <td className="px-4 py-3">
                                            <div className="flex items-center justify-end gap-1">
                                                ...
                                                >
                                                    <Copy size={15} className="text-q-text-secondary" />
                                                </button>
                                                <button
                ...
                <div className="text-center py-12 bg-q-surface border border-q-border rounded-xl">
                    <Send size={48} className="mx-auto text-q-text-secondary mb-4 opacity-50" />
                    <h3 className="text-lg font-medium text-q-text mb-2">{t('adminEmail.campaigns.noCampaigns')}</h3>
                    <p className="text-q-text-secondary text-sm mb-4">
                    ...
                    <div className="bg-q-bg border border-q-border w-full max-w-lg rounded-2xl shadow-2xl overflow-hidden" onClick={e => e.stopPropagation()}>
                        <div className="p-5 border-b border-q-border flex items-center justify-between bg-gradient-to-r from-purple-500/10 via-blue-500/5 to-transparent">
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    <h2 className="text-lg font-bold text-q-text">{t('adminEmail.campaigns.newCampaignModal.title')}</h2>
                                    <p className="text-xs text-q-text-secondary">{t('adminEmail.campaigns.newCampaignModal.configureDetails')}</p>
                                </div>
                            </div>
                            <button onClick={() => setShowNewCampaignModal(false)} className="p-2 hover:bg-q-surface-overlay/40 rounded-lg transition-colors">
                                <X size={18} className="text-q-text-secondary" />
                            </button>
                        </div>
            ...
            {showEmailEditor && emailDocument && (
                <div className="absolute inset-0 z-40 bg-q-bg">
                ...
                <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-[200] flex items-center justify-center p-4">
                    <div className="bg-q-surface rounded-xl border border-q-border w-full max-w-md shadow-2xl">
                        ...
                        <div className="p-6 space-y-4">
                            <p className="text-q-text-muted text-sm">{t('adminEmail.campaigns.testEmailModal.description')}</p>
                            <div>
                    ...
                    <div className="w-full max-w-lg bg-q-bg border-l border-q-border shadow-2xl overflow-y-auto animate-in slide-in-from-right duration-300" onClick={e => e.stopPropagation()}>
                        <div className="sticky top-0 z-10 bg-q-bg border-b border-q-border p-5">
                            <div className="flex items-center justify-between mb-3">
                                <h3 className="text-lg font-bold text-q-text truncate pr-4">{detailCampaign.name}</h3>
                                <button onClick={() => { setShowDetailPanel(false); setDetailCampaign(null); }} className="p-2 hover:bg-q-surface-overlay/40 rounded-lg transition-colors shrink-0"><X size={18} className="text-q-text-secondary" /></button>
                            </div>
                            <p className="text-sm text-q-text-secondary truncate">{detailCampaign.subject}</p>
                        </div>
                        <div className="p-5 space-y-6">
                            {/* Status */}
                            <div>
                                <label className="text-xs font-bold text-q-text-secondary uppercase tracking-wider mb-2 block">{t('adminEmail.campaigns.detailPanel.status')}</label>
                                <div className="flex items-center gap-3">
                            ...
                            {/* Details */}
                            <div>
                                <label className="text-xs font-bold text-q-text-secondary uppercase tracking-wider mb-2 block">{t('adminEmail.campaigns.detailPanel.details')}</label>
                                <div className="bg-q-surface border border-q-border rounded-xl divide-y divide-editor-border">
                                    <div className="flex justify-between items-center px-4 py-2.5 text-sm"><span className="text-q-text-secondary">{t('adminEmail.campaigns.detailPanel.type')}</span><span className="text-q-text capitalize">{detailCampaign.type || 'newsletter'}</span></div>
                                    <div className="flex justify-between items-center px-4 py-2.5 text-sm"><span className="text-q-text-secondary">{t('adminEmail.campaigns.tenant')}</span><span className="text-q-text flex items-center gap-1"><Building2 size={12} /> {detailCampaign.tenantName}</span></div>
                                    {detailCampaign.previewText && (<div className="flex justify-between items-center px-4 py-2.5 text-sm"><span className="text-q-text-secondary">{t('adminEmail.campaigns.detailPanel.preview')}</span><span className="text-q-text truncate max-w-[200px]">{detailCampaign.previewText}</span></div>)}
                                    <div className="flex justify-between items-center px-4 py-2.5 text-sm"><span className="text-q-text-secondary">{t('adminEmail.campaigns.detailPanel.created')}</span><span className="text-q-text">{formatDate(detailCampaign.createdAt)}</span></div>
                                    {detailCampaign.stats && detailCampaign.stats.sent > 0 && (
                                        <>
                                            <div className="flex justify-between items-center px-4 py-2.5 text-sm"><span className="text-q-text-secondary">{t('adminEmail.campaigns.detailPanel.sentCount')}</span><span className="text-q-text font-medium">{detailCampaign.stats.sent.toLocaleString()}</span></div>
                                            <div className="flex justify-between items-center px-4 py-2.5 text-sm"><span className="text-q-text-secondary">{t('adminEmail.campaigns.detailPanel.openRate')}</span><span className="text-q-text font-medium">{detailCampaign.stats.sent > 0 ? `${((detailCampaign.stats.uniqueOpens || 0) / detailCampaign.stats.sent * 100).toFixed(1)}%` : '—'}</span></div>
                                            <div className="flex justify-between items-center px-4 py-2.5 text-sm"><span className="text-q-text-secondary">{t('adminEmail.campaigns.detailPanel.clickRate')}</span><span className="text-q-text font-medium">{(detailCampaign.stats.uniqueOpens || 0) > 0 ? `${((detailCampaign.stats.uniqueClicks || 0) / detailCampaign.stats.uniqueOpens * 100).toFixed(1)}%` : '—'}</span></div>
                                        </>
                                    )}
                                </div>
                            </div>
    ...
    const renderAudiences = () => (
        <div className="space-y-6">
            ...
            {/* Search */}
            <div className="bg-q-surface border border-q-border rounded-xl p-4">
                <div className="flex items-center gap-2 bg-q-bg/50 rounded-lg px-3 py-2">
                    <Search size={16} className="text-q-text-secondary flex-shrink-0" />
                    <input type="text" placeholder={t('adminEmail.audiences.searchAudiences')} value={filters.audienceSearch} onChange={e => setAudienceSearch(e.target.value)} className="flex-1 bg-transparent outline-none text-sm text-q-text placeholder:text-q-text-secondary" />
                </div>
            </div>
            {/* Audience Cards */}
            {adminAudiences.length > 0 ? (
                <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
                    {adminAudiences.map(aud => (
                        <div key={aud.id} className="bg-q-surface border border-q-border rounded-xl p-5 hover:border-q-accent/30 transition-all">
                            ...
                            <div className="flex gap-2">
                                <button onClick={() => { setSelectedAudienceId(aud.id); setManualEmail(''); setShowImportCSV(false); setShowAddUsers(false); setAudienceMembers(prev => ({ ...prev, [aud.id]: (aud as any)?.members || [] })); }} className="flex-1 text-xs px-3 py-2 bg-q-bg border border-q-border rounded-lg hover:border-q-accent/30 transition-colors text-q-text">
                                    {t('adminEmail.audiences.manage')}
                                </button>
                                <button onClick={() => { setSelectedAudienceId(aud.id); setShowImportCSV(true); }} className="text-xs px-3 py-2 bg-q-bg border border-q-border rounded-lg hover:border-q-accent/30 transition-colors text-q-text flex items-center gap-1">
                                    <Upload size={12} /> CSV
                                </button>
                                <button onClick={() => { setSelectedAudienceId(aud.id); setShowAddUsers(true); setAddUserSearch(''); setAddUserSelectedIds([]); }} className="text-xs px-3 py-2 bg-q-bg border border-q-border rounded-lg hover:border-q-accent/30 transition-colors text-q-text flex items-center gap-1">
                                    <UserPlus size={12} />
                                </button>
                            </div>
                            ...
                            {selectedAudienceId === aud.id && !showImportCSV && !showAddUsers && (
                                <div className="mt-4 border-t border-q-border pt-4">
                                    <div className="flex items-center gap-2 mb-3">
                                        <input type="email" value={manualEmail} onChange={e => setManualEmail(e.target.value)} onKeyDown={e => { if (e.key === 'Enter') handleAddManualEmail(aud.id); }} placeholder={t('adminEmail.audiences.addManualEmail')} className="flex-1 bg-q-bg border border-q-border rounded-lg px-3 py-1.5 text-xs text-q-text focus:outline-none focus:ring-1 focus:ring-q-accent" />
                                        <button onClick={() => handleAddManualEmail(aud.id)} className="px-3 py-1.5 text-xs bg-q-accent/20 text-q-accent rounded-lg hover:bg-q-accent/30">{t('adminEmail.audiences.add')}</button>
                                    </div>
                            ...
                            {selectedAudienceId === aud.id && showImportCSV && (
                                <div className="mt-4 border-t border-q-border pt-4">
                                    <label className="block text-xs font-medium text-q-text-secondary mb-2">{t('adminEmail.audiences.importCSV')}</label>
                                    <input type="file" accept=".csv" onChange={e => { if (e.target.files?.[0]) handleCSVUpload(aud.id, e.target.files[0]); }} disabled={csvUploading} className="text-xs text-q-text" />
                                    {csvUploading && <p className="text-xs text-q-accent mt-1">{t('adminEmail.audiences.importing')}</p>}
                                    <button onClick={() => setShowImportCSV(false)} className="mt-2 text-xs text-q-text-secondary hover:text-q-text">{t('adminEmail.audiences.close')}</button>
                                </div>
                            )}
                            {/* Add registered users */}
                            {selectedAudienceId === aud.id && showAddUsers && (
                                <div className="mt-4 border-t border-q-border pt-4">
                                    <label className="block text-xs font-medium text-q-text-secondary mb-2">{t('adminEmail.audiences.addRegisteredUsers')}</label>
                                    <input type="text" value={addUserSearch} onChange={e => setAddUserSearch(e.target.value)} placeholder={t('adminEmail.audiences.searchByEmailOrName')} className="w-full bg-q-bg border border-q-border rounded-lg px-3 py-1.5 text-xs text-q-text mb-2 focus:outline-none focus:ring-1 focus:ring-q-accent" />
                                    <div className="space-y-1 max-h-32 overflow-y-auto mb-2">
                                        {allUsers.filter(u => addUserSearch === '' || u.email?.toLowerCase().includes(addUserSearch.toLowerCase()) || u.displayName?.toLowerCase().includes(addUserSearch.toLowerCase())).slice(0, 20).map(u => (
                                            <label key={u.id} className="flex items-center gap-2 px-2 py-1 bg-q-bg/50 rounded text-xs cursor-pointer hover:bg-q-bg">
                                                ...
                                                <span className="text-q-text truncate">{u.email}</span>
                                                {u.displayName && <span className="text-q-text-secondary">({u.displayName})</span>}
                                            </label>
                                        ))}
                                    </div>
                                    <div className="flex gap-2">
                                        <button onClick={() => { handleAddRegisteredUsers(aud.id, addUserSelectedIds); }} disabled={addUserSelectedIds.length === 0} className="px-3 py-1.5 text-xs bg-q-accent/20 text-q-accent rounded-lg hover:bg-q-accent/30 disabled:opacity-50">{t('adminEmail.audiences.addSelected', { count: addUserSelectedIds.length })}</button>
                                        <button onClick={() => setShowAddUsers(false)} className="text-xs text-q-text-secondary hover:text-q-text">{t('adminEmail.audiences.close')}</button>
                                    </div>
                                </div>
                            )}
                        </div>
    ...
    const renderAIStudio = () => (
        <div className="bg-q-surface border border-q-border rounded-2xl shadow-xl flex flex-col" style={{ height: 'calc(100vh - 170px)' }}>
            ...
            <div className="p-4 border-b border-q-border flex items-center justify-between bg-gradient-to-r from-purple-500/10 via-blue-500/5 to-transparent rounded-t-2xl">
                ...
                <button onClick={() => { stopVoiceSession(); initAIStudio(); }} className="h-8 w-8 flex items-center justify-center rounded-lg text-q-text-secondary hover:text-q-text hover:bg-q-surface-overlay/40 transition-colors" title={t('adminEmail.aiStudio.resetConversation')}><RefreshCcw className="w-4 h-4" /></button>
            </div>
            ...
            <div className="flex-1 flex overflow-hidden">
                ...
                <div className="w-72 border-l border-q-border bg-q-surface/30 p-4 overflow-y-auto hidden lg:flex flex-col gap-4 custom-scrollbar">
                    ...
                    <div className="mt-auto">
                        <h4 className="text-xs font-bold text-q-text-secondary uppercase tracking-wider mb-2">{t('adminEmail.aiStudio.suggestions')}</h4>
        ...
        <div className="flex h-screen bg-q-bg text-q-text">
            ...
            <div className="flex-1 flex flex-col overflow-hidden relative">
                <header className="h-14 bg-q-bg border-b border-q-border flex-shrink-0 flex items-center justify-between px-4 sm:px-6 sticky top-0 z-10">
                    <div className="flex items-center gap-3">
                        <Mail className="text-q-accent w-5 h-5" />
                ...
                {showEmailEditor && emailDocument ? (
                    ...
                    <>
                        {/* Tab Navigation */}
                        <div className="bg-q-bg border-b border-q-border px-4 sm:px-6 overflow-x-auto flex-shrink-0">
                            <div className="flex items-center gap-1 min-w-max">
                                {tabs.map(tab => (
                                    <button key={tab.id} onClick={() => setActiveTab(tab.id)} className={`flex items-center gap-2 px-4 py-3 text-sm font-medium border-b-2 transition-colors whitespace-nowrap ${activeTab === tab.id ? 'border-q-accent text-q-accent' : 'border-transparent text-q-text-secondary hover:text-q-text'}`}>
                                        {tab.icon}
                                        {tab.label}
                                        {tab.count !== undefined && tab.count > 0 && (<span className={`px-1.5 py-0.5 text-[10px] rounded-full font-bold ${activeTab === tab.id ? 'bg-q-accent/20 text-q-accent' : 'bg-q-surface-overlay/50 text-q-text-secondary'}`}>{tab.count}</span>)}
                                    </button>
                                ))}
                            </div>
                        </div>
                            ...
                            {isLoading ? (
                                <div className="flex items-center justify-center py-24">
                                    <div className="flex flex-col items-center gap-3">
                                        ...
                                        <span className="text-sm text-q-text-secondary">{t('adminEmail.loadingData')}</span>
                                    </div>
                                </div>
                            ) : (
                                <>

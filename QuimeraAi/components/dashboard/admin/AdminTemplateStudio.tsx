            ...
            <BriefSection title={`${t('aiTemplateStudio.briefPanel.components')} (${brief.suggestedComponents.length})`} icon={<LayoutTemplate size={13} />}>
                <div className="flex flex-wrap gap-1">
                    {brief.suggestedComponents.map(comp => (
                        <button
                            ...
                            className="group/chip flex items-center gap-1 px-2 py-0.5 rounded-md bg-primary/15 text-editor-accent text-[10px] border border-primary/20 hover:bg-red-500/15 hover:border-red-500/30 hover:text-red-400 transition-colors cursor-pointer"
                            title={`Remove ${comp}`}
                        >
                            {comp}
                ...
                <div className="relative mt-1.5">
                    <button
                        ...
                        className="flex items-center gap-1 px-2 py-1 rounded-md border border-dashed border-editor-border hover:border-editor-accent/50 text-editor-text-secondary hover:text-editor-accent text-[10px] transition-colors cursor-pointer"
                    >
                        <Plus size={10} />
                        {t('aiTemplateStudio.briefPanel.addComponent', { defaultValue: 'Add component' })}
                    </button>
                    {showComponentPicker && availableToAdd.length > 0 && (
                        <div className="absolute left-0 top-full mt-1 z-50 bg-editor-panel-bg border border-editor-border rounded-lg shadow-xl max-h-48 overflow-y-auto w-48 custom-scrollbar">
                            {availableToAdd.map(comp => (
                                <button
                                    key={comp.key}
                                    onClick={() => { onToggleComponent(comp.key); setShowComponentPicker(false); }}
                                    className="w-full text-left px-3 py-1.5 text-[10px] text-editor-text-secondary hover:bg-editor-accent/10 hover:text-editor-accent transition-colors cursor-pointer"
                                >
                                    {comp.label}
                                </button>
                            ))}

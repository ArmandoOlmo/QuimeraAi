                        ...
                        {showKitManager ? (
                            ...
                            <div className="bg-q-surface/80 border border-q-border rounded-2xl overflow-hidden">
                                ...
                                <div className="p-4 border-b border-q-border flex flex-wrap items-center gap-3">
                                    ...
                                    <select value={uploadCategory} onChange={(e) => setUploadCategory(e.target.value as MediaCategory)} className="h-8 px-2 py-1 bg-q-surface border border-q-border rounded text-xs font-medium focus:outline-none focus:border-q-accent">{MEDIA_CATEGORIES.map(cat => (<option key={cat.id} value={cat.id}>{folderLabels[cat.id] || cat.label}</option>))}</select>

...
const UnifiedMediaLibrary: React.FC<UnifiedMediaLibraryProps> = ({ onBack }) => {
 ...
 <header className="quimera-dashboard-header-bar h-14 px-4 sm:px-6 flex items-center justify-between sticky top-0 z-40">
                            ...
                            <div className="overflow-hidden rounded-2xl border border-q-border bg-q-surface/80">
                                ...
                                {isSelectionMode && selectedIds.size > 0 && (
                                    <div className="mx-4 mt-4 flex shrink-0 animate-fade-in-up items-center justify-between rounded-lg border border-primary/30 bg-primary/10 p-3">
                                        ...
                                        <div className="flex gap-2"><button onClick={selectAll} className="px-3 py-1.5 text-xs font-bold bg-primary/20 hover:bg-primary/30 rounded-lg transition-colors">{t('superadmin.unifiedMedia.selectAll', 'Seleccionar todo')}</button><button onClick={() => setShowBulkDeleteModal(true)} className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-bold bg-q-error text-white hover:bg-q-error rounded-lg transition-colors"><Trash2 size={14} /> {t('superadmin.unifiedMedia.delete', 'Eliminar')}</button></div>
                                    </div>
                                )}
                                                ...
                                                return (
                                                    <div key={asset.id} className={`rounded-xl transition-all duration-200 group relative overflow-hidden h-full ${isSelected ? 'ring-2 ring-q-accent' : ''} cursor-pointer`} onClick={() => { if (isSelectionMode) toggleSelect(asset.id); }} onDoubleClick={() => { if (!isSelectionMode) setPreviewAsset(asset); }}>
                                                        <div className="aspect-square w-full bg-secondary/30 relative overflow-hidden">
                                                            ...
                                                            <div className="absolute top-2 left-2 z-10"><span className={`inline-flex items-center gap-1 px-2 py-0.5 text-[10px] font-semibold rounded-full border backdrop-blur-sm ${cfg?.color || 'bg-q-surface-overlay/10 text-q-text-muted border-q-border/30'}`}>{cfg && iconComponents[cfg.icon]}</span></div>
                                                            {asset.isAiGenerated && <div className="absolute top-2 right-2 z-10"><span className="inline-flex items-center gap-1 px-2 py-0.5 text-[10px] font-semibold rounded-full bg-q-accent/80 text-q-text-on-accent backdrop-blur-sm"><Sparkles size={10} /></span></div>}
                                                            {isSelectionMode && <div className="absolute top-2 right-2 z-10"><button onClick={(e) => { e.stopPropagation(); toggleSelect(asset.id); }} className="p-1.5 bg-q-surface dark:bg-gray-800 rounded-md shadow-lg">{isSelected ? <CheckSquare size={18} className="text-q-accent" /> : <Square size={18} className="text-q-text-muted" />}</button></div>}
                                                            {!isSelectionMode && (
                                                                <div className="absolute top-2 right-2 flex flex-col gap-1 opacity-0 group-hover:opacity-100 transition-opacity z-10">
                                                                    {!isVideoAsset && (
                                                                        <button onClick={(e) => { e.stopPropagation(); handleUseAsReference(asset.downloadURL || asset.url); }} className="p-1.5 bg-q-accent hover:bg-q-accent text-q-text-on-accent rounded-md shadow-lg transition-transform hover:scale-110" title={t('superadmin.unifiedMedia.useAsRef', 'Usar como referencia')}><Zap size={14} /></button>
                                                                    )}
                                                                    {!isVideoAsset && (
                                                                        <button onClick={(e) => { e.stopPropagation(); handleCreateVideo(asset.downloadURL || asset.url); }} className="p-1.5 bg-q-accent hover:bg-q-accent text-q-text-on-accent rounded-md shadow-lg transition-transform hover:scale-110" title={t('mediaGeneration.createVideo', { defaultValue: 'Create video' })}><Film size={14} /></button>
                                                                    )}
            ...
            {previewAsset && (
                <div className="fixed inset-0 z-[100] bg-q-text/80 backdrop-blur-sm flex items-center justify-center p-4 animate-fade-in" onClick={() => setPreviewAsset(null)}>
                    <div className="bg-q-bg border border-q-border rounded-2xl shadow-2xl max-w-4xl w-full max-h-[90vh] overflow-hidden flex flex-col" onClick={e => e.stopPropagation()}>
                        <div className="p-4 border-b border-q-border flex justify-between items-center">
                            <div className="flex items-center gap-3">
                                ...
                                {previewAsset.isAiGenerated && <span className="inline-flex items-center gap-1 px-2 py-0.5 text-xs font-medium rounded-full bg-q-accent/20 text-q-accent border border-q-accent/30"><Sparkles size={10} /> AI</span>}
                            </div>
                        ...
                        <div className="flex-1 overflow-auto">
                            <div className="flex-1 bg-q-text/90 flex items-center justify-center relative min-h-[300px]">
                            ...
                            <div className="p-4 border-t border-q-border">
                                ...
                                {previewAsset.aiPrompt && <div className="mb-3 bg-q-accent/10 p-3 rounded-lg border border-q-accent/30"><div className="flex items-center mb-2"><Sparkles size={14} className="text-q-accent mr-2" /><span className="text-xs font-bold text-q-accent uppercase">{t('superadmin.unifiedMedia.aiPrompt', 'Prompt IA')}</span></div><p className="text-sm text-foreground">{previewAsset.aiPrompt}</p></div>}
                                {previewAsset.usedIn && previewAsset.usedIn.length > 0 && <div className="mb-3 bg-q-accent/10 p-3 rounded-lg border border-q-accent/30"><div className="flex items-center mb-2"><FolderOpen size={14} className="text-q-accent mr-2" /><span className="text-xs font-bold text-q-accent uppercase">{t('superadmin.unifiedMedia.usedIn', 'Usado en')}</span></div><div className="flex flex-wrap gap-2">{previewAsset.usedIn.map(entry => (<span key={entry} className="px-2 py-1 text-xs bg-q-accent/10 text-q-accent rounded-full">{getArticleName(entry)}</span>))}</div></div>}
                                <div className="flex gap-2 pt-3 border-t border-q-border">
                                    ...
                                    {!previewAsset.isSystemAsset && <button onClick={() => handleDelete(previewAsset)} className="flex items-center gap-2 px-4 py-2 bg-q-error/10 border border-q-error/30 text-q-error rounded-lg text-sm font-medium hover:bg-q-error/20 transition-colors ml-auto"><Trash2 size={16} /> {t('superadmin.unifiedMedia.delete', 'Eliminar')}</button>}
                                </div>
                            </div>
                        </div>
                    </div>

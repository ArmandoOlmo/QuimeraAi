...
const InvoiceManager: React.FC<InvoiceManagerProps> = ({
    ...
    if (view === 'form') return (
        <div className="space-y-5">
            <div className="flex items-center justify-between">
                ...
                <button onClick={() => { resetForm(); setView('list'); }} className="text-q-text-muted hover:text-foreground"><X size={20} /></button>
            </div>
            <div className="rounded-2xl border border-q-border/60 bg-q-surface/80 backdrop-blur-xl p-5 space-y-4">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    <input type="text" value={clientName} onChange={e => setClientName(e.target.value)} placeholder={t('accounting.clientName', 'Client Name')} className="h-10 px-3 rounded-xl border border-q-border/60 bg-secondary/30 text-sm text-foreground placeholder:text-q-text-muted focus:ring-2 focus:ring-primary/40" />
                    <input type="email" value={clientEmail} onChange={e => setClientEmail(e.target.value)} placeholder={t('accounting.clientEmail', 'Client Email')} className="h-10 px-3 rounded-xl border border-q-border/60 bg-secondary/30 text-sm text-foreground placeholder:text-q-text-muted focus:ring-2 focus:ring-primary/40" />
                </div>
            </div>
            <div className="rounded-2xl border border-q-border/60 bg-q-surface/80 backdrop-blur-xl p-5 space-y-4">
                <div className="flex items-center justify-between">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                    <div><label className="text-xs text-q-text-muted font-medium mb-1 block">{t('accounting.issueDate', 'Issue Date')}</label><input type="date" value={issueDate} onChange={e => setIssueDate(e.target.value)} className="h-10 w-full px-3 rounded-xl border border-q-border/60 bg-secondary/30 text-sm text-foreground focus:ring-2 focus:ring-primary/40" /></div>
                    <div><label className="text-xs text-q-text-muted font-medium mb-1 block">{t('accounting.dueDate', 'Due Date')}</label><input type="date" value={dueDate} onChange={e => setDueDate(e.target.value)} className="h-10 w-full px-3 rounded-xl border border-q-border/60 bg-secondary/30 text-sm text-foreground focus:ring-2 focus:ring-primary/40" /></div>
                    <div><label className="text-xs text-q-text-muted font-medium mb-1 block">{t('accounting.paymentTerms', 'Payment Terms')}</label><input type="text" value={paymentTerms} onChange={e => setPaymentTerms(e.target.value)} className="h-10 w-full px-3 rounded-xl border border-q-border/60 bg-secondary/30 text-sm text-foreground focus:ring-2 focus:ring-primary/40" /></div>
                </div>
                <div><label className="text-xs text-q-text-muted font-medium mb-1 block">{t('accounting.reminderNote', 'Payment Reminder')}</label><textarea value={reminderNote} onChange={e => setReminderNote(e.target.value)} rows={2} placeholder={t('accounting.reminderPlaceholder', 'Friendly payment reminder...')} className="w-full px-3 py-2 rounded-xl border border-q-border/60 bg-secondary/30 text-sm text-foreground placeholder:text-q-text-muted focus:ring-2 focus:ring-primary/40 resize-none" /></div>
            </div>
            <div className="rounded-2xl border border-q-border/60 bg-q-surface/80 backdrop-blur-xl p-5 space-y-4">
                ...
                <div className="space-y-3">
                    {items.map((item, idx) => (
                        <div key={item.id} className="grid grid-cols-12 gap-2 items-center">
                            <input type="text" value={item.description} onChange={e => updateItem(idx, 'description', e.target.value)} placeholder={t('accounting.itemDescription', 'Description')} className="col-span-5 h-9 px-3 rounded-lg border border-q-border/60 bg-secondary/30 text-sm text-foreground placeholder:text-q-text-muted focus:ring-2 focus:ring-primary/40" />
                            <input type="number" value={item.quantity || ''} onChange={e => updateItem(idx, 'quantity', parseFloat(e.target.value) || 0)} min={0} className="col-span-2 h-9 px-2 rounded-lg border border-q-border/60 bg-secondary/30 text-sm text-foreground text-center focus:ring-2 focus:ring-primary/40" />
                            <input type="number" value={item.unitPrice || ''} onChange={e => updateItem(idx, 'unitPrice', parseFloat(e.target.value) || 0)} min={0} step="0.01" className="col-span-2 h-9 px-2 rounded-lg border border-q-border/60 bg-secondary/30 text-sm text-foreground text-center focus:ring-2 focus:ring-primary/40" />
                            <div className="col-span-2 text-sm font-semibold text-foreground text-right">${(item.quantity * item.unitPrice).toFixed(2)}</div>
                            <button onClick={() => removeItem(idx)} className="col-span-1 justify-self-center text-q-text-muted hover:text-destructive" disabled={items.length === 1}><X size={14} /></button>
                        </div>
                    ))}
                </div>
                ...
                <div className="border-t border-q-border/40 pt-4 space-y-2 max-w-xs ml-auto">
                    <div className="flex justify-between text-sm"><span className="text-q-text-muted">{t('accounting.subtotal', 'Subtotal')}</span><span className="text-foreground font-medium">${subtotal.toFixed(2)}</span></div>
                    <div className="flex justify-between text-sm"><span className="text-q-text-muted">{t('accounting.tax', 'Tax')}</span><span className="text-foreground font-medium">${taxTotal.toFixed(2)}</span></div>
                    <div className="flex justify-between text-base font-bold border-t border-q-border/40 pt-2"><span>{t('accounting.total', 'Total')}</span><span className="text-primary">${total.toFixed(2)}</span></div>
                </div>
            </div>
    ...
    return (
        <div className="space-y-4">
            ...
            {filtered.length === 0 ? (
                <div className="rounded-2xl border border-dashed border-q-border bg-q-surface/50 p-12 text-center"><FileText size={40} className="mx-auto text-q-text-muted/40 mb-3" /><p className="font-medium text-foreground">{t('accounting.noInvoices', 'No invoices yet')}</p><p className="text-xs text-q-text-muted mt-1">{t('accounting.createFirstInvoice', 'Create your first invoice')}</p></div>
            ) : (
                            ...
                            <div className="flex items-center justify-between gap-4">
                                <div className="flex items-center gap-4 min-w-0">
                                    ...
                                    <div className="min-w-0">
                                        ...
                                        <p className="text-sm text-q-text-muted truncate">{inv.clientName}</p>
                                    </div>
                                </div>

...
const BlogPost: React.FC<BlogPostProps> = ({ post, theme, onBack, textColor, backgroundColor, accentColor, layoutType }) => {
    ...
    if (layoutType === 'profile') {
        return (
            <div className="min-h-screen pb-20 animate-fade-in-up" style={{ backgroundColor, color: textColor }}>
                ...
                {/* Profile Header */}
                <div className="container mx-auto px-6 max-w-3xl text-center py-12">
                    {/* Profile Photo */}
                    <div className="mb-8">
                        {post.featuredImage ? (
                            <div className="relative inline-block">
                                <div className="absolute inset-0 rounded-2xl blur-2xl opacity-30" style={{ backgroundColor: accentColor }} />
                                <img
                                    src={post.featuredImage}
                                    alt={post.title}
                                    className="w-40 h-56 md:w-48 md:h-64 rounded-2xl object-cover mx-auto relative z-10 border-4 shadow-2xl"
                                    style={{ borderColor: `${accentColor}40` }}
                                />
                            </div>
                        ) : (
                            <div
                                className="w-40 h-56 md:w-48 md:h-64 rounded-2xl mx-auto flex items-center justify-center"
                                style={{ backgroundColor: `${accentColor}15`, border: `2px solid ${accentColor}30` }}
                            >
                                <User size={64} style={{ color: accentColor }} className="opacity-60" />
                            </div>
                        )}
                    </div>

                    {/* Name */}
                    <h1 className="text-4xl md:text-5xl font-extrabold mb-3 leading-tight font-header" style={{ color: textColor }}>
                        {post.title}
                    </h1>

                    {/* Role / Subtitle */}
                    {post.excerpt && (
                        <p className="text-lg md:text-xl font-medium mb-6" style={{ color: accentColor }}>
                            {post.excerpt}
                        </p>
                    )}

                    {/* Date & Author */}
                    <div className="flex items-center justify-center gap-4 text-sm mb-8" style={{ color: mutedText }}>
                        {post.showDate !== false && (
                        ...
                        {post.showAuthor !== false && post.author && (
                            <span className="flex items-center gap-1.5 border-l pl-4" style={{ borderColor: cardBorder }}>
                                <User size={14} />
                                {post.author}
                            </span>
                        )}
                    ...
                    {/* Divider */}
                    <div className="w-24 h-1 rounded-full mx-auto mb-10" style={{ backgroundColor: accentColor }} />
                </div>
                ...
                <div className="container mx-auto px-6 max-w-3xl">
                    ...
                    {/* Share Section */}
                    <div className="mt-12 pt-8 border-t flex justify-between items-center relative" style={{ borderColor: `${textColor}15` }}>
                        <p className="text-sm opacity-70">Compartir perfil</p>
                        <div className="relative" ref={shareMenuRef}>
                            <button type="button" onClick={handleShare} className="p-2 rounded-full hover:bg-white/10 transition-colors cursor-pointer" title="Share">
                                <Share2 size={20} />
                            </button>
                        </div>
                    </div>
                    ...
                    <div className="mb-10 p-5 rounded-2xl" style={{ backgroundColor: `${accentColor}12`, border: `1px solid ${accentColor}30` }}>
                        ...
                        {(() => {
                            ...
                            return (
                                ...
                                >
                                    <source src={post.podcastVideoUrl} type="video/mp4" />
                    ...
                    <div className="relative" ref={shareMenuRef}>
                        ...
                        {showShareMenu && (
                            ...
                            >
                                ...
                                <div className="p-2">
                                    {/* Twitter/X */}
                                    <button type="button" onClick={shareToTwitter} className="w-full flex items-center gap-3 px-3 py-2 rounded-lg transition-colors text-left cursor-pointer hover:opacity-80" style={{ color: textColor }}>
                                    ...
                                    {/* Facebook */}
                                    <button type="button" onClick={shareToFacebook} className="w-full flex items-center gap-3 px-3 py-2 rounded-lg transition-colors text-left cursor-pointer hover:opacity-80" style={{ color: textColor }}>
                                    ...
                                    {/* LinkedIn */}
                                    <button type="button" onClick={shareToLinkedIn} className="w-full flex items-center gap-3 px-3 py-2 rounded-lg transition-colors text-left cursor-pointer hover:opacity-80" style={{ color: textColor }}>
                                    ...
                                    {/* WhatsApp */}
                                    <button type="button" onClick={shareToWhatsApp} className="w-full flex items-center gap-3 px-3 py-2 rounded-lg transition-colors text-left cursor-pointer hover:opacity-80" style={{ color: textColor }}>
                                    ...
                                    {/* Copy Link */}
                                    <button type="button" onClick={copyLink} className="w-full flex items-center gap-3 px-3 py-2 rounded-lg transition-colors text-left cursor-pointer hover:opacity-80" style={{ color: linkCopied ? accentColor : textColor }}>
                                        {linkCopied ? (
                                            <><Check size={16} className="flex-shrink-0" style={{ color: accentColor }} /><span className="text-sm">Link copied!</span></>
                                        ) : (
                                            <><Link2 size={16} className="flex-shrink-0" /><span className="text-sm">Copy link</span></>
                                        )}
                                    </button>
                                </div>
                            </div>

...
- Module registry: `registry/moduleRegistry.ts`
...
- Visible media blocks are also publish-gated: `featured_media` needs a safe media URL, and `portfolio_grid` needs at least one safe renderable item.
...
- lead capture
- email subscribe
- featured media and portfolio intent from Media AI/assets
- chatbot CTA
...
The AI Studio adapter now separates featured links from social links. Real social/contact handles become `social` links plus a `social_links` icon block, while placeholder social root URLs are ignored. Media-ready projects get draft `featured_media` and `portfolio_grid` blocks tied to Media AI asset targets; when imported or existing media URLs are available, the adapter writes renderable `url` and `items` payloads immediately. These remain review-required until the merchant confirms them.
...
- share tab can generate a QR code and download it
...
- the block editor exposes funnel blocks for Ecommerce, Appointments, CRM, Email Marketing, Media AI, ChatCore, testimonials, FAQ, contact, and featured banners
...
- portfolio grid blocks can select multiple project media assets and persist them as block items
...
- ChatCore: public page embeds ChatCore when the Bio Page settings include an assistant config and passes safe Bio Page context into the assistant prompt.
- Media AI: featured media uses the shared ImagePicker/generation flow, and portfolio grids render selected project image/video assets from block data.
...
The focused suite currently covers slug/URL safety, slug conflict preflight for create/update/publish, publish review-gating for generated links/blocks/media, custom slug/SEO/QR persistence, social link block defaults/context filtering, AI blueprint mapping with renderable media/portfolio payloads, AI Studio Bio Page draft application, regeneration lock preservation, product guardrails, publish/unpublish public URL gating, public published-only/visible-only rendering, public ChatCore context safety, public lead attribution/dedupe, Email API subscription routing/fallback, analytics source/block summaries, conversion rate, UTM attribution, referrers, and device breakdowns.

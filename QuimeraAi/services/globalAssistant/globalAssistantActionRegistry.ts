...
export const GLOBAL_ASSISTANT_ACTIONS: AssistantActionDefinition[] = [
    ...
    action('agency', 'create_agency_client', 'Create and provision an agency-managed client workspace through onboarding-api with a canonical agency service plan, BusinessBlueprint, module activations, billing state, and activity audit.', objectSchema({
        tenantId: stringSchema('Agency tenant id. Defaults to the active tenant.'),
        businessName: stringSchema('Client business name.'),
        industry: stringSchema('Client industry.'),
        contactEmail: stringSchema('Primary client contact email.'),
        contactPhone: stringSchema('Primary client contact phone.'),
        selectedPlanId: stringSchema('Agency service plan id. Defaults to the active default plan when omitted.'),
        setupBilling: booleanSchema('Whether agency-managed billing should be configured as pending setup.'),
        monthlyPrice: numberSchema('Monthly price charged to the client. Defaults to the selected service-plan price.'),
        enabledFeatures: { type: 'array', items: stringSchema('Feature flag such as cms, leads, ecommerce, chatbot, email, analytics, finance.') },
        initialUsers: { type: 'array', items: objectSchema({
            email: stringSchema('Client user email.'),
            name: stringSchema('Client user name.'),
            role: stringSchema('Client role.', ['client', 'client_admin', 'client_user']),
        }) },
        projectTemplate: stringSchema('Starting project template id.'),
        aiStudioMode: stringSchema('AI Studio provisioning mode.', ['draft', 'generate']),
        logoUrl: stringSchema('Optional already-uploaded client logo URL.'),
        primaryColor: stringSchema('Brand primary color.'),
        secondaryColor: stringSchema('Brand secondary color.'),
        generateWebsite: booleanSchema('Generate Website Builder blueprint.'),
        generateStorefront: booleanSchema('Generate Storefront Builder blueprint.'),
        generateEcommerce: booleanSchema('Generate Ecommerce blueprint.'),
        generateChatbot: booleanSchema('Generate ChatCore blueprint.'),
        generateEmailFlows: booleanSchema('Generate Email Marketing blueprint.'),
        generateAppointments: booleanSchema('Generate Appointments blueprint.'),
        generateRestaurantModule: booleanSchema('Generate Restaurant blueprint.'),
        generateRealtyModule: booleanSchema('Generate Realty blueprint.'),
        generateBioPage: booleanSchema('Generate Bio Page blueprint.'),
        generateMediaAssets: booleanSchema('Generate Media AI blueprint.'),
        metadata: objectSchema(),
    }, ['businessName', 'contactEmail']), 'high', {
        requiredService: 'agency',
        requiredFeature: 'agencyModule',

...
export interface WebsiteEditorSyncInput {
    ...
    pages?: SitePage[];
    touchedSection?: PageSection;
    action: WebsiteEditorSyncAction;
    now?: string;
}

const ECOMMERCE_PAGE_SECTIONS = new Set<PageSection>([
    'featuredProducts',
    'categoryGrid',
    'productHero',
    'saleCountdown',
...
const MANUAL_SECTION_ACTIONS = new Set<WebsiteEditorSyncAction>([
    'component_add',
    'component_remove',
    'component_reorder',
    'section_visibility',
...
const SOURCE_ALIASES: Record<string, WebsiteEcommerceBlockSource> = {
    newest: 'new_arrivals',
    new: 'new_arrivals',
...
const ready = (warnings: string[] = [], blockers: string[] = []) => ({
...
const cloneJson = <T,>(value: T): T => {
    ...
    try {
        ...
        return Array.isArray(value) ? [] as T : {} as T;
    }
};

const getSectionSettings = (data: PageData | null, section: PageSection): Record<string, unknown> => {
    const value = data ? (data as Record<string, unknown>)[section] : undefined;
...
const normalizeEcommerceSource = (
    value: unknown,
    fallback: WebsiteEcommerceBlockSource,
): WebsiteEcommerceBlockSource => {
...
const numberIn = <TNumber extends number>(
    value: unknown,
    allowed: readonly TNumber[],
): TNumber | undefined => (
    typeof value === 'number' && allowed.includes(value as TNumber) ? value as TNumber : undefined
);
...
const buildWebsitePlanFallback = (input: WebsiteEditorSyncInput, now: string): WebsitePlan => {
    const data = input.data || {} as PageData;
    const brandName = input.brandIdentity?.name || input.projectName || firstString(
        (data as Record<string, any>).header?.logoText,
        (data as Record<string, any>).hero?.headline,
    ) || 'Untitled Website';
    ...
    return {
        ...
        businessProfile: {
            ...
            tagline: firstString((data as Record<string, any>).hero?.subheadline),
            services: [],
            contactInfo: {},
            hasEcommerce,
        },
...
const createBaseBlueprint = (input: WebsiteEditorSyncInput, now: string): BusinessBlueprint => {
    if (input.businessBlueprint) return input.businessBlueprint;
...
const getTouchedSections = (input: WebsiteEditorSyncInput): Set<PageSection> => {
    const touched = new Set<PageSection>();
    if (input.touchedSection) touched.add(input.touchedSection);
...
const createSectionState = (
    ...
    pageIds: string[],
    existing: WebsiteSectionBlueprint | undefined,
    input: WebsiteEditorSyncInput,
    shouldMarkTouched: boolean,
): WebsiteSectionBlueprint => {
    const base: Partial<BlueprintModuleState> = {
        enabled: visible,
...
const buildSectionBlueprints = (
    ...
    const existingByType = new Map<PageSection, WebsiteSectionBlueprint>();
    existingSections.forEach(section => {
        if (!existingByType.has(section.type)) existingByType.set(section.type, section);
    });

    const sectionTypes = new Set<PageSection>(input.componentOrder);
    if (input.touchedSection) sectionTypes.add(input.touchedSection);
    existingSections
    ...
    return Array.from(sectionTypes).map(section => {
        ...
        return createSectionState(
            now,
            section,
            index >= 0 ? index : existingSection?.order ?? input.componentOrder.length,
            visible,
            getSectionSettings(input.data, section),
            pageIds,
            existingSection,
            input,
            shouldMarkTouched,
...
const resolveBlockSettings = (
    type: WebsiteEcommerceBlockType,
    fallbackSource: WebsiteEcommerceBlockSource,
    ...
    const sourceType = normalizeEcommerceSource(
        sectionSettings.sourceType || currentSource.type || sectionSettings.source || fallbackSource,
        fallbackSource,
    );
    ...
    return {
        ...
        layout: (firstString(sectionSettings.layout, sectionSettings.variant) as WebsiteEcommerceBlockSettings['layout']) || definition?.defaultLayout,
        productCardVariant: firstString(sectionSettings.productCardVariant, sectionSettings.cardVariant, sectionSettings.productCardStyle) as WebsiteEcommerceBlockSettings['productCardVariant'],
        columns: numberIn(sectionSettings.columns, [2, 3, 4, 5] as const),
...
const buildEcommerceBlocks = (
    existing: WebsiteBlueprint | undefined,
    input: WebsiteEditorSyncInput,
    now: string,
): WebsiteEcommerceBlockBlueprint[] => {
    const existingBlocks = Array.isArray(existing?.ecommerceBlocks) ? existing!.ecommerceBlocks : [];
    const existingByType = new Map<WebsiteEcommerceBlockType, WebsiteEcommerceBlockBlueprint>();
    existingBlocks.forEach(block => {
        if (!existingByType.has(block.type)) existingByType.set(block.type, block);
    });

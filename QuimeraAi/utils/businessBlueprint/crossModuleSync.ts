...
function createEmailDrafts(input: CrossModuleSyncInput): CrossModuleSyncDraft[] {
    ...
    const drafts = [
        ...
        createEmailFlowDraft(input, 'refund', 'Refund confirmation draft', 'order_refunded', 'Refund update for {{orderNumber}}', 'Confirm refund details only from stored refund records.', ['orderNumber', 'customerName', 'refundAmount']),
        createEmailFlowDraft(input, 'shipping_confirmation', 'Shipping confirmation draft', 'order_fulfilled', 'Your order has been fulfilled', 'Confirm fulfillment only from stored fulfillment and tracking records.', ['orderNumber', 'customerName', 'trackingUrl']),

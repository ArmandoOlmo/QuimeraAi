...
import type {
    ...
    StorefrontThemeResolutionInput,
    StorefrontThemeSelectionInput,
} from '../../types/storefrontTheme';

export const STOREFRONT_CATALOG_SIZE_RULES: StorefrontCatalogSizeRule[] = [
    {
        size: 'empty',
        minProducts: 0,
        maxProducts: 0,
...
export const STOREFRONT_THEME_PRESETS: Record<StorefrontThemePresetId, StorefrontThemePresetDefinition> = {
    minimal: {
        id: 'minimal',
        label: 'Minimal',
...
const PRESET_PRIORITY: StorefrontThemePresetId[] = [
    'fitness',
    'food',
    'restaurant',
    'realEstate',
...
function normalizeIndustry(value?: string): string {
    ...
    if (industry.includes('restaurant') || industry.includes('cafe') || industry.includes('bakery') || industry.includes('food')) return 'restaurant';
    if (industry.includes('real') || industry.includes('property') || industry.includes('realt') || industry.includes('inmobili')) return 'real-estate';
    if (industry.includes('fitness') || industry.includes('gym') || industry.includes('wellness') || industry.includes('sport')) return 'fitness';
    if (industry.includes('fashion') || industry.includes('apparel') || industry.includes('clothing')) return 'fashion';
    if (industry.includes('beauty') || industry.includes('spa') || industry.includes('cosmetic')) return 'beauty';
    if (industry.includes('digital') || industry.includes('software') || industry.includes('course') || industry.includes('saas')) return 'digital';
    if (industry.includes('luxury') || industry.includes('jewelry')) return 'luxury';
    if (industry.includes('shop') || industry.includes('store') || industry.includes('ecommerce') || industry.includes('retail') || industry.includes('marketplace')) return 'ecommerce';
    return industry || 'services';
}

function isIndustryCompatible(preset: StorefrontThemePresetDefinition, industry?: string): boolean {
...
function hasRequiredModules(preset: StorefrontThemePresetDefinition, modules: string[]): boolean {
    return preset.compatibility.requiredModules.every(module => modules.includes(module));
}

function hasUnsupportedModules(preset: StorefrontThemePresetDefinition, modules: string[]): boolean {
    return preset.compatibility.unsupportedModules.some(module => modules.includes(module));
}

function definedTheme(values: Partial<StorefrontThemeSettings>): Partial<StorefrontThemeSettings> {
    return Object.entries(values).reduce((acc, [key, value]) => {
        if (value !== undefined && value !== null && value !== '') {
            acc[key as keyof StorefrontThemeSettings] = value as never;
        }
        return acc;
    }, {} as Partial<StorefrontThemeSettings>);
}

function themeFromGlobalColors(colors?: StorefrontThemeResolutionInput['projectGlobalColors']): Partial<StorefrontThemeSettings> {
    if (!colors) return {};
...
export function getStorefrontCatalogSize(productCount = 0): StorefrontCatalogSize {
    const count = Math.max(0, Math.floor(productCount));
    const rule = STOREFRONT_CATALOG_SIZE_RULES.find(item => (
        count >= item.minProducts && (item.maxProducts === undefined || count <= item.maxProducts)
    ));
    return rule?.size || 'enterprise';
}

export function getStorefrontCatalogSizeRule(size: StorefrontCatalogSize): StorefrontCatalogSizeRule {
    return STOREFRONT_CATALOG_SIZE_RULES.find(rule => rule.size === size) || STOREFRONT_CATALOG_SIZE_RULES[0];
}

export function getCompatibleStorefrontThemePresets(input: StorefrontThemeSelectionInput = {}): StorefrontThemePresetDefinition[] {
...
export function selectStorefrontThemePreset(input: StorefrontThemeSelectionInput = {}): StorefrontThemePresetDefinition {
    const catalogSize = input.catalogSize || getStorefrontCatalogSize(input.productCount);
    const modules = input.enabledModules || ['ecommerce-engine'];
    ...
    if (catalogSize === 'large' || catalogSize === 'enterprise') return STOREFRONT_THEME_PRESETS.marketplace;
    if (normalizeIndustry(input.industry) === 'restaurant') return STOREFRONT_THEME_PRESETS.food;
    if (normalizeIndustry(input.industry) === 'real-estate') return STOREFRONT_THEME_PRESETS.realEstate;
    return STOREFRONT_THEME_PRESETS.minimal;
}

export function buildStorefrontThemeFallbackChain(input: StorefrontThemeSelectionInput = {}): string[] {
...
export function resolveStorefrontTheme(input: StorefrontThemeResolutionInput = {}): StorefrontThemeResolution {
    const catalogSize = input.catalogSize || getStorefrontCatalogSize(input.productCount);
...
export function resolveStorefrontThemeSettings(input: StorefrontThemeResolutionInput = {}): StorefrontThemeSettings {

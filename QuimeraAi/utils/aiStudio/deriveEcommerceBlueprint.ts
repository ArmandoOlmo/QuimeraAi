import type { StarterProductBlueprint } from '../../types/businessBlueprint';
import type {
    AiStudioBusinessBriefInput,
    AiStudioCommerceIndustry,
    AiStudioEcommerceBlueprint,
    AiStudioStarterProduct,
} from './types';
import {
...
function includesSignal(signals: AiStudioCommerceIndustry[], signal: AiStudioCommerceIndustry): boolean {
    return signals.includes(signal);
}

function extractUserProvidedPrice(text: string): number | undefined {
...
function serviceCategories(input: AiStudioBusinessBriefInput): string[] {
    const services = [
        ...(input.services || []),
...
function inferProductCategories(input: AiStudioBusinessBriefInput, signals: AiStudioCommerceIndustry[]): string[] {
    ...
    if (includesSignal(signals, 'restaurant')) {
        ...
        if (/\b(meal kit|meal kits|kit|take home|para llevar)\b/i.test(text)) categories.push('Meal Kits');
        return categories;
    }
    ...
    if (includesSignal(signals, 'fashion')) return ['Apparel', 'Accessories', 'New Arrivals', 'Gift Cards'];
    if (includesSignal(signals, 'beauty')) return ['Skincare', 'Beauty Packages', 'Gift Cards', 'Add-ons'];
    if (includesSignal(signals, 'digital')) return ['Digital Guides', 'Courses', 'Templates', 'Downloads'];
    if (includesSignal(signals, 'fitness')) return ['Training Gear', 'Programs', 'Recovery', 'Gift Cards'];
    if (includesSignal(signals, 'luxury')) return ['Signature Pieces', 'Accessories', 'Gift Cards', 'Private Consultations'];

    const fromServices = serviceCategories(input);
    if (fromServices.length > 0) return uniqueValues([...fromServices, 'Gift Cards'], 6);
...
function inferStoreType(signals: AiStudioCommerceIndustry[], categories: string[]): string {
    if (includesSignal(signals, 'restaurant')) return 'restaurant_commerce_drafts';
    if (includesSignal(signals, 'real-estate')) return 'real_estate_services_drafts';
    if (includesSignal(signals, 'digital')) return 'digital_products_drafts';
    if (includesSignal(signals, 'services') && !includesSignal(signals, 'commerce')) return 'service_packages_drafts';
    if (includesSignal(signals, 'marketplace') || categories.length > 6) return 'multi_category_catalog_drafts';
    if (includesSignal(signals, 'commerce')) return 'product_catalog_drafts';
    return 'not_applicable';
}

function inferCatalogStrategy(enabled: boolean, signals: AiStudioCommerceIndustry[]): string {
    if (!enabled) return 'no_catalog_optional_recommendations';
    if (includesSignal(signals, 'real-estate')) return 'optional_services_and_digital_resources';
    if (includesSignal(signals, 'services') && !includesSignal(signals, 'commerce')) return 'paid_services_and_packages';
    if (includesSignal(signals, 'restaurant')) return 'gift_cards_catering_events_and_merch';
    return 'draft_starter_catalog_for_review';
}

function createStarterProduct(
    category: string,
    ...
    const product: StarterProductBlueprint = {
        name: `${category} Draft`,
        category,
...
function recommendationsFor(signals: AiStudioCommerceIndustry[], enabled: boolean): string[] {
    if (!enabled) {
    ...
    if (includesSignal(signals, 'restaurant')) recommendations.push('Validate gift cards, catering packages, and event tickets with the restaurant team.');
    if (includesSignal(signals, 'real-estate')) recommendations.push('Use digital guides and consultation packages as optional lead-generation offers.');
    if (includesSignal(signals, 'services')) recommendations.push('Confirm paid consultation, deposit, and appointment rules before enabling checkout.');
    if (includesSignal(signals, 'digital')) recommendations.push('Confirm file delivery and access rules before selling digital products.');

    return recommendations;
}

export function deriveEcommerceBlueprintFromBusinessBrief(
    input: AiStudioBusinessBriefInput,
): AiStudioEcommerceBlueprint {
    ...
    const giftCardsEnabled = enabled && (
        hasGiftCardCategory ||
        (
    ...
    const digitalProductsEnabled = enabled && (
        includesSignal(signals, 'digital') ||
        includesSignal(signals, 'real-estate') ||

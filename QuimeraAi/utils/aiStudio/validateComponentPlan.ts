...
export interface ValidateComponentPlanInput {
...
const addIssue = (
    ...
    issue: ComponentPlanValidationIssue,
) => {
...
function hasRequiredData(componentId: string, context: ComponentSelectionContext): boolean {
...
function checkProtectedManualSection(
    issues: ComponentPlanValidationIssue[],
    componentId: string,
    input: ValidateComponentPlanInput,
) {
    ...
    const protectedSection = input.existingBusinessBlueprint?.websiteBlueprint.sectionBlueprints
        ?.find(section => section.type === targetSection && (
            section.metadata.userModified ||
...
export function validateComponentPlan(input: ValidateComponentPlanInput): ComponentPlanValidationResult {
    const issues: ComponentPlanValidationIssue[] = [];

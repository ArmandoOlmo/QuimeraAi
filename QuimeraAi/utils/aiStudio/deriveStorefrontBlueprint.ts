...
function normalizeBrandColors(brandProfile?: unknown): Record<string, string> {
    const profile = brandProfile && typeof brandProfile === 'object'
        ? brandProfile as { colors?: unknown }
        : undefined;
    const colors = (profile?.colors && typeof profile.colors === 'object')
        ? profile.colors as Record<string, unknown>
        : (brandProfile && typeof brandProfile === 'object' ? brandProfile as Record<string, unknown> : {});

    return Object.entries(colors)
        .filter((entry): entry is [string, string] => typeof entry[1] === 'string' && entry[1].trim().startsWith('#'))
...
function choosePreset(input: AiStudioBusinessBriefInput, ecommerceBlueprint: AiStudioEcommerceBlueprint): StorefrontThemePresetId {
    const text = getBriefText(input);
    const signals = classifyCommerceSignals(input);

    if (signals.includes('marketplace') || ecommerceBlueprint.productCategories.length > 8) return 'marketplace';
    if (signals.includes('fitness') || /\b(sports?|gym|fitness|crossfit|cycling|bike|bicycle|bicicleta)\b/i.test(text)) return 'fitness';
    if (signals.includes('luxury')) return 'luxury';
    if (signals.includes('fashion')) return 'fashion';
    if (signals.includes('restaurant') || signals.includes('food')) return 'food';
    if (signals.includes('beauty')) return 'beauty';
    if (signals.includes('real-estate')) return 'realEstate';
    if (signals.includes('digital')) return 'digital';
    if (signals.includes('services')) return 'services';
    return ecommerceBlueprint.enabled ? 'minimal' : 'minimal';
}

function chooseProductCardVariant(preset: StorefrontThemePresetId): ProductCardVariant {
    switch (preset) {
        case 'fitness':
            return 'quickBuy';
        case 'luxury':
...
export function deriveStorefrontBlueprintFromBusinessBrief(
    ...
    const sections = STOREFRONT_SECTION_TYPES.map((type, order) => (
        createSection(type, order, enabled, now, productCardVariant)
    ));

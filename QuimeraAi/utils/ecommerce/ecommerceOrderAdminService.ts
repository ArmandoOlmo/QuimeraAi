...
export const getOrderTimeline = (input: EcommerceAdminOrderInput): EcommerceOrderTimelineEvent[] => {
    ...
    const createdAt = readString(readField(input, 'created_at', 'createdAt'), nowIso());
    const paidAt = readOptionalString(readField(input, 'paid_at', 'paidAt'));
    const shippedAt = readOptionalString(readField(input, 'shipped_at', 'shippedAt'));
    const cancelledAt = readOptionalString(readField(input, 'cancelled_at', 'cancelledAt'));

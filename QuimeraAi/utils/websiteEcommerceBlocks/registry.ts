...
export const WEBSITE_ECOMMERCE_BLOCK_TYPES: WebsiteEcommerceBlockType[] = [
    'featuredProducts',
    'productCarousel',
    'categoryShowcase',
    'promoBanner',
...
const baseDefinition = (
    definition: Omit<
        WebsiteEcommerceBlockDefinition,
        'canonicalSystem' | 'presentationOwner' | 'requiredModule' | 'requiredFeature' | 'requiredService' | 'writes'
    >,
): WebsiteEcommerceBlockDefinition => ({
    ...definition,
...
export const websiteEcommerceBlockRegistry: Record<WebsiteEcommerceBlockType, WebsiteEcommerceBlockDefinition> = {
...
export const isWebsiteEcommerceBlockType = (value: unknown): value is WebsiteEcommerceBlockType => (
    typeof value === 'string' && WEBSITE_ECOMMERCE_BLOCK_TYPES.includes(value as WebsiteEcommerceBlockType)
);

export const getWebsiteEcommerceBlockDefinition = (
    type: WebsiteEcommerceBlockType | string,
): WebsiteEcommerceBlockDefinition | undefined => (
    isWebsiteEcommerceBlockType(type) ? websiteEcommerceBlockRegistry[type] : undefined
);

const getSourceConfig = (
    block: WebsiteEcommerceBlockValidationInput,
): { type?: string; productIds?: string[]; categoryId?: string; collectionId?: string } => {
    const settings = block.settings as WebsiteEcommerceBlockSettings | undefined;
    return {
...
const pushIssue = (
    issues: WebsiteEcommerceBlockValidationIssue[],
    issue: WebsiteEcommerceBlockValidationIssue,
) => {
...
export const validateWebsiteEcommerceBlock = (
    block: WebsiteEcommerceBlockValidationInput,
): WebsiteEcommerceBlockValidationIssue[] => {
    const issues: WebsiteEcommerceBlockValidationIssue[] = [];
    ...
    if (!definition.allowedSources.includes(sourceType as WebsiteEcommerceBlockSource)) {
        pushIssue(issues, {
...
const sectionSeedMap: Partial<Record<PageSection, Omit<WebsiteEcommerceBlockSeed, 'id'>>> = {
    featuredProducts: {
        type: 'featuredProducts',
        source: 'featured',
        targetRoute: '/store',
...
export const createWebsiteEcommerceBlockSeedsFromSections = (
    sections: PageSection[],
): WebsiteEcommerceBlockSeed[] => sections
    .map<WebsiteEcommerceBlockSeed | undefined>((section, index) => {
        const seed = sectionSeedMap[section];
        if (!seed) return undefined;
        const settings: WebsiteEcommerceBlockSettings = {
            ...seed.settings,
    ...
    .filter((seed): seed is WebsiteEcommerceBlockSeed => Boolean(seed));

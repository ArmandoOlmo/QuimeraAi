...
describe('chatbot customer request notes', () => {
    it('builds a bilingual detailed note from customer intent and transcript context', () => {
        ...
        expect(notes).toContain('- Cliente: Ana Cliente, ana@example.com, +1 787 555 0101.');
        expect(notes).toContain('- Lo que desea: Quiere reservar una consulta para ver paquetes premium.');
        expect(notes).toContain('- Customer: Ana Cliente, ana@example.com, +1 787 555 0101.');
        expect(notes).toContain('- Request: Quiere reservar una consulta para ver paquetes premium.');
        ...
        expect(notes).toContain('  - Cliente: Necesito una cita y quiero saber el precio del paquete premium.');
    ...
    it('converts technical generated summaries into readable bilingual follow-up notes', () => {
        ...
        expect(readable).toContain('Español / Spanish');
        expect(readable).toContain('English / Inglés');
        expect(readable).toContain('- Cliente: Luis Rivera, luis@example.com.');
        expect(readable).toContain('- Lo que desea: Quiere comparar paquetes premium antes de reservar.');
        expect(readable).toContain('- Customer: Luis Rivera, luis@example.com.');
        expect(readable).toContain('- Request: Quiere comparar paquetes premium antes de reservar.');
    ...
    it('builds detailed notes from voice appointment transcripts', () => {
        ...
        expect(notes).toContain('- Conversation context:');
        expect(notes).toContain('  - Customer: Soy Carlos Rivera');

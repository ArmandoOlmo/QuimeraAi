...
export async function sendPaidOrderTransactionalEmails({
  ...
  results.push(await sendOrderConfirmation({
    ...
    canonical: {
      supabase,
      sourceComponent: "stripe-webhook",
      sourceEvent: "checkout.paid",
      sourceEntityType: "order",
      sourceEntityId: String(order.id || readPaymentIntentId(order) || ""),
    },
    context,
    eventId,
    now: paidAt,
  ...
  for (const product of lowStockProducts) {
    results.push(await sendLowStockAlert({
      ...
      canonical: {
        supabase,
        sourceComponent: "stripe-webhook",
        sourceEvent: "inventory.low_stock",
        sourceEntityType: "product",
        sourceEntityId: String(product.id || product.product_id || ""),
      },
      context: {
        ...context,
        products: [product],

    ...
    const saveGeneratedImageToLibrary = async (
        ...
        try {
            if (destination === 'admin') {
                ...
                const usedIn = options?.projectId ? [`template:${options.projectId}`] : [];
                ...
                const { error: mediaErr } = await supabase.from('media_assets').insert([mediaAssetRecord]);
                if (mediaErr) throw mediaErr;

                const { error: legacyErr } = await supabase.from('admin_assets').insert([{
                    id: assetId,
                    name: fileName,
                    url: publicUrl,
                    size,
                ...
                if (legacyErr) {
                    console.warn('[AIContext] media_assets saved but legacy admin_assets sync failed:', legacyErr);
                }

                notifyLibraryUpdated('admin', options?.projectId);
                return;
            }

...
type ToolHandler = (args: Record<string, any>, auth: McpAuthContext) => Promise<any>;
...
async function readJson(req: IncomingMessage & { body?: any }): Promise<JsonRpcRequest | JsonRpcRequest[]> {
...
function fail(id: JsonRpcRequest['id'], error: any) {
...
function commonGenerationInput(args: Record<string, any>, auth: McpAuthContext, sourceTool: string): AiGatewayInput {
  return {
...
async function getProjectForTenant(projectId: string, tenantId: string) {
...
function setDeepValue(target: Record<string, any>, path: string, value: any): void {
...
function mergeSectionData(base: any, section: string, content: any, replace = false): any {
  const next = { ...(base || {}) };
...
async function updateProjectData(projectId: string, updates: Record<string, any>) {
  const supabase = getSupabaseAdmin();
  const { data, error } = await supabase
    .from('projects')
...
function pick(input: Record<string, any>, allowed: string[]): Record<string, any> {
...
function normalizeTemplateArgs(args: Record<string, any>): Record<string, any> {
...
function summarizeTemplateContent(template: Record<string, any>) {
  ...
  const data =
    rawData.data && typeof rawData.data === 'object' && !Array.isArray(rawData.data) ? rawData.data : rawData;
...
function assertTemplateHasContent(template: Record<string, any>): void {
  const summary = summarizeTemplateContent(template);
  if (!summary.hasRenderableContent) {
    throw Object.assign(
      new Error('Template content is required. Provide PageData in data with componentOrder, use sourceProjectId, or provide pages with sectionData.'),
      { status: 400, details: summary },
    );
  }
}
...
function extractPageDataFromTemplateInput(
  pageDataInput: Record<string, any> | undefined,
  ...
  if (sourceData?.data && typeof sourceData.data === 'object') return sourceData.data;
  if (sourceData?.hero || sourceData?.header) return sourceData;
  return pageDataInput || {};
}

/** Read/write PageData inside projects.data (Admin snapshot uses nested `data.data`). */
function getProjectDataWriteContext(project: Record<string, any>) {
  ...
  const usesNestedPageData = Boolean(
    snapshot.data &&
  ...
  return {
    pageData,
    updateSection(section: string, updater: (sectionState: Record<string, any>) => Record<string, any>) {
      pageData[section] = updater({ ...(pageData[section] || {}) });
    },
    buildDataColumn(): Record<string, any> {
      const now = new Date().toISOString();
      ...
      for (const [key, value] of Object.entries(snapshot)) {
        if (metaKeys.has(key)) preserved[key] = value;
      }
...
const toolHandlers: Record<string, ToolHandler> = {
  ...
  get_template: async (args, auth) => {
    ...
    if (!args.templateId) throw Object.assign(new Error('templateId is required.'), { status: 400 });
  ...
  create_template: async (args, auth) => {
    ...
    if (!normalizedArgs.name) throw Object.assign(new Error('name is required.'), { status: 400 });

    const sourceProject: Record<string, any> = normalizedArgs.sourceProjectId
    ...
    const brandIdentity =
      normalizedArgs.brandIdentity || normalizedArgs.brand_identity || sourceProject.brand_identity || {};
    const componentOrder =
      normalizedArgs.componentOrder || normalizedArgs.component_order || sourceProject.component_order || [];
    const sectionVisibility =
      normalizedArgs.sectionVisibility ||
  ...
  update_template: async (args, auth) => {
    ...
    const tenantId = assertTenantAccess(auth, args.tenantId);
    const normalizedArgs = normalizeTemplateArgs(args);
    if (!normalizedArgs.templateId) throw Object.assign(new Error('templateId is required.'), { status: 400 });
    ...
    const brandIdentity =
      normalizedArgs.brandIdentity ?? normalizedArgs.brand_identity ?? existing.brand_identity ?? {};
    const componentOrder =
      normalizedArgs.componentOrder ?? normalizedArgs.component_order ?? existing.component_order ?? [];
    const sectionVisibility =
      normalizedArgs.sectionVisibility ??
      normalizedArgs.section_visibility ??
    ...
    const updates: Record<string, any> = {
      ...
      data: buildAdminTemplateDataColumn({
        ...
        thumbnailUrl:
          normalizedArgs.thumbnailUrl ?? normalizedArgs.thumbnail_url ?? existing.thumbnail_url ?? '',
  ...
  create_project: async (args, auth) => {
    ...
    const tenantId = assertTenantAccess(auth, args.tenantId);
    if (!args.name) throw Object.assign(new Error('name is required.'), { status: 400 });
  ...
  get_project: async (args, auth) => {
    requireScope(auth, 'projects:read');
    const tenantId = assertTenantAccess(auth, args.tenantId);
    if (!args.projectId) throw Object.assign(new Error('projectId is required.'), { status: 400 });
  ...
  update_project_page: async (args, auth) => {
    ...
    if (!page || typeof page !== 'object') throw Object.assign(new Error('page or updates is required.'), { status: 400 });

    const pageId = args.pageId || page.id || randomUUID();
    const nextPage = { ...page, id: pageId, updatedAt: new Date().toISOString() };
    const index = pages.findIndex((item: any) => item.id === pageId || (page.slug && item.slug === page.slug));
    if (index >= 0) pages[index] = args.replace ? nextPage : { ...pages[index], ...nextPage };
    else pages.push({ showInNavigation: true, isHomePage: pages.length === 0, ...nextPage });
  ...
  update_project_sections: async (args, auth) => {
    ...
    if (!sections || typeof sections !== 'object') throw Object.assign(new Error('sections is required.'), { status: 400 });

    if (args.pageId) {
      const pages = Array.isArray(project.pages) ? [...project.pages] : [];
      const pageIndex = pages.findIndex((page: any) => page.id === args.pageId);
      if (pageIndex === -1) throw Object.assign(new Error('Page not found.'), { status: 404 });
      pages[pageIndex] = {
        ...pages[pageIndex],
    ...
    if (args.replace) {
      ...
      for (const [section, content] of Object.entries(sections)) {
        writeContext.updateSection(section, (sectionState) =>
  ...
  validate_project: async (args, auth) => {
    ...
    const pages = Array.isArray(project.pages) ? project.pages : [];
    const warnings = [];
    if (pages.length === 0) warnings.push('Project has no pages.');
    if (!pages.some((page: any) => page.isHomePage || page.slug === '/')) warnings.push('Project has no home page.');
    if (!project.seo_config && !pages.some((page: any) => page.seo)) warnings.push('Project has no SEO metadata.');
    if (!project.ai_assistant_config) warnings.push('Project has no chatbot configuration.');
  ...
  update_lead: async (args, auth) => {
    requireScope(auth, 'leads:write');
    const tenantId = assertTenantAccess(auth, args.tenantId);
    if (!args.leadId) throw Object.assign(new Error('leadId is required.'), { status: 400 });
  ...
  create_lead_task: async (args, auth) => {
    requireScope(auth, 'leads:write');
    const tenantId = assertTenantAccess(auth, args.tenantId);
    if (!args.leadId || !args.title) throw Object.assign(new Error('leadId and title are required.'), { status: 400 });
    const lead = await getLeadForTenant(args.leadId, tenantId);
    const { data, error } = await getSupabaseAdmin()
      .from('lead_tasks')
      .insert({
  ...
  create_article: async (args, auth) => {
    ...
    assertTenantAccess(auth, args.tenantId);
    if (!args.title || !args.content) throw Object.assign(new Error('title and content are required.'), { status: 400 });
  ...
  update_article: async (args, auth) => {
    requireScope(auth, 'cms:write');
    assertTenantAccess(auth, args.tenantId);
    if (!args.articleId && !args.slug) throw Object.assign(new Error('articleId or slug is required.'), { status: 400 });
    const updates = {
  ...
  update_navigation: async (args, auth) => {
    requireScope(auth, 'cms:write');
    assertTenantAccess(auth, args.tenantId);
    if (!Array.isArray(args.links)) throw Object.assign(new Error('links array is required.'), { status: 400 });
    const { data, error } = await getSupabaseAdmin()
      .from('app_navigation')
  ...
  update_seo_metadata: async (args, auth) => {
    requireScope(auth, 'cms:write');
    const tenantId = assertTenantAccess(auth, args.tenantId);
    if (!args.projectId || !args.seo) throw Object.assign(new Error('projectId and seo are required.'), { status: 400 });
    const project = await getProjectForTenant(args.projectId, tenantId);
    if (args.pageId) {
      const pages = Array.isArray(project.pages) ? [...project.pages] : [];
      const pageIndex = pages.findIndex((page: any) => page.id === args.pageId);
  ...
  create_product: async (args, auth) => {
    ...
    const tenantId = assertTenantAccess(auth, args.tenantId);
    if (!args.projectId || !args.name) throw Object.assign(new Error('projectId and name are required.'), { status: 400 });
    await getProjectForTenant(args.projectId, tenantId);
    const { data, error } = await getSupabaseAdmin()
      .from('store_products')
      .insert({
  ...
  update_product: async (args, auth) => {
    requireScope(auth, 'commerce:write');
    const tenantId = assertTenantAccess(auth, args.tenantId);
    if (!args.projectId || !args.productId) throw Object.assign(new Error('projectId and productId are required.'), { status: 400 });
    await getProjectForTenant(args.projectId, tenantId);
    const updates = {
  ...
  update_order_status: async (args, auth) => {
    requireScope(auth, 'commerce:write');
    const tenantId = assertTenantAccess(auth, args.tenantId);
    if (!args.projectId || !args.orderId) throw Object.assign(new Error('projectId and orderId are required.'), { status: 400 });
    await getProjectForTenant(args.projectId, tenantId);
    const updates = {
  ...
  update_appointment: async (args, auth) => {
    requireScope(auth, 'appointments:write');
    const tenantId = assertTenantAccess(auth, args.tenantId);
    if (!args.appointmentId) throw Object.assign(new Error('appointmentId is required.'), { status: 400 });
    const updates = {
  ...
  block_appointment_date: async (args, auth) => {
    requireScope(auth, 'appointments:write');
    const tenantId = assertTenantAccess(auth, args.tenantId);
    if (!args.projectId || !args.date) throw Object.assign(new Error('projectId and date are required.'), { status: 400 });
    await getProjectForTenant(args.projectId, tenantId);
    const { data, error } = await getSupabaseAdmin()
      .from('blocked_dates')
  ...
  get_domain_status: async (args, auth) => {
    requireScope(auth, 'domains:read');
    const tenantId = assertTenantAccess(auth, args.tenantId);
    if (!args.domain && !args.domainId) throw Object.assign(new Error('domain or domainId is required.'), { status: 400 });
    ...
    if (error) throw error;
    if (!data) throw Object.assign(new Error('Domain not found.'), { status: 404 });
    if (data.project_id) await getProjectForTenant(data.project_id, tenantId);
    return { domain: data };
  },
  ...
  get_project_summary: async (args, auth) => {
    ...
    for (const result of [leads, products, orders, appointments]) if (result.error) throw result.error;
    return {
  ...
  ai_generate_content: async (args, auth) => {
    ...
    if (!input.brief) throw Object.assign(new Error('brief is required.'), { status: 400 });
  ...
  ai_generate_image_prompt: async (args, auth) => {
    ...
    if (!input.brief) throw Object.assign(new Error('brief or prompt is required.'), { status: 400 });
  ...
  ai_generate_project_assets: async (args, auth) => {
    ...
    if (requests.length === 0) throw Object.assign(new Error('assets array is required.'), { status: 400 });
    if (requests.length > 12) throw Object.assign(new Error('Maximum 12 assets per batch.'), { status: 400 });
  ...
  ai_apply_generated_images: async (args, auth) => {
    ...
    for (const replacement of replacements) {
      ...
      if (pageId) {
        ...
        if (pageIndex === -1) throw Object.assign(new Error(`Page not found: ${pageId}`), { status: 404 });
      ...
      if (replacement.assetId) updatedAssetIds.push(replacement.assetId);
    }
  ...
  get_generation_job: async (args, auth) => {
    const tenantId = assertTenantAccess(auth, args.tenantId);
    if (!args.jobId) throw Object.assign(new Error('jobId is required.'), { status: 400 });
    ...
    if (error) throw error;
    if (!data) throw Object.assign(new Error('Generation job not found.'), { status: 404 });
    return data;
  },
};
...
async function handleRpc(message: JsonRpcRequest, auth: McpAuthContext | null) {
  ...
  if (message.method === 'tools/call') {
    ...
    if (!handler) throw Object.assign(new Error(`Unknown tool: ${name}`), { status: 400 });
...
export default async function handler(req: IncomingMessage & { body?: any }, res: ServerResponse) {
  ...
  const hasError = responses.some((response: any) => response.error);

...
type RateBucket = 'read' | 'write' | 'ai' | 'batch';
...
function bucketForTool(toolName: string): RateBucket {
  if (toolName === 'ai_generate_full_project' || toolName === 'ai_generate_project_assets') return 'batch';
  if (toolName.startsWith('ai_')) return 'ai';
  if (
...
export async function enforceMcpRateLimit(
  ...
  auth: McpAuthContext,
  toolName: string,
): Promise<{ bucket: RateBucket; limit: number; remaining: number }> {

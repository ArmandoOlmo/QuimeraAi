...
function parseBearer(req: { headers: Record<string, string | string[] | undefined> }): string | null {
...
function normalizeScopes(row: Record<string, any>): string[] {
  const raw = row.scopes || row.permissions || row.metadata?.scopes || row.metadata?.permissions;
  if (Array.isArray(raw)) return raw.filter((scope) => typeof scope === 'string');
  if (typeof raw === 'string') {
    try {
      const parsed = JSON.parse(raw);
      if (Array.isArray(parsed)) return parsed.filter((scope) => typeof scope === 'string');
    } catch {
...
export function requireScope(auth: McpAuthContext, required: string): void {
...
export function requireAnyScope(auth: McpAuthContext, requiredScopes: string[]): void {
  if (requiredScopes.some((scope) => hasScope(auth.scopes, scope))) return;
...
async function resolveTenantFromProject(
...
export async function authenticateMcpRequest(
  ...
  const matchingRow = (rows || []).find((row: Record<string, any>) => {
    if (typeof row.key_hash !== 'string') return false;
...
export function assertTenantAccess(auth: McpAuthContext, tenantId?: string): string {

...
export default async function handler(req: IncomingMessage, res: ServerResponse) {
  ...
  try {
    ...
    const result = await service.applySnapshot({
      agencyTenantId,
    ...
    sendJson(res, result.status === 'failed' ? 409 : 200, result);
  } catch (error) {
    handleAgencyApiError(res, error, 'Failed to apply Agency snapshot.');
  }
}

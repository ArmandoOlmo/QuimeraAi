import { getProviderRetryAfterSeconds, isProviderRateLimitError, resolveEmailProvider, } from './emailProviderService.js';
...
import { assertEmailReadiness, getEmailSettings, getSenderHeader } from './emailSettingsService.js';
...
export async function processEmailOutbox(input) {
    ...
    for (const row of rows) {
        ...
        try {
            ...
            if (!provider)
                throw new Error(`Email provider secret is not configured server-side for ${providerName}`);
...
async function loadDueOutboxRows(supabase, input) {
    let query = supabase
        .from('email_outbox')
        .select('*')
        .eq('status', 'queued')
    ...
    if (error)
        throw error;
    return (data || []).filter((row) => Number(row.attempts || 0) < Number(row.max_attempts || 3));
}
async function claimOutboxRow(supabase, row, now) {
...
async function markOutbox(supabase, outboxId, updates) {
...
async function deferOutbox(supabase, row, now, seconds, message) {
...
async function getCachedSettings(supabase, cache, projectId) {
...
async function countRecentProviderSends(supabase, input) {
...
async function updateCampaignDeliveryStats(supabase, campaignId, patch) {
...
function resultRow(row, status, reason, providerMessageId) {
    return {
        outboxId: row.id,
...
function buildProviderHeaders(row) {
    const headers = {};
    if (row.email_log_id)
        headers['X-Quimera-Email-Log-Id'] = String(row.email_log_id);
    if (row.idempotency_key)
        headers['X-Quimera-Idempotency-Key'] = String(row.idempotency_key);
    if (row.project_id)
        headers['X-Quimera-Project-Id'] = String(row.project_id);
    return headers;
}
function buildProviderCustomArgs(row) {
    const values = {
        projectId: row.project_id,
        emailLogId: row.email_log_id,
        campaignId: row.campaign_id,
...
function resolveRateLimit(settings, override) {

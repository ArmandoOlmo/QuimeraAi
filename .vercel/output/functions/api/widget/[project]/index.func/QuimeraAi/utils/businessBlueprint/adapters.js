import { BUSINESS_BLUEPRINT_SCHEMA_VERSION, BUSINESS_BLUEPRINT_VERSION, } from '../../types/businessBlueprint.js';
import { getStorefrontCatalogSize, getStorefrontCatalogSizeRule, } from '../storefrontTheme/index.js';
...
import { createRestaurantBlueprintFromWebsitePlan, normalizeRestaurantBlueprint, } from './restaurantBlueprint.js';
const ECOMMERCE_SECTIONS = new Set([
    'announcementBar',
    'productHero',
    'featuredProducts',
    'categoryGrid',
...
function pickBrandColor(colors, keys) {
    return keys.map(key => cleanOptionalString(colors?.[key], 80)).find(Boolean);
}
function buildChatbotAppearanceSettings(input) {
...
function metadata(now, generatedBy = 'ai') {
    return {
        generatedBy,
        userModified: false,
        generatedAt: now,
...
export function createBlueprintModuleState(now, overrides = {}) {
    const { readiness: readinessOverride, metadata: metadataOverride, ...stateOverrides } = overrides;
    return {
        enabled: true,
        status: 'generated',
        needsReview: false,
...
function normalizeIndustry(value) {
    ...
    if (/\b(restaurant|restaurante|caf[eé]|cafeteria|food|comida|steakhouse|bakery|panader[ií]a|catering|bar|sushi|pizza|brunch|fine dining|casual dining|food truck|menu|reservas?)\b/i.test(industry))
        return 'restaurant';
    if (industry.includes('real') || industry.includes('property') || industry.includes('inmobili') || industry.includes('bienes raices') || industry.includes('bienes raíces'))
        return 'real-estate';
    if (industry.includes('fitness') || industry.includes('gym') || industry.includes('wellness'))
        return 'fitness';
    if (industry.includes('fashion'))
        return 'fashion';
    if (industry.includes('beauty') || industry.includes('spa'))
        return 'beauty';
    if (industry.includes('digital') || industry.includes('software'))
        return 'digital';
    if (industry.includes('shop') || industry.includes('store') || industry.includes('ecommerce') || industry.includes('retail'))
        return 'ecommerce';
    return industry || 'services';
}
function buildAppointmentSignalText(plan) {
...
function hasAppointmentSignal(plan, sections) {
...
function toBlueprintId(prefix, value, index) {
    const slug = value
        .toLowerCase()
        .normalize('NFD')
        .replace(/[\u0300-\u036f]/g, '')
...
function normalizeSocialProfileUrl(platform, value) {
    ...
    if (asUrl && !new RegExp(`^https?:\\/\\/(www\\.)?${platform}\\.com\\/?$`, 'i').test(asUrl))
        return asUrl;
    const handle = raw
        .replace(/^@/, '')
        .replace(/^https?:\/\/(www\.)?[^/]+\//i, '')
        .replace(/\/$/, '')
        .trim();
    if (!handle || ['profile', 'username', 'handle'].includes(handle.toLowerCase()))
        return '';
    const profileBase = {
...
function createBioPageSocialLinks(contactInfo = {}) {
    const socialSources = [
    ...
    const socialLinks = socialSources
        .map((source, index) => {
...
function titleFromMediaPath(value, fallback) {
    const cleaned = value
        .split(/[/.]/)
        .filter(Boolean)
        .slice(-2)
...
function createBioPageMediaItems(plan) {
...
function readContentString(item, keys, maxLength = 240) {
...
function readContentRating(item) {
    if (!item || typeof item !== 'object' || Array.isArray(item))
        return undefined;
...
function createBioPageTestimonialItems(plan) {
...
function createBioPageFaqItems(plan) {
...
function createBioPageContactBlockPayload(contactInfo = {}) {
...
function createStorefrontBlueprint(plan, now) {
...
function createEcommerceBlueprint(plan, now) {
    const hasEcommerce = Boolean(plan.businessProfile.hasEcommerce);
...
function chatbotKnowledgeSource(input) {
    return {
        id: input.id,
        name: input.name,
        type: input.type,
        ...
        readiness: ready(input.warnings || [bi('La fuente debe revisarse antes de que ChatCore la use como conocimiento de producción.', 'Source must be reviewed before ChatCore can use it as production knowledge.')], input.blockers || []),
...
function chatbotAction(actionType, ownerModule, options = {}) {
    return {
        ...
        status: 'needs_review',
        readiness: ready(options.warnings || [bi('La acción permanece desactivada hasta revisarse en el Action Registry del Chatbot Engine.', 'Action is disabled until reviewed in the Chatbot Engine Action Registry.')], options.blockers || []),
        needsReview: true,
    };
}
function chatbotSurface(sourceSurface, enabled, contextKeys, routePattern) {
    return {
        enabled,
        ...
        contextKeys,
        readiness: enabled
            ? ready([bi('La superficie queda preparada para revisión; el despliegue a producción sigue siendo explícito.', 'Surface is prepared for review; production deployment remains explicit.')])
            : ready([], [bi('El despliegue de esta superficie está desactivado hasta configurarse.', 'Surface deployment is disabled until configured.')]),
        needsReview: true,
    };
}
function createChatbotBlueprintFromInput(input) {
    ...
    const serviceNames = input.services.map(service => service.name).filter((name) => Boolean(name));
    ...
    return {
        ...createBlueprintModuleState(input.now, {
            ...
            needsReview: true,
            readiness: ready([
                bi('ChatbotBlueprint V2 se generó como infraestructura en borrador.', 'ChatbotBlueprint V2 was generated as draft infrastructure.'),
                bi('Las fuentes de conocimiento, acciones públicas, superficies de despliegue y ajustes de voz requieren revisión antes de activarse.', 'Knowledge sources, public actions, deployment surfaces, and voice settings require review before activation.'),
            ], [
        ...
        agentProfile: {
            ...
            personality: bi('útil, conciso, cuidadoso y consciente del contexto comercial', 'helpful, concise, careful, and commercially aware'),
            tone: bi('profesional', 'professional'),
            languageMode: 'visitor_language',
            supportedLanguages: ['es', 'en'],
            brandVoice: input.tagline || input.description || bi('Usa la voz de marca del proyecto y evita afirmaciones no verificadas.', 'Use the project brand voice and avoid unsupported claims.'),
            welcomeMessage: bi(`Hola. Puedo ayudarte con ${input.businessName || 'este negocio'} y dirigir solicitudes al equipo correcto cuando esté configurado.`, `Hi. I can help with ${input.businessName || 'this business'} and route requests to the right team when configured.`),
            fallbackMessage: bi('No tengo suficiente información revisada para responder eso con seguridad. Puedo recopilar tus datos o pasar esto a un humano.', 'I do not have enough reviewed information to answer that safely. I can collect your details or hand this to a human.'),
            escalationMessage: bi('Puedo conectar esta conversación con un humano para revisión.', 'I can connect this conversation with a human for review.'),
        ...
        knowledgeSources: [
            ...
            ...(hasEcommerce ? [
                chatbotKnowledgeSource({
                    ...
                    warnings: [bi('Usar solo productos publicados del Ecommerce Engine; los productos iniciales de AI siguen como borradores hasta revisión del comercio.', 'Use published Ecommerce Engine products only; AI starter products remain drafts until merchant review.')],
                }),
                chatbotKnowledgeSource({
                    ...
                    ownerModule: 'ecommerce',
                    warnings: [bi('No inventar detalles de políticas de envío, devoluciones, pagos, impuestos, descuentos o inventario.', 'Do not invent shipping, return, payment, tax, discount, or inventory policy details.')],
                }),
                chatbotKnowledgeSource({
                    ...
                    visibility: 'private',
                    status: 'disabled',
                    blockers: [bi('La consulta de órdenes requiere verificación de identidad del cliente e integración canónica con Ecommerce Engine.', 'Order lookup requires customer identity verification and canonical Ecommerce Engine integration.')],
                }),
            ] : []),
            ...(wantsBooking ? [
                chatbotKnowledgeSource({
                    ...
                    warnings: [bi('Disponibilidad, buffers, horarios bloqueados, zona horaria y modo de confirmación deben revisarse.', 'Availability, buffers, blocked times, timezone, and confirmation mode must be reviewed.')],
                }),
            ] : []),
            ...(hasRestaurant ? [
                chatbotKnowledgeSource({
                    ...
                    warnings: [bi('Usar solo datos activos de restaurante/menu; no inventar platos, precios, disponibilidad ni garantías de alérgenos.', 'Use active restaurant/menu data only; do not invent menu items, prices, availability, or allergen guarantees.')],
                }),
            ] : []),
            ...(hasRealEstate ? [
                chatbotKnowledgeSource({
                    ...
                    warnings: [bi('Usar solo listados públicos activos y evitar asesoría legal, financiera o de mercado no sustentada.', 'Use public active listings only and avoid legal, financial, or unsupported market advice.')],
                }),
            ] : []),
            ...(hasFinance ? [
                chatbotKnowledgeSource({
                    ...
                    visibility: 'private',
                    status: 'disabled',
                    blockers: [bi('Las facturas y solicitudes de pago requieren revisión humana; el chatbot solo puede crear borradores internos.', 'Invoices and payment requests require human review; the chatbot can only create internal drafts.')],
                    warnings: [bi('No crear enlaces de pago, cargos de Stripe, asientos contables ni reglas fiscales desde ChatCore.', 'Do not create payment links, Stripe charges, ledger entries, or tax rules from ChatCore.')],
                }),
            ] : []),
            chatbotKnowledgeSource({
                ...
                warnings: [bi('Solo bloques visibles y publicados de Bio Page pueden exponerse públicamente.', 'Only published visible Bio Page blocks can be exposed publicly.')],
            }),
            chatbotKnowledgeSource({
                ...
                visibility: 'private',
                status: 'disabled',
                blockers: [bi('Los datos de leads son privados y solo pueden usarse para alimentar CRM, notas internas y handoff humano.', 'Lead data is private and can only be used to feed CRM, internal notes, and human handoff.')],
            }),
            chatbotKnowledgeSource({
                ...
                visibility: 'internal',
                warnings: [bi('El chatbot puede crear borradores y suscripciones con consentimiento; no debe enviar campañas automaticamente.', 'The chatbot can create consented subscriptions and drafts; it must not send campaigns automatically.')],
            }),
        ],
        actions: [
            ...
            chatbotAction('create_appointment', 'appointments', { publicAllowed: true, requiresConsent: true, blockers: [bi('La disponibilidad debe validarse del lado servidor antes de reservar.', 'Availability must be checked server-side before booking.')] }),
            ...
            chatbotAction('check_order_status', 'ecommerce', { publicAllowed: true, requiresConfirmation: false, idempotencyRequired: false, warnings: [bi('El runtime solo devuelve estado de orden cuando el cliente valida email o token de acceso.', 'Runtime returns order status only after the customer verifies email or access token.')] }),
            ...
            chatbotAction('back_in_stock_request', 'ecommerce', { publicAllowed: true, requiresConfirmation: false, requiresConsent: true, warnings: [bi('El runtime solo registra el interes del comprador; no promete inventario ni activa emails sin revision.', 'Runtime only records buyer interest; it does not promise inventory or activate emails without review.')] }),
            chatbotAction('start_checkout', 'ecommerce', { publicAllowed: true, warnings: [bi('El runtime prepara el carrito y redirige al checkout real; no crea pagos desde ChatCore.', 'Runtime prepares the cart and redirects to real checkout; it does not create payments from ChatCore.')] }),
            chatbotAction('subscribe_email_audience', 'email-marketing', { publicAllowed: true, requiresConsent: true, warnings: [bi('El runtime valida consentimiento de marketing y supresiones antes de suscribir.', 'Runtime validates marketing consent and suppressions before subscribing.')] }),
            chatbotAction('queue_email_follow_up', 'email-marketing', { publicAllowed: true, requiresConsent: true, warnings: [bi('El runtime crea un borrador en revision; no envia emails sin aprobacion humana.', 'Runtime creates a review draft; it does not send emails without human approval.')] }),
            chatbotAction('create_finance_quote_request', 'finance', { publicAllowed: true, requiresConsent: true, warnings: [bi('El runtime crea una factura/cotizacion borrador en Finance; no crea enlaces de pago ni cargos de Stripe.', 'Runtime creates a draft invoice/quote in Finance; it does not create payment links or Stripe charges.')] }),
            chatbotAction('request_media_asset', 'media-ai', { publicAllowed: true, requiresConsent: true, warnings: [bi('El runtime crea un asset borrador en Media AI para revision; no genera, adjunta ni publica automaticamente.', 'Runtime creates a draft asset in Media AI for review; it does not generate, attach, or publish automatically.')] }),
        ...
        handoff: {
            ...
            fallbackMessage: bi('Un humano puede revisar esta conversación y dar seguimiento.', 'A human can review this conversation and follow up.'),
        ...
        testing: (() => {
            const testScenarios = [
                {
                    ...
                    prompt: bi(`Qué ofrece ${input.businessName || 'este negocio'}?`, `What does ${input.businessName || 'this business'} offer?`),
                ...
                ...(hasEcommerce ? [{
                        ...
                        expectedActions: ['search_products', 'recommend_products'],
                        expectedSources: ['ecommerce_products'],
                        status: 'draft',
                        needsReview: true,
                    }, {
                        ...
                        expectedActions: ['start_checkout', 'create_product_inquiry'],
                        expectedSources: ['ecommerce_products', 'ecommerce_policies'],
                        status: 'draft',
                        needsReview: true,
                    }] : []),
                ...(wantsBooking ? [{
                        ...
                        expectedActions: ['check_availability', 'create_appointment'],
                        expectedSources: ['appointments_services', 'business_blueprint'],
                        status: 'draft',
                        needsReview: true,
                    }] : []),
                ...(hasRestaurant ? [{
                        ...
                        expectedActions: ['request_restaurant_reservation'],
                        expectedSources: ['restaurant_menu'],
                        status: 'draft',
                        needsReview: true,
                    }] : []),
                ...(hasRealEstate ? [{
                        ...
                        expectedActions: ['request_realty_showing', 'register_open_house'],
                        expectedSources: ['realty_listings'],
                        status: 'draft',
                        needsReview: true,
                    }] : []),
                ...(hasFinance ? [{
                        ...
                        expectedActions: ['create_finance_quote_request'],
                        expectedSources: ['finance_invoices_private', 'business_blueprint'],
                        status: 'draft',
                        needsReview: true,
                    }] : []),
                ...(hasMedia ? [{
                        ...
                        expectedActions: ['request_media_asset'],
                        expectedSources: ['business_blueprint', 'website_content'],
                        status: 'draft',
                        needsReview: true,
                    }] : []),
            ...
            return {
                ...
                blockedAnswerRules: [
                    bi('No inventar precios, políticas, inventario, asesoría legal/financiera, estado de órdenes, platos de menú, reservas, disponibilidad de propiedades ni testimonios.', 'Do not invent prices, policies, inventory, legal advice, financial advice, order status, menu items, reservations, listing availability, or testimonials.'),
                    bi('No ejecutar acciones públicas a menos que Action Registry las marque como habilitadas y listas.', 'Do not execute public actions unless the Action Registry marks them enabled and ready.'),
                ],
                expectedActions: uniqueStrings(testScenarios.flatMap(scenario => scenario.expectedActions)),
                evaluationStatus: 'not_run',
                readiness: ready([bi('Los escenarios de prueba son borradores hasta ejecutarse en Test Lab.', 'Test scenarios are drafts until run in Test Lab.')]),
            };
        })(),
        analytics: {
            events: [
        ...
        deployment: {
            ...
            readiness: ready([], [
                ...
                bi('El agent ID del proveedor de voz debe configurarse por proyecto antes de habilitar voz.', 'Voice provider agent ID must be configured per project before voice is enabled.'),
            ]),
        },
        businessKnowledge,
...
function createChatbotBlueprint(plan, now) {
    ...
    return createChatbotBlueprintFromInput({
        ...
        brandColors: plan.brandProfile.colors,
...
export function normalizeChatbotBlueprint(value, input) {
...
function createLeadBlueprint(plan, now) {
    return {
        ...createBlueprintModuleState(now),
...
function createEmailMarketingBlueprint(plan, now) {
    ...
    const transactionalFlows = [
        ...
        ...(hasRealEstate ? [{
                ...
                triggerEvent: 'realty_property_inquiry',
    ...
    return {
        ...
        flows: [
            ...
            ...(plan.businessProfile.hasEcommerce
                ? [
                    { type: 'abandoned_cart', status: 'draft', triggerEvent: 'cart_abandoned' },
                    { type: 'order_confirmation', status: 'draft', triggerEvent: 'order_created' },
                    { type: 'post_purchase', status: 'draft', triggerEvent: 'purchase_completed' },
                ]
                : []),
        ],
...
function createMediaBlueprint(plan, now) {
    return {
        ...createBlueprintModuleState(now),
...
function createBioPageBlueprint(plan, now) {
    const sections = getPlannedSections(plan);
    const industry = normalizeIndustry(plan.businessProfile.industry);
    ...
    const links = [...baseLinks, ...socialLinks];
    ...
    const normalizedLinks = links.map((link, index) => ({
        ...
        openInNewTab: link.openInNewTab ?? (link.linkType === 'external' || link.linkType === 'social'),
...
function createAppointmentsBlueprint(plan, now) {
    const sections = getPlannedSections(plan);
    const wantsBooking = hasAppointmentSignal(plan, sections);
    ...
    return {
        ...
        websiteBuilderBlocks: [
            ...(hasPublicBookingBlock ? [{
                    ...
                    purpose: 'appointment_cta',
                    status: 'draft',
                }] : []),
            ...(wantsBooking ? [{
                    componentId: 'publicBookingForm',
                    purpose: 'public_booking_form',
                    status: 'needs_review',
                }] : []),
        ],
    };
}
function createRestaurantBlueprint(plan, now) {
    return createRestaurantBlueprintFromWebsitePlan(plan, now);
}
function createRealEstateBlueprint(plan, now) {
    ...
    const propertyDrafts = isRealEstate
        ? (plan.contentMap.properties || []).slice(0, 12).map((property, index) => {
        ...
        : [];
    const offer = (enabled = isRealEstate) => ({
        enabled,
        status: enabled ? 'needs_review' : 'disabled',
        priceSource: 'unset',
        needsReview: enabled,
    });
    const artifact = (key, module, title, description, dependencies = [], analyticsEvents = []) => ({
        id: `real-estate-${key}`,
        key,
        module,
        title,
...
export function isRealEstateBlueprintV2(value) {
    if (!value || typeof value !== 'object' || Array.isArray(value))
        return false;
    const blueprint = value;
    return Boolean(blueprint.agentProfile &&
...
export function normalizeRealEstateBlueprint(value, fallback) {
    if (isRealEstateBlueprintV2(value))
        return value;
    ...
    return {
        ...
        leadFunnels: !Array.isArray(legacy.leadFunnels) && legacy.leadFunnels ? legacy.leadFunnels : base.leadFunnels,
...
function createFinanceBlueprint(plan, now) {
    return {
        ...createBlueprintModuleState(now, {
...
function createAnalyticsBlueprint(plan, now) {
    return {
        ...createBlueprintModuleState(now),
        events: [
            'newsletter_signup',
...
function createAutomationBlueprint(plan, now) {
    return {
        ...createBlueprintModuleState(now, {
            needsReview: true,
...
export function createBusinessBlueprintFromWebsitePlan(plan, options = {}) {
    ...
    return {
        ...
        brandProfile: {
            ...createBlueprintModuleState(now),
            colors: plan.brandProfile.colors,
...
export function isBusinessBlueprint(value) {
    return Boolean(value &&
        typeof value === 'object' &&
        !Array.isArray(value) &&
        value.schemaVersion === BUSINESS_BLUEPRINT_SCHEMA_VERSION &&
        typeof value.blueprintVersion === 'string' &&
        Boolean(value.businessProfile) &&
        Boolean(value.websiteBlueprint));
}
function isCurrentRestaurantBlueprint(value) {
    if (!value || typeof value !== 'object' || Array.isArray(value))
        return false;
    const blueprint = value;
    return Boolean(blueprint.profile &&
        blueprint.menuDraft &&
...
function isCurrentChatbotBlueprint(value) {
    if (!value || typeof value !== 'object' || Array.isArray(value))
        return false;
    const blueprint = value;
    return Boolean(blueprint.engineVersion === 'v2' &&
...
export function migrateBusinessBlueprint(value) {
    if (!value || typeof value !== 'object' || Array.isArray(value))
        return null;
    if (isBusinessBlueprint(value)) {
        ...
        if (normalizedRestaurantBlueprint === value.restaurantBlueprint &&
...
export function shouldProtectFromRegeneration(module) {

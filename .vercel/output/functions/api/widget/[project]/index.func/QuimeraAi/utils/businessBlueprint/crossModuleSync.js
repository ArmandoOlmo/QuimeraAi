import { createCrossModuleSyncKey, findCrossModuleDraftMatch, isAiGeneratedCrossModuleDraft, isUserModifiedCrossModuleDraft, } from './crossModuleSyncIdempotency';
import { getRestaurantEcommerceOfferKeys } from './restaurantBlueprint';
import { buildRealtyMonetizationOfferDrafts, REALTY_MONETIZATION_BLOCKER_MESSAGES, } from '../realtyMonetization';
...
function uniqueValues(values, limit = 60) {
    const seen = new Set();
    const result = [];
...
function hasGiftCards(ecommerceBlueprint) {
    return ecommerceBlueprint.giftCardsEnabled === true || ecommerceBlueprint.giftCards?.enabled === true;
}
function hasRestaurantContext(blueprint, ecommerceBlueprint) {
    return includesAny([
...
function hasRealEstateContext(blueprint, ecommerceBlueprint) {
    return includesAny([
        blueprint.businessProfile.industry,
        blueprint.businessProfile.description,
...
function hasServicesContext(blueprint, ecommerceBlueprint) {
    return includesAny([
        blueprint.businessProfile.industry,
        blueprint.businessProfile.description,
        ecommerceBlueprint.storeType,
...
function createDraft(input, module, itemType, name, details) {
...
function createChatbotDrafts(input) {
...
function createLeadDrafts(input) {
    const blueprint = input.businessBlueprint;
    const ecommerceBlueprint = input.ecommerceBlueprint || blueprint.ecommerceBlueprint;
...
function createEmailFlowDraft(input, type, name, triggerType, subjectDraft, bodyOutlineDraft, variablesNeeded) {
...
function createEmailDrafts(input) {
    const blueprint = input.businessBlueprint;
    const ecommerceBlueprint = input.ecommerceBlueprint || blueprint.ecommerceBlueprint;
    const drafts = [
...
function createAnalyticsDrafts(input) {
    ...
    return eventNames.map(eventName => createDraft(input, 'analytics', 'event_definition', eventName, {
        ...
        metadata: {
            eventName,
            realEstateEngine: shouldSyncRealty && (eventName.startsWith('realty_')
                || (realty?.analytics?.events || []).includes(eventName)
                || (realty?.analyticsEvents || []).includes(eventName)
...
function createRealEstateEcommerceDrafts(input) {
    if (!shouldCreateRealEstateSyncDrafts(input))
        return [];
...
function createFinanceDrafts(input) {
    if (!shouldCreateRealEstateSyncDrafts(input))
        return [];
    const realty = input.businessBlueprint.realEstateBlueprint;
...
function planModule(input, module, candidates) {
...
function buildResult(input, dryRun) {
    const blueprint = input.businessBlueprint;
    const ecommerceBlueprint = input.ecommerceBlueprint || blueprint.ecommerceBlueprint;
    ...
    const addPlan = (module, candidates, enabled) => {
    ...
    const planFor = (module) => plans.find(plan => plan.module === module);
    ...
    const nextBusinessBlueprint = applyState ? {
        ...blueprint,
        updatedAt: now,
        lastSyncedAt: now,
        crossModuleSync: {

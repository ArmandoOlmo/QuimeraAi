...
const normalizeIndustry = (value) => {
    ...
    if (/\b(electric bike|e-bike|ebike|bicycle|bike|cycling|bicicleta)\b/i.test(text))
        return 'electric_bikes';
    if (/\b(ai|artificial intelligence|software|saas|app|platform)\b/i.test(text))
        return 'ai_saas';
    if (/\b(real estate|realtor|broker|property|inmobili|bienes raices)\b/i.test(text))
        return 'real_estate';
    if (/\b(restaurant|restaurante|caf[eé]|food|comida|catering|steakhouse|bakery|panader[ií]a|bar|sushi|pizza|brunch|fine dining|casual dining|food truck)\b/i.test(text))
        return 'restaurant';
    if (/\b(gallery|portfolio|photography|artist|art|creative)\b/i.test(text))
        return 'portfolio';
    if (/\b(luxury|premium|boutique|high-end)\b/i.test(text))
        return 'premium_retail';
    if (/\b(ecommerce|shop|store|retail|tienda)\b/i.test(text))
        return 'ecommerce';
    if (/\b(local|service|consulting|repair|appointment)\b/i.test(text))
        return 'services';
    return text.trim() || 'general';
};
export function classifyBusinessCapabilities(input, availableData = {}, ecommerceBlueprint) {
    ...
    if (input.hasEcommerce || input.existingWebsitePlan?.businessProfile.hasEcommerce || ecommerceBlueprint?.enabled)
        add('ecommerce');
    if ((availableData.productsCount || 0) > 0 || ecommerceBlueprint?.starterProducts.length)
        add('physicalProducts');
    if ((availableData.servicesCount || 0) > 0 || input.services?.length || input.existingWebsitePlan?.businessProfile.services.length)
        add('services');
    if (ecommerceBlueprint?.digitalProductsEnabled)
        add('digitalProducts');
    if (ecommerceBlueprint?.giftCardsEnabled)
        add('ecommerce');
    if (!capabilities.includes('leadGeneration'))
        add('leadGeneration');
    if (capabilities.includes('restaurant'))
        add('localBusiness');
...
export function classifyPageIntent(input) {
    ...
    const has = (capability) => input.capabilities.includes(capability);
    if (input.builder === 'storefront')
        return 'storefront_home';
    if (/\b(product detail|product page|detalle de producto)\b/i.test(text))
        return 'product_detail';
    if (/\b(collection|category page|categoria|coleccion)\b/i.test(text))
        return 'product_collection';
    if (has('realEstate'))
        return 'real_estate_home';
    if (has('restaurant'))
        return 'restaurant_home';
    if (has('portfolio') && /\b(gallery|galeria)\b/i.test(text))
        return 'gallery_home';
    if (has('portfolio'))
        return 'portfolio_home';
    if (/\b(ai|saas|software|platform|app|automation|agent)\b/i.test(text) && (!has('physicalProducts') || has('subscription') || has('digitalProducts')))
        return 'ai_saas_landing';
    if (has('ecommerce'))
        return 'ecommerce_home';
    if (has('appointments') || has('booking'))
        return 'appointment_landing';
    if (has('localBusiness'))
        return 'local_business_home';
    if (has('services'))
        return 'service_landing';
    if (has('contentMarketing'))
        return 'blog_home';
    if (has('leadGeneration'))
        return 'lead_capture';
    return 'website_home';
}
function availableDataFromInput(input, ecommerceBlueprint) {
...
export function createComponentSelectionContext(input, options = {}) {
    ...
    return {
        ...
        availableData,
        existingComponentIds: input.existingWebsitePlan?.componentPlan.map(item => item.component),
    };
}
function industryScore(componentIndustries, industry) {
    if (componentIndustries.includes('all'))
        return 0.85;
    if (componentIndustries.includes(industry))
        return 1;
    if (componentIndustries.some(item => industry.includes(item) || item.includes(industry)))
        return 0.85;
    if (industry === 'electric_bikes' && componentIndustries.some(item => ['premium_retail', 'fitness', 'ecommerce'].includes(item)))
        return 0.9;
    if (industry === 'premium_retail' && componentIndustries.includes('ecommerce'))
        return 0.75;
    if (industry === 'services' && componentIndustries.includes('local_business'))
        return 0.75;
    return 0.3;
}
function dataAvailabilityScore(componentId, context) {
...
function conversionScore(componentRoles, capabilities) {
    const roleSignals = new Set();
    if (capabilities.includes('ecommerce'))
        roleSignals.add('sales');
    if (capabilities.includes('leadGeneration'))
        roleSignals.add('lead_capture');
    if (capabilities.includes('appointments') || capabilities.includes('booking') || capabilities.includes('restaurantReservations'))
        roleSignals.add('booking');
    if (capabilities.includes('portfolio') || capabilities.includes('realEstate'))
        roleSignals.add('trust');
    if (capabilities.includes('marketplace'))
        roleSignals.add('catalog_discovery');
    if (componentRoles.some(role => roleSignals.has(role)))
        return 1;
    if (componentRoles.includes('education') || componentRoles.includes('navigation'))
        return 0.65;
    return 0.45;
}
function scoreComponent(componentId, context) {
    ...
    let penalties = 0;
    if (!component.compatibleBuilders.includes(context.builder))
        penalties += 0.35;
    if (component.incompatibleIndustries.some(item => context.industry.includes(item)))
        penalties += 0.25;
    if (component.requiredModules.some(module => !(context.capabilities || []).includes(module)))
        penalties += 0.25;
    if (componentId === 'productCarousel' && (context.availableData?.productsCount || 0) < 4)
        penalties += 0.15;
    if (componentId === 'featuredProducts' && !((context.availableData?.productsCount || 0) > 0 || context.availableData?.hasDraftProducts))
        penalties += 0.4;
    if (componentId === 'categoryShowcase' && (context.availableData?.categoriesCount || 0) === 0)
        penalties += 0.3;
    if (componentId === 'saleCountdown' && !context.availableData?.hasMerchantApprovedPromotion)
        penalties += 0.45;
    const total = Math.max(0, Math.min(1, (industryMatch * 0.25 +
        pageIntentMatch * 0.2 +
        dataAvailability * 0.2 +
        conversionGoalMatch * 0.15 +
        visualFit * 0.1 +
        mobileFit * 0.1) - penalties));
    return {
        industryMatch,
        pageIntentMatch,
        dataAvailability,
...
function reasonFor(componentId, context) {
    const pageIntent = context.pageIntent || 'website_home';
    if (componentId === 'hero')
        return `Primary ${pageIntent} section selected from registry for ${context.industry}.`;
    if (componentId === 'featuredProducts')
        return 'Ecommerce is enabled and product data or reviewable starter drafts exist.';
    if (componentId === 'categoryShowcase')
        return 'Ecommerce categories are available for catalog discovery.';
    if (componentId === 'productCarousel')
        return 'Enough products exist to justify a product carousel.';
    if (componentId === 'shopCTA')
        return 'Routes visitors to storefront without creating ecommerce data.';
    if (componentId === 'appointmentCTA')
        return 'Business signals support appointment, repair, consultation, reservation, or booking capture.';
    if (componentId === 'restaurantMenu')
        return 'Restaurant intent benefits from menu or offer highlights.';
    if (componentId === 'realEstateListings')
        return 'Real estate intent benefits from listing visibility and lead capture.';
    if (componentId === 'leadForm')
        return 'Lead generation is a core capability for this page.';
    return `Selected because it matches ${pageIntent}, ${context.industry}, and available business capabilities.`;
}
export function selectComponentsForPage(context) {
    const pageIntent = context.pageIntent || 'website_home';
...
export function selectAiStudioComponents(input, options = {}) {
...
//# sourceMappingURL=componentSelection.js.map
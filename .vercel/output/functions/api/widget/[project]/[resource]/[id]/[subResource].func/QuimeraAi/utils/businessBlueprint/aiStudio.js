import { critiqueComponentDesign, deriveCrossModuleBlueprints, deriveEcommerceBlueprintFromBusinessBrief, deriveRestaurantBlueprintFromBusinessBrief, deriveStorefrontBlueprintFromBusinessBrief, deriveWebsiteEcommerceBlocks, selectAiStudioComponents, selectComponentVariants, validateComponentPlan, } from '../aiStudio';
...
function businessBriefInputFromWebsitePlan(plan, now) {
    return {
...
export function createAiStudioBusinessBlueprint(plan, options = {}) {
    ...
    const componentVariantSelection = selectComponentVariants(componentSelection.componentPlan, componentSelection.context);
...
export function attachAiStudioBusinessBlueprint(project, plan, options = {}) {
    const businessBlueprint = createAiStudioBusinessBlueprint(plan, {
        ...options,
        projectId: project.id,
    });
    const data = applyRenderableSectionVariantSettingsToData(project.data, businessBlueprint.websiteBlueprint.sectionBlueprints);
    return {
        ...project,
        data,
        businessBlueprint,

...
const uniqueValues = (values, limit = 16) => {
    const seen = new Set();
    const result = [];
...
const formatList = (label, values) => values.length ? `${label}: ${values.join(', ')}` : null;
const listingHasReviewedPublicData = (listing) => listing.status === 'active' && listing.publicEnabled === true && listing.needsReview === false;
...
const baseMetadata = (type, realty, sourcePath) => ({
...
const profileContent = (blueprint, realty) => {
...
export function buildRealtyChatbotKnowledgeDrafts(blueprint) {
    if (!blueprint?.realEstateBlueprint)
        return [];
...
//# sourceMappingURL=realtyChatbotKnowledge.js.map
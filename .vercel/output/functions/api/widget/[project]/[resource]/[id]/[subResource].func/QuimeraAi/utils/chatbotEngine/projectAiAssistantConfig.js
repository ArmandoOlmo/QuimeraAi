...
function mergeDocuments(primary, fallback) {
    const primaryDocuments = arrayValue(primary)
        .filter(document => document && typeof document === 'object' && !Array.isArray(document));
    const fallbackDocuments = arrayValue(fallback)
        .filter(document => document && typeof document === 'object' && !Array.isArray(document));
...
function getBusinessBlueprint(data) {
    return (nonEmptyRecord(data?.businessBlueprint) ||
        nonEmptyRecord(data?.data?.businessBlueprint));
}
function getBrandPrimaryColor(blueprint) {
    const colors = nonEmptyRecord(blueprint?.brandProfile?.colors);
    return (stringValue(colors?.primary) ||
        stringValue(colors?.primaryColor) ||
        stringValue(colors?.accent) ||
        '#111827');
}
function labelLanguages(languages) {
    ...
    if (!unique.includes('Spanish'))
        unique.push('Spanish');
    if (!unique.includes('English'))
        unique.push('English');
    return unique.join(', ');
}
...
function buildBlueprintKnowledgeDocument(blueprint, chatbotBlueprint) {
    ...
    const sourceTimestamp = stringValue(metadata?.generatedAt) ||
...
function deriveConfigFromChatbotBlueprint(data) {
    ...
    if (!blueprint || !chatbotBlueprint)
        return null;
    if (chatbotBlueprint.enabled === false || chatbotBlueprint.status === 'disabled')
        return null;
    const agentProfile = nonEmptyRecord(chatbotBlueprint.agentProfile);
    const businessProfile = nonEmptyRecord(blueprint.businessProfile);
...
function hydrateLegacyConfigWithBlueprint(legacyConfig, blueprintConfig) {
    if (!blueprintConfig)
        return legacyConfig;
...
export function resolveProjectAiAssistantConfig(project) {
    ...
    const legacyConfig = (nonEmptyRecord(project.ai_assistant_config) ||
...
//# sourceMappingURL=projectAiAssistantConfig.js.map
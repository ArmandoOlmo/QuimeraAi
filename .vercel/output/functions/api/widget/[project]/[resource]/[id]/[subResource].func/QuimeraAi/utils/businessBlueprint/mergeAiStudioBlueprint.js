import { BUSINESS_BLUEPRINT_SCHEMA_VERSION, BUSINESS_BLUEPRINT_VERSION, } from '../../types/businessBlueprint';
import { getStorefrontCatalogSize, } from '../storefrontTheme';
...
function protectedOrMerged(existing, next) {
    return existing && shouldProtectFromRegeneration(existing) ? existing : next;
}
function mapBlockSource(block) {
    switch (block.type) {
        case 'giftCardBlock':
            return 'gift_cards';
...
function toWebsiteEcommerceBlockBlueprint(block, index, now) {
...
function getSelectedWebsiteSections(input) {
    const sections = [];
    input.componentPlan?.selectedComponents.forEach(selection => {
        const component = getComponentDefinition(selection.componentId);
        if (component?.implementationStatus !== 'rendered')
...
function enrichWebsiteSectionBlueprint(section, input, designBySection) {
    const design = designBySection.get(section.type);
    if (!design || shouldProtectFromRegeneration(section))
        return section;
...
function mergeWebsiteBlueprint(base, existing, blocks, now, designInput = {}) {
    ...
    selectedWebsiteSections.forEach(section => {
        if (baseSectionBlueprints.some(item => item.type === section))
            return;
    ...
    const next = {
        ...
        sectionBlueprints: baseSectionBlueprints.map(section => (enrichWebsiteSectionBlueprint(section, designInput, designBySection))),
    ...
    if (!existing)
        return next;
    if (existing.metadata.lockedFromRegeneration)
        return existing;
    ...
    const existingSectionList = (existing.sections || existing.componentOrder || [])
        .filter(section => !isRetiredDesignSuiteSection(section));
    const existingSections = new Set(existingSectionList);
    ...
    const protectedOrder = new Map(protectedSections.map(section => [section.type, section.order]));
    sections.sort((a, b) => {
    ...
    const sanitizedSectionVisibility = Object.fromEntries(Object.entries(sectionVisibility).filter(([section]) => !isRetiredDesignSuiteSection(section)));
...
function mergeEcommerceBlueprint(base, existing, derived, now) {
    const next = {
        ...base,
...
function mergeStorefrontBlueprint(base, existing, derived, ecommerceBlueprint, now) {
    const next = {
        ...base,
        enabled: derived.enabled,
        status: derived.enabled ? 'needs_review' : 'disabled',
...
function mergeChatbotBlueprint(base, existing, derived, now) {
...
function mergeLeadBlueprint(base, existing, derived, now) {
    const next = {
        ...base,
        enabled: derived.enabled,
        status: derived.status,
...
function mergeEmailMarketingBlueprint(base, existing, derived, now) {
    const next = {
        ...base,
        enabled: derived.enabled,
        status: derived.status,
...
function mergeRestaurantBlueprint(base, existing, derived, now) {
    const next = {
        ...(derived || base),
        metadata: {
...
export function mergeAiStudioBlueprint(input) {
...
//# sourceMappingURL=mergeAiStudioBlueprint.js.map
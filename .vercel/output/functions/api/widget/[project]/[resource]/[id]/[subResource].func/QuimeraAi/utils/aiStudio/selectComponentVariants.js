...
function preferredHeroVariant(context) {
    ...
    if (context.pageIntent === 'ai_saas_landing')
        return 'gradientOrb';
    if (context.pageIntent === 'restaurant_home' || context.pageIntent === 'portfolio_home' || context.pageIntent === 'gallery_home')
        return 'editorialOverlay';
    if (context.pageIntent === 'real_estate_home')
        return 'imageBackground';
    if (context.pageIntent === 'local_business_home')
        return 'splitMediaLeft';
    return 'splitMediaRight';
}
function fallbackVariantFor(componentId, pageIntent) {
    if (componentId === 'hero')
        return preferredHeroVariant({ inputText: '', industry: '', builder: 'website', pageIntent });
...
function chooseLayoutVariant(componentId, context, lastVariants) {
    const anatomy = getComponentAnatomy(componentId);
    if (!anatomy)
        return { layoutVariant: 'unknown', patternIds: [] };
...
function chooseStyleVariant(componentId, context) {
    const anatomy = getComponentAnatomy(componentId);
    if (!anatomy)
        return fallbackStyle;
    if (context.industry === 'ai_saas' && anatomy.styleVariants.some(variant => variant.id === 'aiSaas'))
        return 'aiSaas';
    if (['premium_retail', 'electric_bikes'].includes(context.industry) && anatomy.styleVariants.some(variant => variant.id === 'premiumRetail'))
        return 'premiumRetail';
    if (['restaurant', 'portfolio'].includes(context.industry) && anatomy.styleVariants.some(variant => variant.id === 'editorial'))
        return 'editorial';
    if (['services', 'local_business'].includes(context.industry) && anatomy.styleVariants.some(variant => variant.id === 'localTrust'))
        return 'localTrust';
    return anatomy.styleVariants[0]?.id || fallbackStyle;
}
function chooseBackground(componentId, layoutVariant) {
    const anatomy = getComponentAnatomy(componentId);
    if (!anatomy)
        return 'plain';
    if (['editorialOverlay', 'imageBackground', 'productSpotlight', 'mapRight'].includes(layoutVariant) && anatomy.backgroundOptions.includes('imageOverlay'))
        return 'imageOverlay';
    if (layoutVariant === 'gradientOrb' && anatomy.backgroundOptions.includes('gradient'))
        return 'gradient';
    if (['premiumCards', 'editorialTiles', 'bentoGrid'].includes(layoutVariant) && anatomy.backgroundOptions.includes('surface'))
        return 'surface';
    return anatomy.backgroundOptions[0] || 'plain';
}
function chooseMedia(componentId, layoutVariant) {
    const anatomy = getComponentAnatomy(componentId);
    if (!anatomy)
        return 'none';
    if (['featuredProducts', 'productCarousel', 'productHero'].includes(componentId) && anatomy.mediaOptions.includes('productMedia'))
        return 'productMedia';
    if (['gallery', 'imageWithText'].includes(componentId) && anatomy.mediaOptions.includes('gallery'))
        return 'gallery';
    if (['realEstateListings', 'propertySearch'].includes(componentId) && anatomy.mediaOptions.includes('listingCards'))
        return 'listingCards';
    if (['contact', 'restaurantLocation'].includes(componentId) && anatomy.mediaOptions.includes('map'))
        return 'map';
    if (layoutVariant.includes('Grid') && anatomy.mediaOptions.includes('imageGrid'))
        return 'imageGrid';
    if (anatomy.mediaOptions.includes('singleImage') && /(image|media|spotlight|overlay|banner)/i.test(layoutVariant))
        return 'singleImage';
    return anatomy.mediaOptions[0] || 'none';
}
function chooseDensity(componentId, layoutVariant) {
    if (/(compact|strip|marketplace|table|grid)/i.test(layoutVariant) && !['hero', 'gallery'].includes(componentId))
        return 'dense';
    if (/(editorial|spotlight|imageBackground|productSpotlight)/i.test(layoutVariant))
        return 'rich';
    return 'balanced';
}
function chooseMobileBehavior(componentId, layoutVariant) {
    const anatomy = getComponentAnatomy(componentId);
    const variant = anatomy?.layoutVariants.find(item => item.id === layoutVariant);
    if (variant?.mobileBehavior)
        return variant.mobileBehavior;
    if (/(carousel)/i.test(layoutVariant))
        return 'carouselMobile';
    if (/(accordion)/i.test(layoutVariant))
        return 'accordionMobile';
    if (/(compact|strip)/i.test(layoutVariant))
        return 'compactStrip';
    return 'stackedMobile';
}
function activeSlotsFor(componentId, layoutVariant) {
    const anatomy = getComponentAnatomy(componentId);
    if (!anatomy)
        return [];
    const variant = anatomy.layoutVariants.find(item => item.id === layoutVariant);
...
export function selectComponentVariants(componentPlan, context) {
    const warnings = [];
    const lastVariants = [];
    ...
    if (variants.length >= 4 && centeredCount === variants.length)
        warnings.push('All selected sections are centered; visual rhythm should vary.');
    if (variants.length >= 4 && gridCount / variants.length > 0.65)
        warnings.push('Too many selected sections use grids/cards.');
    return {
        variants,
        warnings,
        sourceMap: {
...
//# sourceMappingURL=selectComponentVariants.js.map
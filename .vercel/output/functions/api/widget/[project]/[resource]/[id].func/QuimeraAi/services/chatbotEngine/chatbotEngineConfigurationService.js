...
import { appendBlueprintSnapshot, createBlueprintSnapshot, } from '../../utils/businessBlueprint/versionHistory';
...
function buildKnowledgeSourceId(input, contentHash) {
...
function buildKnowledgePreview(content, sourceUrl) {
    const source = content || sourceUrl;
    if (!source)
        return undefined;
    return source.replace(/\s+/g, ' ').trim().slice(0, 500) || undefined;
}
function getBusinessBlueprintTenantId(blueprint) {
    const metadata = isRecord(blueprint.metadata)
        ? blueprint.metadata
        : {};
...
async function recordConfigurationMutationEvent(projectId, loaded, input, client, event) {
...
function ensureConfigurableKnowledgeSource(source, input) {
    if (!input.enabled)
        return;
...
function createKnowledgeSourceBlueprint(input, now) {
...
export function addChatbotKnowledgeSourceToBlueprint(blueprint, input) {
    const now = input.now || new Date().toISOString();
...
function updateChatbotBlueprintMetadata(blueprint, input, now) {
    return {
        ...blueprint.metadata,
        userModified: true,
        lockedFromRegeneration: true,
...
function updateChatbotBlueprintSourceMap(blueprint, sourceMapKey, sourceMapValue) {
    return {
        ...(blueprint.sourceMap || {}),
        [sourceMapKey]: sourceMapValue,
    };
}
async function loadProjectChatbotConfig(projectId, client) {
...
async function persistProjectChatbotBlueprint(projectId, projectData, chatbotBlueprint, now, client) {
    ...
    const nextBusinessBlueprint = nextData.businessBlueprint;
...
function resolveTestingEvaluationStatus(testScenarios) {
    if (testScenarios.length === 0)
        return 'not_run';
    if (testScenarios.some(scenario => scenario.status === 'failed'))
        return 'failing';
    if (testScenarios.every(scenario => scenario.status === 'passed'))
        return 'passing';
    return 'needs_review';
}
function resolveTestingReadiness(evaluationStatus) {
    if (evaluationStatus === 'passing')
        return { isReady: true, blockers: [], warnings: [] };
    if (evaluationStatus === 'failing')
        return { isReady: false, blockers: [testingFailedBlocker], warnings: [] };
    if (evaluationStatus === 'needs_review')
        return { isReady: false, blockers: [], warnings: [testingNeedsReviewWarning] };
    return { isReady: false, blockers: [], warnings: [testingNotRunWarning] };
}
function uniqueValues(values) {
    return Array.from(new Set(values));
}
function isDeployableStatus(status) {
    return ['draft', 'test', 'deployed', 'paused', 'disabled'].includes(status);
}
function getSurfaceReadiness(status) {
    if (status === 'deployed')
        return { isReady: true, blockers: [], warnings: [] };
    if (status === 'test')
        return { isReady: false, blockers: [], warnings: [surfaceTestWarning] };
    return { isReady: false, blockers: [], warnings: [surfacePausedWarning] };
}
function resolveDeploymentStatus(channels) {
    const channelValues = SURFACE_DEPLOYMENT_KEYS.map(key => channels[key]);
    if (channelValues.some(channel => channel.enabled && channel.status === 'deployed'))
        return 'deployed';
    if (channelValues.some(channel => channel.enabled && channel.status === 'test'))
        return 'test';
    if (channelValues.some(channel => channel.status === 'paused'))
        return 'paused';
    return 'draft';
}
function syncDeploymentFromChannels(blueprint) {
    const deployedSurfaces = uniqueValues(SURFACE_DEPLOYMENT_KEYS
...
function updateSurfaceChannelStatus(surface, status) {
    return {
        ...surface,
...
export function reviewChatbotActionInBlueprint(blueprint, input) {
    const now = input.now || new Date().toISOString();
...
export function reviewChatbotKnowledgeSourceInBlueprint(blueprint, input) {
    const now = input.now || new Date().toISOString();
...
export function updateChatbotTestScenarioInBlueprint(blueprint, input) {
    const now = input.now || new Date().toISOString();
    let reviewedScenario = null;
...
export function updateChatbotSurfaceDeploymentInBlueprint(blueprint, input) {
    const now = input.now || new Date().toISOString();
...
export function updateChatbotVoiceSettingsInBlueprint(blueprint, input) {
    const now = input.now || new Date().toISOString();
    ...
    const voiceStatus = input.enabled ? 'deployed' : 'disabled';
    const nextChannels = {
        ...blueprint.channels,
        voice: {
...
export function applyChatbotBlueprintToProjectData(projectDataInput, chatbotBlueprint) {
...
export async function getChatbotActionRegistry(projectId, client = supabase) {
    const { data, error } = await client
        .from('projects')
        .select('data')
        .eq('id', projectId)
...
export async function getChatbotConfig(projectId, client = supabase) {
...
export async function updateChatbotConfig(projectId, chatbotBlueprint, input = {}, client = supabase) {
    const now = input.now || new Date().toISOString();
    const loaded = await loadProjectChatbotConfig(projectId, client);
...
export async function updateProjectChatbotActionReview(projectId, input, client = supabase) {
    const now = input.now || new Date().toISOString();
    const loaded = await loadProjectChatbotConfig(projectId, client);
...
export async function updateProjectChatbotKnowledgeSourceReview(projectId, input, client = supabase) {
    const now = input.now || new Date().toISOString();
    const loaded = await loadProjectChatbotConfig(projectId, client);
...
export async function addProjectChatbotKnowledgeSource(projectId, input, client = supabase) {
    const now = input.now || new Date().toISOString();
    const loaded = await loadProjectChatbotConfig(projectId, client);
...
export async function updateProjectChatbotTestScenarioStatus(projectId, input, client = supabase) {
    const now = input.now || new Date().toISOString();
    const loaded = await loadProjectChatbotConfig(projectId, client);
...
export async function updateProjectChatbotSurfaceDeployment(projectId, input, client = supabase) {
    const now = input.now || new Date().toISOString();
    const loaded = await loadProjectChatbotConfig(projectId, client);
...
export async function updateProjectChatbotVoiceSettings(projectId, input, client = supabase) {
    const now = input.now || new Date().toISOString();
    const loaded = await loadProjectChatbotConfig(projectId, client);
...
export function enableAction(projectId, actionType, actorId, client = supabase) {
    return updateProjectChatbotActionReview(projectId, { actionType, enabled: true, actorId }, client);
}
export function disableAction(projectId, actionType, actorId, client = supabase) {
    return updateProjectChatbotActionReview(projectId, { actionType, enabled: false, actorId }, client);
}
//# sourceMappingURL=chatbotEngineConfigurationService.js.map
...
export const REALTY_MONETIZATION_BLOCKER_MESSAGES = {
...
const normalizeStatus = (status) => {
    if (status === 'needs_review' || status === 'configured' || status === 'disabled')
        return status;
    return 'draft';
};
const normalizePriceSource = (priceSource) => {
    if (priceSource === 'user-provided' || priceSource === 'ai-suggested' || priceSource === 'imported')
        return priceSource;
    return 'unset';
};
const createDefaultOffer = (config, blueprint) => {
...
const isPaymentConfigured = (blueprint) => blueprint.ecommerceBlueprint?.paymentMode === 'test' || blueprint.ecommerceBlueprint?.paymentMode === 'live';
const isTaxConfigured = (blueprint) => blueprint.ecommerceBlueprint?.taxMode === 'manual' || blueprint.ecommerceBlueprint?.taxMode === 'automatic';
const buildReadinessBlockers = (blueprint, offer, description) => {
    const blockers = [];
    if (!isPaymentConfigured(blueprint))
        blockers.push('payment_not_configured');
    if (!isTaxConfigured(blueprint))
        blockers.push('tax_not_configured');
    if (offer.needsReview !== false || offer.status !== 'configured')
        blockers.push('needs_merchant_review');
    if (!description.trim())
        blockers.push('missing_product_description');
    if (normalizePriceSource(offer.priceSource) === 'unset')
        blockers.push('missing_price');
    if (offer.enabled === false || offer.status === 'disabled')
        blockers.push('offer_disabled');
    blockers.push('checkout_disabled');
    return blockers;
};
const buildProductDraft = (config, offer) => {
...
export function buildRealtyMonetizationOfferDrafts(blueprint) {
    if (!blueprint)
        return [];
...
//# sourceMappingURL=realtyMonetization.js.map
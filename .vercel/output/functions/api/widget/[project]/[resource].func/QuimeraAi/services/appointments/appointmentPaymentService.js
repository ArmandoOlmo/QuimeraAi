...
const readPaymentMode = (metadata) => (normalizeString(metadata.paymentMode, 80)
    || normalizeString(metadata.appointmentPaymentMode, 80)
    || 'none').toLowerCase();
const readPaymentAmount = (metadata) => (normalizeAmount(metadata.depositAmount)
...
const buildOrderNumber = (appointment) => {
    const suffix = appointment.id.replace(/[^a-zA-Z0-9]/g, '').slice(0, 8).toUpperCase() || Date.now().toString(36).toUpperCase();
    return `APT-${suffix}`;
};
export async function createAppointmentPaymentDraft(client, appointment) {
    const warnings = [];
    ...
    if (!requiresPayment)
        return { warnings };
    if (!amount)
        return { warnings: ['payment_draft_skipped_missing_amount'] };
    if (!appointment.projectId)
        return { warnings: ['payment_draft_skipped_missing_project'] };
    ...
    if (!customerEmail || !customerName)
        return { warnings: ['payment_draft_skipped_missing_customer'] };
...
//# sourceMappingURL=appointmentPaymentService.js.map
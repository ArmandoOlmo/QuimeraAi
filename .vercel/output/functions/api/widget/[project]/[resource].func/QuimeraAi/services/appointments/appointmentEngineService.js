...
const ACTIVE_BOOKING_STATUSES = ['scheduled', 'confirmed', 'in_progress', 'rescheduled'];
...
const toIso = (value) => {
    ...
    if (Number.isNaN(date.getTime()))
        throw Object.assign(new Error('Invalid appointment date.'), { status: 400, code: 'invalid_date' });
    return date.toISOString();
};
export const timestampFromIso = (value) => {
    if (!value)
...
const buildInitialMeetingNotes = (input, content, source) => {
...
async function createOrLinkLeadForCanonicalAppointment(client, input, appointment, startIso, appointmentNotes, metadata, tags) {
    const warnings = [];
    ...
    const nextParticipants = leadId
        ? nextAppointment.participants?.map((participant, index) => (index === 0 && participant.type === 'lead' && !participant.leadId
            ? { ...participant, leadId }
            : participant))
        : nextAppointment.participants;
    const needsParticipantLink = Boolean(leadId
        && nextParticipants
    ...
    const hasReadableAppointmentNote = Boolean(readableAppointmentNotes) && (nextAppointment.notes || []).some(note => (typeof note.content === 'string'
    ...
    const repairedAppointmentNotes = readableAppointmentNotes && !hasReadableAppointmentNote
        ? [
            ...(nextAppointment.notes || []).filter(note => !(note.aiGenerated === true
                && typeof note.content === 'string'
                && note.content.includes('Resumen de solicitud del cliente / Customer request summary'))),
            ...buildInitialMeetingNotes(input, readableAppointmentNotes, (input.source || appointment.source || 'dashboard')),
        ]
        : undefined;
...
const isOverlappingRange = (start, end, busyStart, busyEnd) => start < busyEnd && end > busyStart;
const formatSlotLabel = (start, end) => `${start.toLocaleDateString(undefined, { month: 'short', day: 'numeric' })} ${start.toLocaleTimeString(undefined, { hour: '2-digit', minute: '2-digit' })} - ${end.toLocaleTimeString(undefined, { hour: '2-digit', minute: '2-digit' })}`;
const getAppointmentLocale = (appointment) => (typeof appointment.metadata?.locale === 'string' ? appointment.metadata.locale : undefined);
export const mapAppointmentRowToAppointment = (row) => ({
    id: row.id,
    title: row.title || '',
...
export const mapBlockedTimeRowToBlockedDate = (row) => ({
    id: row.id,
    title: row.title || '',
    startDate: timestampFromIso(row.start_date),
    endDate: timestampFromIso(row.end_date),
...
export function mapAppointmentPatchToRow(appointment, tenantId, projectId) {
    const row = {};
    ...
    if (appointment.startDate !== undefined)
        row.start_date = toIso(appointment.startDate);
    if (appointment.endDate !== undefined)
        row.end_date = toIso(appointment.endDate);
    if (appointment.timezone !== undefined)
        row.timezone = appointment.timezone;
    if (appointment.allDay !== undefined)
        row.all_day = appointment.allDay;
    if (appointment.organizerId !== undefined)
        row.organizer_id = appointment.organizerId;
    if (appointment.organizerName !== undefined)
        row.organizer_name = appointment.organizerName;
    if (appointment.organizerEmail !== undefined)
        row.organizer_email = appointment.organizerEmail;
    if (appointment.participants !== undefined)
        row.participants = appointment.participants;
    if (appointment.location !== undefined)
        row.location = appointment.location;
    if (appointment.recurrence !== undefined)
        row.recurrence = appointment.recurrence;
    if (appointment.isRecurringInstance !== undefined)
        row.is_recurring_instance = appointment.isRecurringInstance;
    if (appointment.reminders !== undefined)
        row.reminders = appointment.reminders;
    if (appointment.attachments !== undefined)
        row.attachments = appointment.attachments;
    if (appointment.notes !== undefined)
        row.notes = appointment.notes;
    if (appointment.followUpActions !== undefined)
        row.follow_up_actions = appointment.followUpActions;
    if (appointment.aiInsights !== undefined)
        row.ai_insights = appointment.aiInsights;
    if (appointment.aiPrepEnabled !== undefined)
        row.ai_prep_enabled = appointment.aiPrepEnabled;
    if (appointment.autoTranscription !== undefined)
        row.auto_transcription = appointment.autoTranscription;
    if (appointment.googleSync !== undefined)
        row.google_sync = appointment.googleSync;
    if (appointment.linkedLeadIds !== undefined)
        row.linked_lead_ids = appointment.linkedLeadIds;
    if (appointment.linkedDealIds !== undefined)
        row.linked_deal_ids = appointment.linkedDealIds;
    if (appointment.linkedProjectIds !== undefined)
        row.linked_project_ids = appointment.linkedProjectIds;
    if (appointment.parentAppointmentId !== undefined)
        row.parent_appointment_id = appointment.parentAppointmentId;
    if (appointment.tags !== undefined)
        row.tags = appointment.tags;
    if (appointment.color !== undefined)
        row.color = appointment.color;
    if (appointment.customColor !== undefined)
        row.custom_color = appointment.customColor;
    if (appointment.outcome !== undefined)
        row.outcome = appointment.outcome;
    if (appointment.outcomeNotes !== undefined)
        row.outcome_notes = appointment.outcomeNotes;
    if (appointment.rating !== undefined)
        row.rating = appointment.rating;
    if (appointment.actualDuration !== undefined)
        row.actual_duration = appointment.actualDuration;
    if (appointment.createdBy !== undefined)
        row.created_by = appointment.createdBy;
    if (appointment.updatedBy !== undefined)
        row.updated_by = appointment.updatedBy;
    if (appointment.cancelledBy !== undefined)
        row.cancelled_by = appointment.cancelledBy;
    if (appointment.cancelledReason !== undefined)
        row.cancelled_reason = appointment.cancelledReason;
    if (appointment.source !== undefined)
        row.source = appointment.source;
    if (appointment.sourceComponent !== undefined)
        row.source_component = appointment.sourceComponent;
    if (appointment.sourceModule !== undefined)
        row.source_module = appointment.sourceModule;
    if (appointment.sourceConversationId !== undefined)
        row.source_conversation_id = appointment.sourceConversationId;
    if (appointment.sourceLeadId !== undefined)
        row.source_lead_id = appointment.sourceLeadId;
    if (appointment.publicSubmissionId !== undefined)
        row.public_submission_id = appointment.publicSubmissionId;
    if (appointment.syncKey !== undefined)
        row.sync_key = appointment.syncKey;
    if (appointment.idempotencyKey !== undefined)
        row.idempotency_key = appointment.idempotencyKey;
    if (appointment.createdBySystem !== undefined)
        row.created_by_system = appointment.createdBySystem;
    if (appointment.needsReview !== undefined)
        row.needs_review = appointment.needsReview;
    if (appointment.generatedByAI !== undefined)
        row.generated_by_ai = appointment.generatedByAI;
    if (appointment.correlationId !== undefined)
        row.correlation_id = appointment.correlationId;
    if (appointment.bookingServiceId !== undefined)
        row.booking_service_id = appointment.bookingServiceId;
    if (appointment.ecommerceProductId !== undefined)
        row.ecommerce_product_id = appointment.ecommerceProductId;
    if (appointment.ecommerceOrderId !== undefined)
        row.ecommerce_order_id = appointment.ecommerceOrderId;
    if (appointment.paymentStatus !== undefined)
        row.payment_status = appointment.paymentStatus;
    if (appointment.metadata !== undefined)
        row.metadata = appointment.metadata;
    if (tenantId)
...
export async function getAppointmentsByProject(client, projectId, options = {}) {
    let query = client
        .from('project_appointments')
        .select('*')
    ...
    if (options.startDate)
        query = query.gte('start_date', toIso(options.startDate));
    if (options.endDate)
        query = query.lte('start_date', toIso(options.endDate));
...
export async function getBlockedTimesByProject(client, projectId, options = {}) {
    let query = client
        ...
        .order('start_date', { ascending: true });
    if (options.startDate)
        query = query.gte('end_date', toIso(options.startDate));
    if (options.endDate)
        query = query.lte('start_date', toIso(options.endDate));
    if (options.limit)
        query = query.limit(options.limit);
...
export async function checkAppointmentConflicts(client, projectId, startDate, endDate, excludeId) {
...
export async function createAppointmentCanonical(client, input) {
    if (!input.projectId)
        throw Object.assign(new Error('projectId is required.'), { status: 400, code: 'missing_project_id' });
    ...
    const resolveDuplicateAppointment = async (row) => {
        const appointment = mapAppointmentRowToAppointment(row);
        const appointmentNotes = normalizeString(input.notes, 6000);
        const readableAppointmentNotes = buildReadableChatbotCustomerRequestNote(appointmentNotes || (typeof appointment.metadata?.customerRequestSummary === 'string' ? appointment.metadata.customerRequestSummary : undefined), input.description || appointment.description, {
            customer: {
        ...
        const linked = await createOrLinkLeadForCanonicalAppointment(client, input, appointment, startIso, appointmentNotes, metadata, uniqueStrings(input.tags, appointment.tags));
        return {
    ...
    const linked = await createOrLinkLeadForCanonicalAppointment(client, input, appointment, startIso, appointmentNotes, metadata, tags);
    appointment = linked.appointment;
    ...
    if (eventResult.metadata)
        appointment = { ...appointment, metadata: eventResult.metadata };
...
export async function updateAppointmentCanonical(client, id, input) {
    const row = {
...
export async function confirmAppointmentCanonical(client, id, actorId) {
...
export async function markNoShowCanonical(client, id, actorId) {
    const appointment = await updateAppointmentCanonical(client, id, {
        status: 'no_show',
        updatedBy: actorId || undefined,
    });
...
export async function cancelAppointmentCanonical(client, id, reason, cancelledBy) {
    const { data, error } = await client
        .from('project_appointments')
        .update({
        status: 'cancelled',
...
export async function completeAppointmentCanonical(client, id, outcome, notes) {
    const { data, error } = await client
        .from('project_appointments')
        .update({
        status: 'completed',
...
export async function createBlockedTimeCanonical(client, input) {
    const startIso = toIso(input.startDate);
    const endIso = toIso(input.endDate);
    if (new Date(endIso) <= new Date(startIso)) {
...
export async function updateBlockedTimeCanonical(client, id, input) {
    const row = { updated_at: new Date().toISOString() };
    if (input.title !== undefined)
        row.title = input.title;
    if (input.startDate !== undefined)
        row.start_date = toIso(input.startDate);
    if (input.endDate !== undefined)
        row.end_date = toIso(input.endDate);
    if (input.allDay !== undefined)
        row.all_day = input.allDay;
...
export async function deleteBlockedTimeCanonical(client, id) {
    const { error } = await client.from('project_appointment_blocks').delete().eq('id', id);
    if (error)
        throw error;
}
export async function getAppointmentAvailability(client, projectId, options = {}) {
...
export async function getAvailableAppointmentSlots(client, projectId, options = {}) {
    ...
    while (cursorDay < endDate && slots.length < maxSlots) {
        ...
        while (slotStart < windowEnd && slots.length < maxSlots) {
            ...
            if (slotEnd > windowEnd || slotEnd > endDate)
                break;
...
export const createAppointmentFromLead = (client, input) => createAppointmentCanonical(client, {
    ...input,
    source: input.source || 'dashboard',
    sourceModule: input.sourceModule || 'crm',
    linkedLeadId: input.linkedLeadId || input.sourceLeadId,
});
export const createAppointmentFromChat = (client, input) => createAppointmentCanonical(client, {
    ...input,
    source: 'chatbot',
...
export const createAppointmentFromPublicBooking = (client, input) => createAppointmentCanonical(client, {
    ...input,
...
export const createAppointmentFromVerticalModule = (client, input) => createAppointmentCanonical(client, {
    ...input,
    source: input.source || 'dashboard',
    sourceModule: input.sourceModule || 'appointments',
});
//# sourceMappingURL=appointmentEngineService.js.map
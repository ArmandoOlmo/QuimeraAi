...
import { generateContent, generateImage } from './aiGateway.js';
function buildJobAuth(job) {
...
function generationInput(args, auth, sourceTool, overrides = {}) {
    return {
...
async function getProjectForTenant(supabase, projectId, tenantId) {
...
async function updateProjectData(supabase, projectId, updates) {
...
async function processFullProjectJob(supabase, job) {
    ...
    const pageJson = await generateContent(supabase, auth, generationInput(args, auth, 'ai_generate_full_project:page_json', {
        tenantId: job.tenant_id,
        projectId,
    ...
    if (args.apply !== false) {
        ...
        if (generated.page) {
            ...
            const existingIndex = pages.findIndex((item) => item.id === pageId || item.slug === page.slug);
            if (existingIndex >= 0)
        ...
        if (generated.legacyData && typeof generated.legacyData === 'object')
            updates.data = generated.legacyData;
        if (Array.isArray(generated.componentOrder))
            updates.component_order = generated.componentOrder;
        ...
        if (Object.keys(updates).length > 0)
            await updateProjectData(supabase, projectId, updates);
    }
    await updateJob(supabase, job.id, {
    ...
    for (const asset of assets) {
        try {
            const result = await generateImage(supabase, auth, generationInput({ ...args, ...asset }, auth, 'ai_generate_full_project:asset', {
                tenantId: job.tenant_id,
                projectId,
...
export async function processMcpGenerationJobs(supabase, limit = 3) {
...
//# sourceMappingURL=mcpJobRunner.js.map
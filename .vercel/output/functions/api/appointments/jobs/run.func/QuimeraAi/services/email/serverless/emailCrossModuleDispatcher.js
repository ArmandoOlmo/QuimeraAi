...
import { getEmailSettings } from './emailSettingsService.js';
import { processEmailOutbox } from './emailOutboxProcessor.js';
import { queueTransactionalEmail } from './emailTransactionalService.js';
const SENT_STATUSES = new Set(['sent', 'delivered', 'opened', 'clicked']);
const TERMINAL_SKIP_STATUSES = new Set(['skipped']);
export async function queueCrossModuleTransactionalEmail(input) {
...
export async function dispatchCrossModuleTransactionalEmail(input) {
    const queued = await queueCrossModuleTransactionalEmail(input);
    if (queued.status !== 'queued' || !input.provider)
        return queued;
...
export async function recordSkippedCrossModuleTransactionalEmail(input) {
    const settings = await getEmailSettings({ supabase: input.supabase, projectId: input.projectId });
    const existing = await findEmailLogByIdempotencyKey({
        supabase: input.supabase,
        projectId: input.projectId,
...
function withCrossModuleMetadata(input) {
    return {
        ...(input.metadata || {}),
...
function normalizeSkippedReason(reason) {
    ...
    if (value.includes('review') || value.includes('draft') || value.includes('ai-generated'))
        return 'needs_review';
    ...
    if (value.includes('disabled'))
        return 'disabled';
    if (value.includes('readiness') || value.includes('configured') || value.includes('sender'))
        return 'readiness_blocked';
    return 'policy_blocked';
}
function getTransactionalModuleOptIn(transactional, sourceModule, type, sourceEvent) {
    ...
    const enabledBy = (...keys) => keys.some(key => settings[key] === true);
    const disabledBy = (...keys) => keys.some(key => settings[key] === false);
    if (module === 'ecommerce') {
        if (flow.includes('order_confirmation'))
            return decision(enabledBy('orderConfirmation'), 'orderConfirmation');
        if (flow.includes('merchant_new_order') || flow.includes('new_order'))
            return decision(enabledBy('newOrderNotification'), 'newOrderNotification');
        if (flow.includes('payment_failed'))
            return decision(enabledBy('paymentFailed', 'orderPaymentFailed'), 'paymentFailed');
        if (flow.includes('low_stock'))
            return decision(enabledBy('lowStockNotification'), 'lowStockNotification');
        if (flow.includes('shipping'))
            return decision(enabledBy('orderShipped'), 'orderShipped');
        if (flow.includes('delivered'))
            return decision(enabledBy('orderDelivered'), 'orderDelivered');
        if (flow.includes('cancel'))
            return decision(enabledBy('orderCancelled'), 'orderCancelled');
        if (flow.includes('refund'))
            return decision(enabledBy('orderRefunded'), 'orderRefunded');
        if (flow.includes('review'))
            return decision(enabledBy('reviewRequest'), 'reviewRequest');
    ...
    if (module === 'appointments') {
        ...
        if (flow.includes('request'))
            return decision(!disabledBy('appointmentRequestReceived') && enabledBy('appointments', 'appointmentEmails', 'appointmentRequestReceived'), 'appointmentRequestReceived');
        if (flow.includes('confirm'))
            return decision(!disabledBy('appointmentConfirmation') && enabledBy('appointments', 'appointmentEmails', 'appointmentConfirmation'), 'appointmentConfirmation');
        if (flow.includes('cancel'))
            return decision(!disabledBy('appointmentCancellation') && enabledBy('appointments', 'appointmentEmails', 'appointmentCancellation'), 'appointmentCancellation');
        if (flow.includes('follow'))
            return decision(!disabledBy('appointmentFollowUp') && enabledBy('appointments', 'appointmentEmails', 'appointmentFollowUp'), 'appointmentFollowUp');
        if (flow.includes('remind'))
            return decision(!disabledBy('appointmentReminder') && enabledBy('appointments', 'appointmentEmails', 'appointmentReminder'), 'appointmentReminder');
    ...
    if (module === 'realty') {
        if (flow.includes('showing'))
            return decision(enabledBy('realty', 'realtyShowingRequest'), 'realtyShowingRequest');
        if (flow.includes('open_house'))
            return decision(enabledBy('realty', 'realtyOpenHouseRegistration'), 'realtyOpenHouseRegistration');
        if (flow.includes('inquiry') || flow.includes('property'))
            return decision(enabledBy('realty', 'realtyPropertyInquiry'), 'realtyPropertyInquiry');
...
function canResumeReviewedSkippedIntent(existing, metadata) {
    const data = metadata || {};
...
function objectHasReviewDraftFlag(value) {
...
//# sourceMappingURL=emailCrossModuleDispatcher.js.map
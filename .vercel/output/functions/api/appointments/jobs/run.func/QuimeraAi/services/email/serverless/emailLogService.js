...
export async function updateEmailLogStatus(input) {
    ...
    if (input.skippedReason !== undefined)
        updates.skipped_reason = input.skippedReason;
    if (input.metadata)
        updates.metadata = input.metadata;
    if (input.status === 'sent')
        updates.sent_at = input.eventAt || new Date().toISOString();
    if (input.status === 'delivered')
        updates.delivered_at = input.eventAt || new Date().toISOString();
    if (input.status === 'clicked')
        updates.clicked_at = input.eventAt || new Date().toISOString();
    if (input.status === 'bounced')
        updates.bounced_at = input.eventAt || new Date().toISOString();
    if (input.status === 'complained')
        updates.complained_at = input.eventAt || new Date().toISOString();
    const { data, error } = await input.supabase
        .from('email_logs')
        .update(updates)
        .eq('id', input.id)

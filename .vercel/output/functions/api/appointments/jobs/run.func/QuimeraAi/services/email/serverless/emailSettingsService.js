import { buildFromHeader, isValidEmail } from './emailProviderService.js';
...
export function getEmailReadiness(settings) {
    ...
    if (!providerConfigured)
        readinessBlockers.push('Email provider is not explicitly configured.');
    if (!senderConfigured)
        readinessBlockers.push('Sender name and from email are required.');
    if (!marketingEnabled)
        readinessBlockers.push('Email Marketing is not explicitly enabled.');
    if (!unsubscribeConfigured)
        readinessBlockers.push('Marketing unsubscribe footer must be enabled.');
    if (!suppressionConfigured)
        readinessBlockers.push('Suppression checks must be enabled.');
    if (settings.sendingDomain && !domainVerified)
        warnings.push('Sending domain is not verified yet.');
    if (!settings.webhookConfigured)
        warnings.push('Provider webhook is not configured; analytics may be incomplete.');
    if (!testEmailSent)
        warnings.push('No test email has been recorded for this project.');
    return {
        providerConfigured,
        senderConfigured,
        marketingEnabled,
...
export function assertEmailReadiness(settings, mode) {
    if (mode === 'test' && settings.readiness.canSendTest)
        return;
    if (mode === 'marketing' && settings.readiness.canSendMarketing)
        return;
    if (mode === 'transactional' && settings.readiness.canSendTransactional)
        return;
    throw new Error(`Email sending is blocked: ${settings.readiness.readinessBlockers.join(' ') || 'readiness incomplete'}`);
}
export function getSenderHeader(settings) {
    return buildFromHeader({ fromEmail: settings.fromEmail, fromName: settings.fromName });
}
function mapSettingsUpdatesToRow(projectId, userId, updates) {
    const row = {
...
//# sourceMappingURL=emailSettingsService.js.map
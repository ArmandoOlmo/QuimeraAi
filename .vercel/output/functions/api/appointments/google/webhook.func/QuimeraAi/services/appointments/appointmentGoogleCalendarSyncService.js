...
function requireEnvValue(value, name) {
    ...
    if (!normalized)
        throw Object.assign(new Error(`${name} is required for Google Calendar sync.`), { status: 500 });
    return normalized;
}
function resolveGoogleEnv(env = {}) {
    return {
...
export function encryptGoogleCalendarTokens(tokens, encryptionKey) {
...
export function decryptGoogleCalendarTokens(encrypted, encryptionKey) {
    ...
    const decipher = createDecipheriv('aes-256-gcm', resolveEncryptionKey(encryptionKey), base64UrlDecode(String(payload.iv || '')));
...
export function signGoogleCalendarOAuthState(payload, stateSecret) {
...
export function verifyGoogleCalendarOAuthState(state, stateSecret, maxAgeMs = 15 * 60 * 1000) {
...
export function buildGoogleCalendarOAuthUrl(payload, env = {}) {
...
export async function exchangeGoogleCalendarCode(code, env = {}, fetchImpl = fetch) {
    const resolved = resolveGoogleEnv(env);
    ...
    const data = await fetchGoogleJson(GOOGLE_TOKEN_URL, {
        method: 'POST',
...
export async function assertGoogleCalendarProjectAccess(client, userId, projectId) {
...
export async function upsertGoogleCalendarIntegration(client, input, env = {}) {
    const resolved = resolveGoogleEnv(env);
...
export async function getGoogleCalendarIntegrationStatus(client, projectId, calendarId = DEFAULT_CALENDAR_ID) {
    const { data, error } = await client
        .from('project_google_calendar_integrations')
        .select('*')
    ...
    const row = data;
    return {
        configured: Boolean((process.env.GOOGLE_CALENDAR_CLIENT_ID || process.env.VITE_GOOGLE_CLIENT_ID)
...
export async function disconnectGoogleCalendarIntegration(client, projectId, calendarId = DEFAULT_CALENDAR_ID) {
...
export async function createOrRenewGoogleCalendarWatch(client, integration, options = {}) {
    ...
    const address = requireEnvValue(options.webhookUrl ?? process.env.GOOGLE_CALENDAR_WEBHOOK_URL ?? buildWebhookUrlFromAppBase(), 'GOOGLE_CALENDAR_WEBHOOK_URL');
    const calendarId = encodeURIComponent(integration.calendar_id || DEFAULT_CALENDAR_ID);
    const response = await fetchGoogleJson(`${GOOGLE_CALENDAR_API}/calendars/${calendarId}/events/watch`, {
        method: 'POST',
...
export async function markGoogleCalendarWebhookPending(client, headers, now = new Date()) {
    ...
    if (!channelId)
        return { matched: false, reason: 'missing_channel_id' };
    const { data, error } = await client
        .from('project_google_calendar_integrations')
        .select('*')
    ...
    if (!integration)
        return { matched: false, reason: 'unknown_channel' };
...
export async function processGoogleCalendarSyncJobs(client, options = {}) {
...
export async function syncGoogleCalendarIntegration(client, integration, options = {}) {
    const now = new Date(toIso(options.now));
...
async function getValidGoogleAccessToken(client, integration, options) {
    ...
    if (tokens.accessToken && expiresAt > nowMs + TOKEN_REFRESH_SKEW_MS)
    ...
    const refreshed = normalizeTokenResponse(await fetchGoogleJson(GOOGLE_TOKEN_URL, {
        method: 'POST',
        headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
        body,
    }, options.fetchImpl || fetch));
...
async function pushQuimeraAppointmentsToGoogle(client, integration, accessToken, options) {
    ...
    if (error)
        throw error;
    const summary = { pushed: 0, skipped: 0, errors: [] };
...
async function upsertAppointmentToGoogleEvent(appointment, integration, accessToken, options) {
    const calendarId = encodeURIComponent(integration.calendar_id || DEFAULT_CALENDAR_ID);
    ...
    if (eventId) {
        try {
            return await fetchGoogleJson(`${GOOGLE_CALENDAR_API}/calendars/${calendarId}/events/${encodeURIComponent(eventId)}?conferenceDataVersion=1&sendUpdates=all`, {
                method: 'PATCH',
                headers: googleHeaders(accessToken),
    ...
    return fetchGoogleJson(`${GOOGLE_CALENDAR_API}/calendars/${calendarId}/events?conferenceDataVersion=1&sendUpdates=all`, {
        method: 'POST',
        headers: googleHeaders(accessToken),
        body: JSON.stringify(resource),
    }, fetchImpl);
}
async function pullGoogleEventsToQuimera(client, integration, accessToken, options) {
    const summary = { imported: 0, updated: 0, reviewQueued: 0, skipped: 0, errors: [], syncToken: integration.sync_token || null };
    let pageToken;
    let clearedExpiredToken = false;
    while (true) {
        const url = buildGoogleEventsListUrl(integration, options, pageToken, clearedExpiredToken);
        try {
            const response = await fetchGoogleJson(url, { headers: googleHeaders(accessToken) }, options.fetchImpl || fetch);
            ...
            if (response.nextSyncToken)
                summary.syncToken = response.nextSyncToken;
            if (!pageToken)
                break;
        }
        catch (error) {
...
export async function upsertGoogleCalendarEventIntoAppointments(client, integration, event, now = new Date()) {
    ...
    if (event.status === 'cancelled' && !existing)
        return { status: 'skipped' };
...
function buildGoogleEventsListUrl(integration, options, pageToken, forceFullSync) {
    const calendarId = encodeURIComponent(integration.calendar_id || DEFAULT_CALENDAR_ID);
...
function googleEventToAppointmentRow(event, integration, now) {
    const calendarId = integration.calendar_id || DEFAULT_CALENDAR_ID;
    const parsedDate = parseGoogleEventDate(event);
    const syncKey = buildGoogleSyncKey(event, calendarId);
    if (!parsedDate || !syncKey)
        return null;
...
async function queueGoogleExternalReview(client, existing, event, calendarId, now) {
    const syncKey = buildGoogleSyncKey(event, calendarId);
    await updateOrThrow(client
        .from('project_appointments')
        .update({
...
async function findAppointmentById(client, projectId, appointmentId) {
    const { data, error } = await client
        .from('project_appointments')
        .select('*')
        .eq('id', appointmentId)
...
async function findAppointmentBySyncKey(client, projectId, syncKey) {
    const { data, error } = await client
        .from('project_appointments')
        .select('*')
        .eq('project_id', projectId)
...
function appointmentRowToGoogleEvent(appointment) {
    const event = {
    ...
    if (location.type === 'physical' && location.address)
        event.location = String(location.address);
    if (location.type === 'virtual' && location.meetingUrl)
        event.location = String(location.meetingUrl);
    if (location.type === 'phone' && location.phoneNumber)
        event.location = `Tel: ${location.phoneNumber}`;
    const attendees = Array.isArray(appointment.participants) ? appointment.participants : [];
    event.attendees = attendees
        .filter((participant) => normalizeString(participant.email, 320))
        .map((participant) => ({
...
function parseGoogleEventDate(event) {
...
function buildGoogleCalendarMetadata(event, calendarId, syncKey) {
    return {
        source: DEFAULT_PROVIDER,
...
function mapGoogleParticipants(event) {
...
function mapGoogleLocation(event) {
    const meetUrl = event.conferenceData?.entryPoints?.find(entry => entry.entryPointType === 'video')?.uri;
    if (meetUrl)
        return { type: 'virtual', meetingUrl: meetUrl, platform: 'google_meet' };
    if (!event.location)
        return { type: 'virtual' };
    if (event.location.startsWith('http'))
        return { type: 'virtual', meetingUrl: event.location };
    if (event.location.startsWith('Tel:'))
        return { type: 'phone', phoneNumber: event.location.replace(/^Tel:\s*/, '') };
    return { type: 'physical', address: event.location };
}
function mapGoogleReminders(event) {
...
function buildGoogleReminderPayload(reminders) {
    const list = Array.isArray(reminders) ? reminders : [];
    const enabled = list.filter((reminder) => reminder.enabled !== false);
    if (!enabled.length)
        return { useDefault: true };
    return {
        useDefault: false,
        overrides: enabled.map((reminder) => ({
            method: reminder.type === 'email' ? 'email' : 'popup',
            minutes: Number(reminder.minutesBefore || 60),
        })),
    };
}
function stripGoogleDescriptionMetadata(description) {
    if (!description)
        return '';
    return description
        .split('\n')
...
function extractGoogleTags(description) {
    if (!description)
        return [];
    const matches = description.match(/#([a-zA-Z0-9_-]+)/g) || [];
    return Array.from(new Set(matches.map(tag => tag.slice(1))));
}
function shouldRenewWatch(integration, now) {
    if (!integration.watch_channel_id || !integration.watch_expires_at)
        return true;
...
function buildWebhookUrlFromAppBase() {
...
function normalizeTokenResponse(data) {
    ...
    if (!accessToken)
        throw Object.assign(new Error('Google Calendar token response is missing access_token.'), { status: 502 });
    return {
        accessToken,
...
async function fetchGoogleJson(url, init, fetchImpl) {
...
//# sourceMappingURL=appointmentGoogleCalendarSyncService.js.map
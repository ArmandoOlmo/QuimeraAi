...
exports.getOnboardingStatus = exports.autoProvisionClient = void 0;
 ...
 */
async function verifyAgencyOwner(userId) {
    ...
    if (membershipsSnapshot.empty) {
        throw new functions.https.HttpsError('permission-denied', 'Only agency owners can provision new clients');
    }
    ...
    if (!tenantData ||
        !['agency', 'agency_plus', 'enterprise'].includes(tenantData.subscriptionPlan)) {
        throw new functions.https.HttpsError('permission-denied', 'Only agency plans can provision sub-clients');
    }
    return tenantId;
}
/**
 ...
 */
async function createProjectFromTemplate(tenantId, templateId, projectName, industry) {
 ...
 */
exports.autoProvisionClient = functions.https.onCall(async (data, context) => {
    ...
    try {
        ...
        if (!limitCheck.canCreate) {
            throw new functions.https.HttpsError('resource-exhausted', `Sub-client limit reached (${limitCheck.current}/${limitCheck.limit}). Upgrade your plan or add more slots.`);
        }
        ...
        if (data.projectTemplate) {
            try {
                projectId = await createProjectFromTemplate(clientTenantId, data.projectTemplate, `Sitio Web - ${data.businessName}`, data.industry);
            }
            catch (error) {
        ...
        for (const userData of data.initialUsers) {
            try {
                ...
                const expiresAt = firestore_1.Timestamp.fromDate(new Date(Date.now() + 7 * 24 * 60 * 60 * 1000) // 7 days
                );
 ...
 */
exports.getOnboardingStatus = functions.https.onCall(async (data, context) => {
    if (!context.auth) {
        throw new functions.https.HttpsError('unauthenticated', 'User must be authenticated');
    }
...
//# sourceMappingURL=autoProvisionClient.js.map
...
exports.deleteSavedReport = exports.getSavedReport = exports.generateConsolidatedReport = void 0;
 ...
 */
exports.generateConsolidatedReport = functions.https.onCall(async (data, context) => {
    try {
        ...
        if (clientIds.length === 0) {
            throw new functions.https.HttpsError('failed-precondition', 'No clients found for this agency');
        }
        // Parse date range
        const dateRange = {
            start: firestore_1.Timestamp.fromDate(new Date(data.dateRange.start)),
            end: firestore_1.Timestamp.fromDate(new Date(data.dateRange.end)),
        };
        // Generate report data
        const reportData = await aggregateReportData(data.agencyTenantId, clientIds, dateRange, data.metrics);
...
async function aggregateReportData(agencyTenantId, clientIds, dateRange, metrics) {
    ...
    for (const clientId of clientIds) {
        ...
        const clientMetric = await getClientMetrics(clientId, clientData.name, dateRange, metrics);
    ...
    const summary = {
        ...
        avgConversionRate: clientMetrics.length > 0
    ...
    const underperformingClients = clientMetrics
        ...
        recommendation: c.conversionRate < 5
    ...
    if (clientMetrics.filter((c) => c.conversionRate < 5).length > 0) {
        recommendations.push(`${clientMetrics.filter((c) => c.conversionRate < 5).length} clientes tienen tasas de conversión bajas. Considera revisar sus funnels.`);
    }
    if (clientMetrics.filter((c) => c.bounceRate > 70).length > 0) {
        recommendations.push(`${clientMetrics.filter((c) => c.bounceRate > 70).length} clientes tienen bounce rates altos. Revisa la experiencia de usuario.`);
    }
    return {
        summary,
 ...
 */
async function getClientMetrics(clientId, clientName, dateRange, metrics) {
    const result = {
        clientId,
        clientName,
        totalLeads: 0,
 ...
 */
async function verifyAgencyAccess(userId, agencyTenantId) {
    try {
 ...
 */
exports.getSavedReport = functions.https.onCall(async (data, context) => {
    try {
        if (!context.auth) {
            throw new functions.https.HttpsError('unauthenticated', 'User must be authenticated');
        }
 ...
 * Delete saved report
 */
exports.deleteSavedReport = functions.https.onCall(async (data, context) => {
    try {
        if (!context.auth) {
            throw new functions.https.HttpsError('unauthenticated', 'User must be authenticated');
        }
...
//# sourceMappingURL=generateConsolidatedReport.js.map
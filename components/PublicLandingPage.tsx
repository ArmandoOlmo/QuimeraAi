...
import { ArrowRight, Zap, Layout, Palette, Image as ImageIcon, MessageSquare, BarChart3, Check } from 'lucide-react';
        ...
        <div className="container mx-auto px-6 py-4">
          <div className="flex items-center justify-between">
            ...
            <a href="#" className="flex items-center gap-3">
              <img src={QUIMERA_LOGO} alt="Quimera.ai" className="w-10 h-10" />
        ...
        <div className="max-w-4xl mx-auto text-center">
          ...
          <div className="w-48 h-48 mx-auto mb-8">
            <img
              src={QUIMERA_LOGO}
              alt="Quimera AI"
              className="w-full h-full object-contain drop-shadow-[0_0_50px_rgba(250,204,21,0.6)]"
            />
          </div>
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center mb-16">
            <button 
              onClick={onNavigateToRegister}
              className="px-8 py-4 bg-yellow-400 text-black font-bold rounded-xl hover:bg-yellow-300 transition-colors flex items-center justify-center gap-2"
            >
              {t('landing.startFree')}
        ...
        <div className="container mx-auto px-6">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {features.map((feature, index) => (
              <div 
                key={index}
                className="p-8 bg-white/5 border border-white/10 rounded-2xl hover:bg-white/10 hover:border-yellow-400/30 transition-colors"
              >
                <div className="w-12 h-12 bg-yellow-400/10 rounded-xl flex items-center justify-center text-yellow-400 mb-6">
                  {feature.icon}
                </div>
            ...
            {pricingPlans.map((plan, index) => (
              ...
              >
                ...
                <div className="mb-6">
                  ...
                  <span className="text-gray-500">{plan.periodKey ? t(plan.periodKey) : ''}</span>
                </div>
      ...
      <section className="py-24 bg-[#0A0A0A]">
        <div className="container mx-auto px-6">
          <div className="max-w-3xl mx-auto text-center">
            ...
            <button 
              onClick={onNavigateToRegister}
              className="px-10 py-5 bg-yellow-400 text-black font-bold text-lg rounded-xl hover:bg-yellow-300 transition-colors inline-flex items-center gap-3"
            >
              {t('landing.startFree')}
              <ArrowRight className="w-6 h-6" />
            </button>
            <p className="mt-4 text-gray-500 text-sm">{t('landing.noCreditCard')}</p>
          </div>
        </div>
      </section>

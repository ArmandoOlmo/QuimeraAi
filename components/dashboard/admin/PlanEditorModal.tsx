...
const PlanEditorModal: React.FC<PlanEditorModalProps> = ({ isOpen, onClose, planToEdit, serviceModules, onSave }) => {
    ...
    return (
        <Modal isOpen={isOpen} onClose={onClose}>
            <div className="p-6 border-b border-editor-border flex justify-between items-center"><h2 className="text-lg font-semibold text-white">{planToEdit ? t('superadmin.billing.editPlan') : t('superadmin.billing.createPlanTitle')}</h2><button onClick={onClose} className="p-1 rounded-full hover:bg-editor-border"><X /></button></div>
            <form onSubmit={handleSubmit}>
                <div className="p-6 md:p-8 overflow-y-auto space-y-4 max-h-[70vh]">
                    <div><label htmlFor="plan-name" className="block text-sm font-medium text-editor-text-secondary mb-1">{t('superadmin.billing.planName')}</label><input id="plan-name" name="name" type="text" value={formData.name} onChange={handleChange} required className="w-full bg-editor-bg text-white p-2 rounded-md border border-editor-border focus:ring-2 focus:ring-editor-accent" /></div>
                    <div><label htmlFor="plan-description" className="block text-sm font-medium text-editor-text-secondary mb-1">{t('superadmin.billing.description')}</label><textarea id="plan-description" name="description" value={formData.description} onChange={handleChange} rows={2} className="w-full bg-editor-bg text-white p-2 rounded-md border border-editor-border focus:ring-2 focus:ring-editor-accent" /></div>
                    <div className="grid grid-cols-2 gap-4">
                        <div><label htmlFor="plan-price-monthly" className="block text-sm font-medium text-editor-text-secondary mb-1">{t('superadmin.billing.monthlyPriceLabel')}</label><input id="plan-price-monthly" type="number" min="0" value={formData.price.monthly} onChange={e => handlePriceChange('monthly', e.target.value)} required className="w-full bg-editor-bg text-white p-2 rounded-md border border-editor-border focus:ring-2 focus:ring-editor-accent" /></div>
                        <div><label htmlFor="plan-price-annually" className="block text-sm font-medium text-editor-text-secondary mb-1">{t('superadmin.billing.annualPriceLabel')}</label><input id="plan-price-annually" type="number" min="0" value={formData.price.annually} onChange={e => handlePriceChange('annually', e.target.value)} required className="w-full bg-editor-bg text-white p-2 rounded-md border border-editor-border focus:ring-2 focus:ring-editor-accent" /></div>
                    </div>
                    <div>
                        <label className="block text-sm font-medium text-editor-text-secondary mb-2">{t('superadmin.billing.features')}</label>
                        <div className="space-y-2">
                            {formData.features.map((feature, index) => (
                                <div key={index} className="flex items-center space-x-2">
                                    <input type="text" value={feature} onChange={e => handleFeatureChange(index, e.target.value)} placeholder={t('superadmin.billing.featurePlaceholder')} className="flex-grow bg-editor-bg text-white p-2 rounded-md border border-editor-border focus:ring-2 focus:ring-editor-accent" />
                        ...
                        <button type="button" onClick={addFeature} className="mt-2 flex items-center text-xs font-semibold py-1 px-3 text-editor-accent hover:text-editor-accent/80 transition-colors"><Plus size={14} className="mr-1" /> {t('superadmin.billing.addFeature')}</button>
                    </div>
                    <div>
                        <label className="block text-sm font-medium text-editor-text-secondary mb-2">{t('superadmin.billing.includedModules')}</label>
                        <div className="space-y-2">
                    ...
                    <div className="flex items-center justify-between p-3 rounded-md bg-editor-bg border border-editor-border">
                        <label htmlFor="plan-featured" className="text-sm font-medium text-white">{t('superadmin.billing.markFeatured')}</label>
                ...
                <div className="p-6 bg-editor-panel-bg/50 border-t border-editor-border flex justify-end items-center space-x-3">
                    <button type="button" onClick={onClose} className="font-semibold py-2 px-5 rounded-lg hover:bg-editor-border">{t('superadmin.billing.cancel')}</button>
                    <button type="submit" disabled={isLoading} className="text-editor-accent font-bold py-2 px-4 hover:text-editor-accent-hover disabled:opacity-50 flex items-center transition-colors">{isLoading && <div className="w-4 h-4 border-2 border-editor-accent border-t-transparent rounded-full animate-spin mr-2"></div>}{isLoading ? t('superadmin.billing.saving') : t('superadmin.billing.savePlan')}</button>
                </div>
            </form>
        </Modal>
    );

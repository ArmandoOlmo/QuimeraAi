...
import { LogOut, LayoutDashboard, Globe, Settings, ChevronLeft, ChevronRight, Zap, User as UserIcon, PenTool, Menu as MenuIcon, Sun, Moon, Circle, MessageSquare, Users, Link2, Search, DollarSign, GripVertical, LayoutTemplate, Calendar, X } from 'lucide-react';
import {
  DndContext,
  closestCenter,
  KeyboardSensor,
...
const DashboardSidebar: React.FC<DashboardSidebarProps> = ({ isMobileOpen, onClose, hiddenOnDesktop = false, defaultCollapsed = false }) => {
  ...
  const defaultNavItems: NavItemData[] = [
    { id: 'dashboard', icon: LayoutDashboard, label: t('dashboard.title'), view: 'dashboard', route: ROUTES.DASHBOARD, isFixed: true },
            ...
            >
              ...
              >
                {navItems.map((item, index) => (
                  <SortableNavItem key={item.id} item={item} index={index} />
                ))}
              </SortableContext>
            </DndContext>

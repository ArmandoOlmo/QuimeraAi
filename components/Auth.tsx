...
const Auth: React.FC<AuthProps> = ({ onVerificationEmailSent }) => {
    ...
    const featureCards = [
        { icon: <Zap className="text-[#1A0D26]" size={24} />, title: t('auth.authCard1Title'), desc: t('auth.authCard1Desc') },
        { icon: <Layout className="text-[#1A0D26]" size={24} />, title: t('auth.authCard2Title'), desc: t('auth.authCard2Desc') },
        { icon: <Sparkles className="text-[#1A0D26]" size={24} />, title: t('auth.authCard3Title'), desc: t('auth.authCard3Desc') },
        { icon: <ImageIcon className="text-[#1A0D26]" size={24} />, title: t('auth.authCard4Title'), desc: t('auth.authCard4Desc') },
        { icon: <Hexagon className="text-[#1A0D26]" size={24} />, title: t('auth.authCard5Title'), desc: t('auth.authCard5Desc') },
        { icon: <PlayCircle className="text-[#1A0D26]" size={24} />, title: t('auth.authCard6Title'), desc: t('auth.authCard6Desc') },
    ];
                    ...
                    {authMode === 'forgotPassword' && resetEmailSentTo ? (
                        ...
                        <form onSubmit={handleAuthAction} className="space-y-3 sm:space-y-4">
                            {authMode === 'register' && (
                                <div className="space-y-3 sm:space-y-4 animate-fade-in-up">
                                    <div>
                                        ...
                                        <input type="text" value={name} onChange={(e) => setName(e.target.value)} required className="w-full bg-black/40 text-white p-2.5 sm:p-3 rounded-lg border border-white/10 focus:ring-2 focus:ring-yellow-500/50 outline-none transition-all text-sm" placeholder={t('auth.yourNamePlaceholder')} />
                                    </div>
                                    <div>
                            ...
                            <div>
                                ...
                                <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} required className="w-full bg-black/40 text-white p-2.5 sm:p-3 rounded-lg border border-white/10 focus:ring-2 focus:ring-yellow-500/50 outline-none transition-all text-sm" placeholder={t('auth.emailPlaceholder')} />
                            </div>
                            ...
                            {authMode !== 'forgotPassword' && (
                                <div>
                                    ...
                                    <div className="relative">
                                        <input type={showPassword ? "text" : "password"} value={password} onChange={(e) => setPassword(e.target.value)} required className="w-full bg-black/40 text-white p-2.5 sm:p-3 rounded-lg border border-white/10 focus:ring-2 focus:ring-yellow-500/50 outline-none transition-all pr-10 text-sm" placeholder={t('auth.passwordPlaceholder')} />
                            ...
                            {authMode === 'register' && (
                                <div className="animate-fade-in-up">
                                    ...
                                    <input type="password" value={repeatPassword} onChange={(e) => setRepeatPassword(e.target.value)} required className="w-full bg-black/40 text-white p-2.5 sm:p-3 rounded-lg border border-white/10 focus:ring-2 focus:ring-yellow-500/50 outline-none transition-all text-sm" placeholder={t('auth.passwordPlaceholder')} />
                                </div>
                            )}
        ...
        <div className="min-h-screen bg-[#050505] text-white font-sans selection:bg-yellow-500/30 selection:text-yellow-200">
                        ...
                        <div className="relative bg-[#0A0A0A] border border-white/10 rounded-xl overflow-hidden shadow-2xl aspect-video">
                            ...
                            <div className="p-0 h-full bg-cover bg-center opacity-80" style={{ backgroundImage: `url('https://images.unsplash.com/photo-1618005182384-a83a8bd57fbe?q=80&w=2564&auto=format&fit=crop')` }}>
                                <div className="h-full w-full bg-black/50 flex items-center justify-center">
                                    <div className="text-center">
                                        <h3 className="text-4xl font-bold text-white mb-2">{t('auth.demo.luxuryRedefined')}</h3>
                                        <button className="px-6 py-2 bg-white text-black rounded-full text-sm font-bold">{t('auth.demo.exploreCollection')}</button>
                                    </div>
                                </div>
                            </div>
                        </div>
            ...
            <div className="py-12 border-y border-white/5 bg-white/[0.02]">
                <div className="container mx-auto px-6 text-center">
                    <p className="text-xs font-bold text-gray-500 uppercase tracking-widest mb-8">{t('auth.trustedBy')}</p>
            ...
            <section id="features" className="py-24 bg-[#050505] relative">
                <div className="container mx-auto px-6">
                    <div className="text-center max-w-2xl mx-auto mb-20">
                        <h2 className="text-3xl md:text-5xl font-bold mb-6 text-white">{t('auth.whyDifferent')}</h2>
                        <p className="text-gray-400 text-lg">{t('auth.whyDifferentSubtitle')}</p>
                    </div>

                    <div className="grid md:grid-cols-3 gap-8">
                        {[
                            { title: t('auth.instantCreation'), desc: t('auth.instantCreationDesc'), icon: <Zap className="text-yellow-400" size={32} /> },
                            { title: t('auth.aiWriter'), desc: t('auth.aiWriterDesc'), icon: <Layout className="text-purple-400" size={32} /> },
                            { title: t('auth.globalSystem'), desc: t('auth.globalSystemDesc'), icon: <Sparkles className="text-blue-400" size={32} /> }
                        ].map((feature, i) => (
                ...
                <div className="container mx-auto px-6 flex flex-col lg:flex-row items-center gap-16">
                    <div className="lg:w-1/2">
                        ...
                        <ul className="space-y-4 mb-8">
                            {[t('auth.imageGenFeat1'), t('auth.imageGenFeat2'), t('auth.imageGenFeat3'), t('auth.imageGenFeat4')].map(item => (
                                <li key={item} className="flex items-center text-gray-300">
                                    <CheckCircle size={20} className="text-green-500 mr-3" /> {item}
                                </li>
                            ))}
                        </ul>
                        <button onClick={() => openAuth('register')} className="text-white border-b border-yellow-400 pb-1 hover:text-yellow-400 transition-colors">{t('auth.tryItYourself')} &rarr;</button>
                    </div>
                    ...
                    <div className="flex flex-col md:flex-row gap-16">
                        <div className="md:w-1/3">
                            <div className="inline-flex items-center gap-2 text-yellow-400 font-bold text-sm uppercase tracking-widest mb-4">
                                <HelpCircle size={18} /> {t('auth.faqTitle')}
                            </div>
                            <h2 className="text-4xl font-bold text-white mb-6">{t('auth.commonQuestions')}</h2>
                            <p className="text-gray-400 text-lg mb-8">
                                {t('auth.faqSubtitle')}
                            </p>
